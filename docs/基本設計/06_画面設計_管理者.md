...
| 版 | 1.5（1.1 版: レビュー指摘 20 件への対応。1.2 版: 最終点検で 07 D07-15・01 D01-35・00 1.1 版との整合を反映。1.3 版: K-06（Supabase → Firebase）に伴う部分改版。1.4 版: Firebase 化後の分冊間整合（02・04 の 2.1 版への回答を反映）。1.5 版: 10 K-19 の反映。変更点は §13） |
| 作成日 | 2026-09-17（1.1 版: 2026-09-21、1.2 版: 2026-09-21、1.3 版: 2026-09-21、1.4 版: 2026-09-21、1.5 版: 2026-09-24） |
| 対象 | 管理者画面（S-01、S-05〜S-11 と付随画面）、文言の出し分け、色規則、グラフ部品を実装する担当者。07（PDF）、04（API）、08（テスト）の担当者 |

## 0. 本書の位置づけ

本書は、管理者（`admin_user`）が使う画面と、その画面に載るグラフ・文言・色の設計を確定するものです。共通定義（`docs/基本設計/00_共通定義.md`。以下「00」）の用語・識別子・コレクション名・フィールド名・型名・API 規約をそのまま使い、本書で別名を作りません。
...
- 本書が定めないもの（参照のみ）: システム構成・ライブラリ版・Firebase プロジェクト設定（01 分冊）、Firestore のフィールド定義・セキュリティルール・認証の仕組み（セッション Cookie、カスタムクレーム。02 分冊）、採点式と比較計算の純関数（03 分冊）、API の入出力・エラーコード・監査ログの書き込み（04 分冊）、受検者画面（05 分冊）、AI 解説の生成処理と PDF の生成方式（07 分冊）、実装順とテスト計画（08 分冊）。
...
| 02（Firestore データモデル） | 画面が参照するコレクション・フィールド名（00 §2.2 と同一）、セッション Cookie・招待トークン・受検者トークンの仕組み、監査ログの記録対象。K-06 で全面改版されたため節番号では参照しない |
...
| # | 決定事項 | 本書での対応 |
|---|---|---|
| 1 | Next.js（App Router、TypeScript）+ Firebase（Cloud Firestore、Firebase Authentication）+ Vercel。グラフは ApexCharts（react-apexcharts）（10 K-06） | §6 のレーダー 3 種は `react-apexcharts`。ゲージ・スライダー・評価レターは自前 SVG／CSS（設計判断 D06-01。§6.1）。画面は `app/(admin)/admin/` 配下（00 §3.3）。ログイン・パスワード再設定だけがブラウザで Firebase Auth クライアント SDK（`lib/firebase/client.ts`）を使い、認証状態はサーバのセッション Cookie（`POST /auth/session`）に一本化する（00 §2.5、§4.1、D-31）。画面から Firestore には触れない |
...
| M-02 | 管理者サインアップ | S-01「アカウント作成」、§4「管理者追加」 | `/admin/signup?q={inviteToken}` | 不要 | なし（登録後は `admin`。00 §4.2、§5） |
| M-03 | パスワード再設定（初回のパスワード設定を兼ねる） | S-01「パスワードを忘れた方」、§9 認証 | `/admin/password-reset`（メールのリンク経由は `?mode=resetPassword&oobCode=…`） | 不要（メールのリンクに含まれる `oobCode` で新パスワードを設定） | なし |
...
| P-07 | 招待リンク再発行の確認 | （既存に無し。00 §4.2 `POST /api/v1/admin/organization/invite-token`） | M-07 内 | 必要 | `owner`／`super_admin` のみ |
| E-01 | 利用不可（停止中・未所属） | 00 §5、04 §2.4 | 全管理画面（専用 URL は置かず、各ページの Server Component が本文の代わりに描画する。§1.3） | ログイン済み | `adminUsers.isSuspended`／`deletedAt`（04 の 403 `ADMIN_SUSPENDED`）、`adminUsers` 文書なし・クレーム（`organizationId`、`role`）なし（403 `ADMIN_NOT_REGISTERED`） |
...
```mermaid
flowchart TD
    L["M-01 ログイン /admin/login"] -->|signInWithEmailAndPassword → POST /auth/session → login-events| LIST["M-04 回答一覧 /admin"]
    L -->|パスワードを忘れた方| PR["M-03 パスワード再設定 /admin/password-reset"]
    L -->|アカウント作成（招待リンクの案内）| SU["M-02 サインアップ /admin/signup?q=…"]
    SU -->|POST /auth/invite 成功 → sendPasswordResetEmail → T-35 を表示| SU
    SU -.->|メールのリンク（パスワード設定）| PR2
    PR -->|メールのリンク → /admin/password-reset?mode=resetPassword&oobCode=…| PR2["M-03 新しいパスワードの入力"]
    PR2 -->|confirmPasswordReset → signInWithEmailAndPassword → POST /auth/session → login-events| LIST
...
認証の仕組み（Firebase Auth のセッション Cookie とカスタムクレーム。00 §4.1、§5）は 02、API 側の認可は 04 分冊です。画面側は次のとおり振る舞います。
...
| ログイン済みで M-01 にアクセス | `/admin` へリダイレクト（`login/page.tsx` の Server Component が 02 §9.11 の `verifyAdminSessionCookie()`（`lib/auth/session-cookie.ts`）で有効な Cookie を確認したときだけ。無効・期限切れならログイン画面をそのまま表示する） | 設計判断 |
...
| ログイン済みだが `adminUsers` の状態が不正（403 `ADMIN_SUSPENDED`: 停止中・削除済み、403 `ADMIN_NOT_REGISTERED`: クレームに `organizationId`／`role` が無い、`adminUsers` 文書が無い、または文書とクレームが不一致。04 §2.5.1） | E-01 を表示（T-15）。サイドメニュー・ヘッダーは出さず「ログアウト」だけを置く。ブラウザからの API 呼び出しで同じ 403 を受けたときは `window.location.reload()` で Server Component に描画し直させる（§10.4）。専用ルート（`/admin/unavailable` など）は置かない（00 §3.3 に無いため。設計判断 D06-27）。利用停止（Firebase Auth の `disabled` + `revokeRefreshTokens`。00 §5）後は Cookie 検証自体が 401 になり得るため、その場合は M-01 へ戻る | 00 §5、04 §2.4 |
...
| 他組織のリソース ID | 04 が 404 を返す（取得した文書の `organizationId` とクレームの `organizationId` をサーバで照合し、不一致は 404。00 §4.1 の認可 3 段階）。E-02 | 00 §2.1、§4.1、02 |
...
- このレイアウトは **認証チェックを行わず、`AdminShell` も描画しません**。`app/(admin)/admin.css`（§2.2）の読み込みと `lang="ja"` 配下の共通 `metadata`（`robots: noindex`）だけを担います。理由: Next.js の入れ子レイアウトは配下の全ルートに適用されるため、ここでセッション Cookie を確認すると、認証不要パス（M-01〜M-03）と 07 の印刷用ページ（Chromium が管理者 Cookie を持たずに開く）までログイン画面へ送ってしまい、PDF 生成が失敗します。00 §3.3 は `app/(admin)/admin/` 直下に各 `page.tsx` を置く構成のため、ルートグループで認証済みページだけを分ける構成（ファイルパスが 00 §3.3 と変わる）も採りません。
...
- 07 の `print/layout.tsx` はこのレイアウトの内側に置かれます。本書のレイアウトは CSS の読み込みだけなので、印刷用ページが本書の `components/admin/result/*`・`components/charts/*` を再利用する際に必要なクラス定義（`.compat-slider` など）がそのまま使えます。07 §9.4 の「最小の `html` / `body`」は、00 §3.3 の `app/layout.tsx`（ルートレイアウト）の内側では `html` / `body` を再定義できないため、「サイドメニュー・ヘッダーを描画しない最小のラッパー」と読み替えます（07 §9.4 で 1.2 版に反映済み。§9）。
...
- Client Component は `"use client"` を先頭に置き、`firebase-admin`・`lib/firebase/admin.ts`・`lib/db/`・`lib/services/`・`@anthropic-ai/*` を import しません（00 §3.3、01 の Lint 設定）。`firebase/auth`（クライアント SDK）を import してよいのは `lib/firebase/client.ts` と `lib/utils/admin-auth.ts`（§10.4）だけです。API 呼び出しは `lib/utils/admin-api.ts` に集約します（§10.4）。

### 2.5 画面の共通ルール

| 項目 | ルール | 根拠 |
|---|---|---|
| 初期表示 | Server Component が `lib/services/` を直接呼んで初期データを取得し、Client Component に渡す（00 §3.3）。監査ログ（`result.list`、`result.view`、`classification.view`、`usage_log.view`）はサービス側で `auditLogs` に書く（02、04） | 要件定義書 §9 性能 |
...
| 比較値 | 比較計算の結果は API 応答と URL クエリ（`scope`、`teamCode`）だけで扱い、Cookie・`localStorage`・Firestore に保存しない | 要件定義書 §11 の 6 番、00 §1.11 |
...
| ログイン | `establishSession(email, password)`（§10.4）を呼ぶ。中身は (1) Firebase Auth クライアント SDK の `signInWithEmailAndPassword(auth, email, password)`（`auth` は `lib/firebase/client.ts` が初期化したもの。00 §3.3）、(2) `user.getIdToken()` で ID トークンを取得し **`POST /auth/session`**（`{ idToken }`。04）へ送ってセッション Cookie を発行させる、(3) 応答にかかわらず `signOut(auth)` でクライアント SDK の認証状態を破棄する（認証状態をサーバ側の Cookie に一本化する。00 §4.2）。成功したら **`POST /api/v1/admin/me/login-events`**（`recordLoginEvent()`。§10.4）を 1 回呼ぶ → `next` クエリ（`/admin` 配下のみ）または `/admin` へ遷移。`login-events` が失敗（ネットワークエラー・403 など）しても遷移は続行する（監査記録の欠落として扱う。04 §5.1）。403 を受けた場合は遷移先の Server Component が E-01 を描画する。ID トークンは Cookie にもストレージにも保存しない（00 §4.1） | 00 §4.1・§4.2、D-31、04 §5.1 D04-24 |
| 失敗時 | `signInWithEmailAndPassword` の失敗（`auth/invalid-credential`、`auth/user-not-found`、`auth/wrong-password`、`auth/user-disabled`、`auth/too-many-requests` など。コード名は実装時確認）はすべて「メールアドレスまたはパスワードが正しくありません」（T-24）を表示し、理由を区別しない（停止中も同じ文言）。`POST /auth/session` が 401 を返した場合（ID トークンの検証失敗）も T-24。`POST /auth/session` のネットワーク例外は T-33 | 設計判断（要件定義書 §12: 文言は未確認） |
| `?reset=1` 付きで開かれたとき | M-03 でパスワードを変更した直後に自動ログインへ進めなかった状態（§3.3）。フォームの上に T-36「パスワードを変更しました。ログインしてください」を表示し、クエリはフォーム送信時に取り除く | §3.3 |
| パスワードを忘れた方 | `/admin/password-reset` へのリンク | 要件定義書 §7 S-01 |
| アカウント作成 | 既存にはリンクがあるが、新システムのサインアップは招待トークンが必須（00 §4.2。公開サインアップは無効化する）。設計判断 D06-05: リンク先を `/admin/signup`（トークンなし）とし、そこで「管理者追加用リンクから登録してください」の案内を出す | 要件定義書 §7 S-01、00 §4.2 |
| レート制限 | Firebase Auth の既定（同一アカウントへの連続失敗で `auth/too-many-requests`。実装時確認）に依存。画面側での追加制御なし | 01 |

- 1.x 版の `?error=auth_callback`（`/auth/callback` のコード交換失敗）は、K-06 で `/auth/callback` が廃止された（00 §3.3）ため削除しました。メールのリンクの無効・期限切れは M-03 が `verifyPasswordResetCode` で判定して T-23 を表示します（§3.3）。

### 3.2 M-02 管理者サインアップ

| 項目 | 内容 | 根拠 |
|---|---|---|
| 事前検証 | Server Component が `q` を 04 のサービス `validateInviteToken(token)`（`lib/services/invite-acceptance.ts`。04 §8.1・§8.3 で確定。トークンの SHA-256 と `organizations.inviteTokenHash` の一致、`deletedAt == null` を Admin SDK で確認し、組織名を返す。無効なら `null`）に渡し、組織名を得る。無効・欠落なら案内文 T-22「このリンクは無効です。管理者追加用リンクを組織の管理者から受け取ってください」だけを表示し、フォームを出さない | 00 §2.2 `organizations.inviteTokenHash`、04 §8.5 |
| 表示 | 「{組織名} の管理者として登録します」 | 設計判断 |
| 入力 | お名前（必須、100 文字以内）、メールアドレス（必須）。**パスワードはこの画面では入力しない**（パスワードはメールのリンクから M-03 で設定する。設計判断 D06-30） | 00 §2.2 `adminUsers.displayName`、04 `acceptInviteInputSchema`、要件定義書 §9 認証 |
| 送信 | **`POST /auth/invite`**（04。00 §4.2）を `acceptInvite({ inviteToken, name, email })`（§10.4）で呼ぶ。サーバが招待トークンを検証し、02 §9.9 の `createAdminAccount()` で Admin SDK `createUser`（乱数パスワード、`emailVerified: true`）→ カスタムクレーム（`organizationId`、`role: "admin"`）→ `adminUsers` 文書（ID = uid）→ 監査ログ `admin.signup` を作る（04 §6.3）。ブラウザから Firebase Auth の `createUserWithEmailAndPassword` は **呼ばない**（公開サインアップは無効化。00 §4.2）。200 を受けたら (1) `sendPasswordResetEmail(auth, email)`（§3.3 と同じ関数。Firebase がパスワード設定用のメールを送る）を呼び、(2) フォームを閉じて T-35「登録しました。パスワード設定用のメールを送信しました。メール内のリンクからパスワードを設定してください」を表示する（自動ログインはしない。パスワードが無いため）。(1) が失敗しても T-35 を表示し、利用者は M-01「パスワードを忘れた方」から同じメールを再送できる。確認メールは送られない（`emailVerified: true` で作成。04 D04-37 改）ため「確認メールを送信しました」の表示は置かない。途中失敗で Auth ユーザーだけが残った場合も、同じメールアドレスで再送信すれば 200 で完了する（02 D02-40） | 00 §4.2、04 §6.3、D06-30 |
| 失敗 | 404 `INVITE_TOKEN_INVALID`: フォームを閉じて T-22 を表示（事前検証後に再発行された場合）。409 `EMAIL_ALREADY_REGISTERED`: 04 の `error.message` をメールアドレス欄の下に表示し、M-01 へのリンクを添える。422 `VALIDATION_ERROR`: `details.issues` の `path` に対応する欄の下に `message` を表示。429 `RATE_LIMITED`／その他: `Toast` に `error.message` | 04 §2.4 |
| 登録後の役割 | 常に `admin`。画面で役割を選ばせない（役割はサーバがクレームに設定する） | 要件定義書 §4、00 §5 |
...
- 設計判断 D06-30: パスワードの設定を Firebase Auth の再設定メール（`sendPasswordResetEmail` → `confirmPasswordReset`）に統一する理由: (1) サーバの `createUser` にパスワードを渡さずに済み、招待経由の初期パスワードが API の本文とログに載る経路を無くせる、(2) 初期オーナー（`scripts/create-owner`。00 §3.3）も同じ経路でパスワードを設定でき、画面と運用の手順が 1 つになる、(3) メールアドレスの実在確認を兼ねる（リンクを開けた人だけがパスワードを持てる）。メールの送信は画面（クライアント SDK `sendPasswordResetEmail`）が行うことで確定した（02 D02-39、04 D04-58。サーバ API は設けない）。
...
| モード | 判定 | 内容 |
|---|---|---|
| 送信依頼 | URL に `oobCode` クエリが無い | メールアドレスを入力し、Firebase Auth クライアント SDK の `sendPasswordResetEmail(auth, email)` を呼ぶ（`auth` は `lib/firebase/client.ts`）。メール内のリンク先（アクション URL）は Firebase コンソールのメールテンプレート設定で `{公開 URL}/admin/password-reset` に向ける（01 の設定。ブラウザ側で `redirectTo` 相当の URL は組み立てないため、`NEXT_PUBLIC_APP_BASE_URL` も `window.location.origin` も参照しない。01 D01-35 の趣旨は維持）。結果にかかわらず「入力したメールアドレス宛に案内を送信しました」を表示（存在の有無を区別しない。`auth/user-not-found` も同じ表示） |
| 新パスワード入力 | URL に `mode=resetPassword` と `oobCode` がある | 初期表示時に `verifyPasswordResetCode(auth, oobCode)` でコードを検証し、対象のメールアドレスを得る（無効・期限切れ（`auth/invalid-action-code`、`auth/expired-action-code`。コード名は実装時確認）なら T-23「認証リンクが無効か期限切れです。もう一度お試しください」を表示し、送信依頼モードのフォームを出す）。新しいパスワードと確認を入力し、`confirmPasswordReset(auth, oobCode, newPassword)`。成功後、得たメールアドレスと新パスワードで `establishSession(email, newPassword)`（§3.1、§10.4）を呼んでセッション Cookie を発行し、`POST /api/v1/admin/me/login-events` → 「パスワードを変更しました」を表示し `/admin` へ。`establishSession` が失敗した場合は `/admin/login?reset=1` へ遷移し、M-01 で T-36 を表示する |

- メール本文は Firebase Auth のメールテンプレート（Firebase コンソールで日本語化・送信元を設定する。01）。未確認（要件定義書 §12: パスワード再設定メールの文言）。
- 実装時確認: (1) Firebase コンソールのアクション URL のカスタマイズで、メールのリンクを `/admin/password-reset?mode=resetPassword&oobCode=…` の形で本システムに向けられること（向けられない場合は Firebase の既定のアクションページでパスワードを設定させ、M-03 は送信依頼モードだけを持つ。その場合の変更後の遷移は既定ページから M-01 へ手動で戻る）。(2) Preview 環境の URL はデプロイごとに変わるため、Preview ではアクション URL を固定できない。Preview でのパスワード設定は本番・開発プロジェクトのアクション URL 先で行うか、Auth Emulator（`NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_HOST`。00 §3.2）で行う（01 の環境分離の判断に従う）。(3) `confirmPasswordReset` 後にそのメールアドレスでの `signInWithEmailAndPassword` が即時に成功すること。
- 運用者が作成した初期オーナー（`scripts/create-owner`。00 §3.3）と、M-02 で登録された管理者（D06-30）も同じ画面でパスワードを設定します（M-01「パスワードを忘れた方」→ 送信依頼 → メールのリンク → 新パスワード入力）。
- パスワードの長さは M-07 と同じ 8〜72 文字を画面側で検証します（§3.7）。Firebase Auth 側の最小長・強度規則（パスワードポリシー）は 01 が設定し、画面の規則より緩いことを実装時確認とします（SDK が `auth/weak-password` を返した場合は該当欄の下にその旨を表示する）。
...
├───┼──────────┼────┼──────────┼────────┼─────────┼──────────────┼────┤
│ ▤ │山田 太郎   │求職者│[Aチーム ▾]│☐ 除外する│アイリスト│090-…   │2026/09/17 10:30│ 🗑 │
│ … │            │     │           │         │          │        │              │    │
├───┴──────────┴────┴──────────┴────────┴─────────┴──────────────┴────┤
│                        ‹ 1 2 3 … ›   1 ページ 50 件                     │
└────────────────────────────────────────────────────────────────────────┘
...
| 列 | 内容 | データ元（00 §2.2） | 根拠 |
|---|---|---|---|
| 詳細 | アイコンボタン。`/admin/results/{resultId}` へ遷移 | `results` の文書 ID | 要件定義書 §6.2 A-02 |
| お名前 | 氏名。クリックでも詳細へ | `respondents.name` | 同上 |
| 区分 | 「求職者」／「既存スタッフ」。`owner`／`super_admin` のときだけ表示（`admin` には幹部行自体が無いため列も出さない） | `respondents.kind`（`results.respondentKind` に複製） | 要件定義書 §6.2 A-13、§12（表示位置は未確認。設計判断 D06-06: 同一表に区分列で示す） |
| チーム | プルダウン。「未設定」「Aチーム」…「Zチーム」。変更で即時 `PATCH /api/v1/admin/respondents/{respondentId}` `{ "teamCode": "A" }`（未設定は `null`） | `respondents.teamCode`（正。`results.teamCode` に複製。00 D-34） | 要件定義書 §6.2 A-03 |
| 除外 | チェックボックス「除外する」。変更で即時 `PATCH` `{ "isExcluded": true }` | `respondents.isExcluded`（正。`results.isExcluded` に複製） | 要件定義書 §6.2 A-04 |
| 職業 | 職業コード（1〜8。10 K-19）を `lib/masters/occupations.ts` で表示名に変換する。K-19 より前に受検した旧データのコード 9 は「その他」と表示する（新規登録の検証 `occupationCodeSchema` は 1〜8。保存済み文書の検証 `storedOccupationCodeSchema` は旧コード 9 も読む。PR #13・#14） | `respondents.occupationCode` | 要件定義書 §6.1 U-02、00 §1.10、10 K-19 |
| 電話番号 | そのまま表示 | `respondents.phoneNumber` | 要件定義書 §6.2 A-02 |
| 回答日時 | Asia/Tokyo で `YYYY/MM/DD HH:mm` | `results.submittedAt` | 同上 |
| 削除 | ゴミ箱アイコン。P-06 を開く | | 要件定義書 §6.2 A-05 |

- 一覧の元データは 04 が `results` を組織条件でクエリし、氏名・電話番号などは `respondentId` で `respondents` をまとめて取得して突き合わせたものです（00 §2.2）。画面は結合の有無を意識しません。
- 一覧に載るのは送信済み（`results` 文書が存在する）受検者だけです。下書き中（`assessmentSessions.status == "draft"`）の受検者は一覧に出さず、利用履歴（P-01）で「未回答」として確認できます（設計判断 D06-07。既存の一覧が「回答データ」の一覧であることに合わせる。要件定義書 §6.2 A-02）。
...
| 実行 | `DELETE /api/v1/admin/respondents/{respondentId}`（論理削除。`respondents`・`assessmentSessions`・`results` の `deletedAt` を同一バッチで設定。00 §2.2、02） | 要件定義書 §6.2 A-05、D-11 |
...
| データ | `GET /api/v1/admin/usage-logs?page=1&pageSize=50`（`fetchUsageLogs`。§10.4）。`registeredAt` 降順（04 §5.8 の固定順） | 00 §4.2・§2.2 `usageLogs`、04 §5.8 |
...
| 幹部 | `admin` には幹部の履歴を返さない（04 §5.8 が `usageLogs.respondentKind == "applicant"` の等価条件で絞る。02 §3.7・Q6 で確定） | 00 §2.2 `usageLogs`、00 §5、02 §3.7 |
| 削除済み | 論理削除された受検者の履歴も表示する（`usageLogs` は論理削除の対象外。00 §2.2）。物理削除は行わない（10 K-04）ため、名前・電話番号が「（削除済み）」に置き換わる状態は本フェーズでは発生しない（将来 Admin SDK のスクリプトで物理削除を設計する場合に 02・04 が確定） | 00 §2.2、10 K-04 |
...
- 26 チームすべてを常に同じ表記で表示します（既存踏襲。要件定義書 §6.2 A-08）。設計判断 D06-09（1.1 版で確定）: 1.0 版で 04 に依頼した `availableTeamCodes`（未使用チームに「（0 名）」を併記する案）は 04 が採用しませんでした（04 D04-48: 閲覧者の可視範囲で数えたチーム人数は `fetchPopulation()` の母集団と食い違う）。本書は D06-09 の「含まれない場合は全チームを同じ表記で表示」を確定形とし、依頼を取り下げます。母集団 0 件のチームを選んだときの表示は §3.5.3（409 `POPULATION_EMPTY`）です。
...
求職者 ／ アイリスト ／ 回答日時 2026/09/17 10:30 ／ 比較対象: 組織全体（12 名）
```
...
- 母集団に本人を含める（D-05）と幹部を含める（D-06。いずれも 10 K-01 で決定済み）は 02 の `fetchPopulation()`（00 §1.11）に従います。本人が含まれているかは 04 の `includesSubject`（対象受検者が除外中・別チームのとき `false`）で画面に示します。幹部が含まれているかは応答に無いため表示しません。
...
| イラスト | `public/images/types/{aptitudeType}.svg` | §7 | 00 §3.8 |
...
| イラスト | `public/images/positions/{position}.svg` | 00 §3.8 |
...
| 偉人イラスト・偉人名 | `public/images/styles/{socialStyle}.svg`、`STYLE_TEXTS[socialStyle].greatPersonName` | | 付録C §5、00 §3.8 |
...
| 3 | このサロンで活きる強み（付録D §1 の「このクリニックで活きる強み」を置き換え。付録D §5、10 K-19） | `strengths[]` | 箇条書き |
...
- 免責表示は付録D §1 の文言の「院長」を「責任者」に置き換えたもの（T-09。10 K-19）を `lib/presentation/admin-texts.ts` に置き、ボタンの下と本文の末尾に表示します。
- 推定: 同期方式（04 §7.1）では、応答を待っている間に画面を離れて接続が切れても、サーバ側の関数は処理を続けることが多いと考えられますが、実行基盤（Vercel）の挙動に依存し、要件定義書・付録・他分冊に根拠はなく保証されません。画面はこれに依存しない設計とします: 戻ってきたときは初期表示の `aiAnalysis.status` に従い、`generating` なら上表のとおり GET でポーリングします。打ち切られて `generating` のまま残った結果文書は、`results.aiGenerationStartedAt`（フィールド名は 02 が確定）から 10 分経過後の POST／GET で 04 が `failed` に戻します（04 D04-34）。
...
| 条件 | 値 | 根拠・判断 |
|---|---|---|
| 組織 | 同一 `organizationId`（04 のサーバ側クエリの等価条件。00 §2.1） | 00 §2.1、04 §5.7 |
| 削除 | `results.deletedAt == null`（同上） | 00 §2.2 |
...
| 採点バージョン | 条件にしない（分類は `results.aptitudeType` の集計で、比較計算ではない） | 00 §2.4 |
| 分類の人数 | その分類に属する 4 タイプの人数の合計（`results.aptitudeType` の所属分類で数える。`results.socialStyle` では数えない）。04 §5.7 が同じ規則で集計済み（D04-33） | 付録C §8「4 分類 × 16 キャラクター」。D06-19、04 D04-33 |

- `results.socialStyle`（4 値の最大）と `results.aptitudeType` の所属分類は一致しないことがあります（例: タイプはアテンダント＝Expressive でも `socialStyle` が Amiable）。組織内分類は「キャラクター（適性タイプ）」の配置図であり、付録C §8 でも各分類の 4 キャラクターは適性タイプで固定されています。したがって集計はタイプ基準です（D06-19、04 D04-33）。
...
┌─ アカウント情報 ──────────────────────────────────────────────┐
│ お名前          山田 花子                     [変更]           │
│ メールアドレス   （Firebase Auth の email）     [変更]           │
│ 役割            オーナー                                      │
├─ 受検リンク ───────────────────────────────────────────────┤
│ 求職者用回答リンク        https://…/exam?q=…&p=user      [コピーする] │
│ 既存スタッフ用回答リンク   https://…/exam?q=…&p=executives [コピーする] │
│   責任者のみ回答データを閲覧できます                                │
│ 管理者追加用リンク   発行日時: 2026-09-17 12:00（オーナーのみ）           │
│   [管理者追加用リンクを発行する]                                      │
│   （発行直後のみ）https://…/admin/signup?q=…  [コピーする]            │
│   ※ このリンクはこの画面を離れると再表示できません                       │
├─ 管理者一覧（オーナーのみ） ───────────────────────────────────┤
│ お名前 ／ メールアドレス ／ 役割 ／ 状態 ／ 登録日                     │
├─ パスワード ───────────────────────────────────────────────┤
│ [パスワードを変更する]                                          │
├────────────────────────────────────────────────────────────┤
│ [ログアウト]                                                   │
...
| 項目 | 内容 | API | 根拠 |
|---|---|---|---|
| お名前 | `adminUsers.displayName`（00 §2.2。API の JSON キーは `name`）。「変更」でインライン編集 → 保存（`updateMe({ name })`）。成功で応答（`MeDto`）の `name` を表示に反映し `Toast`「変更しました」 | `PATCH /api/v1/admin/me` `{ "name": "…" }` | 要件定義書 §6.2 A-12、04 §5.1 |
| メールアドレス | Firebase Auth の `email`（`GET /api/v1/admin/me` の `email`。`adminUsers` 文書には持たない。00 §2.2）。「変更」で新アドレスと **現在のパスワード** を入力 → `reauthenticate(currentPassword)`（§10.4。クライアント SDK で再認証して ID トークンを得る）→ `updateMe({ email, reauthIdToken })`。Firebase Auth クライアント SDK の `updateEmail`／`verifyBeforeUpdateEmail` をブラウザから直接呼ばない（04 がサーバ側で `reauthIdToken` を検証し Admin SDK `updateUser({ email })` を行い、監査ログ `account.update` を書く。04 §5.1 D04-47 改・D04-57）。変更は即時に反映され確認メールは送られない（`pendingEmail` は無い）。応答は `MeUpdatedDto`（`reloginRequired: true`）で、セッション Cookie が削除されているため、`Toast`「メールアドレスを変更しました。もう一度ログインしてください」（T-37）を出して `/admin/login?reset=1` 相当（T-36 と同じ趣旨の案内）へ遷移する。失敗: 再認証失敗（`auth/invalid-credential` など）は T-38「現在のパスワードが正しくありません」を現在のパスワード欄の下に表示し API を呼ばない、422 `CURRENT_PASSWORD_MISMATCH` も同じ欄に `error.message`、409 `EMAIL_ALREADY_REGISTERED` は `error.message` を新アドレス欄の下に表示、422 `VALIDATION_ERROR` は `details.issues` の `message` を表示 | `PATCH /api/v1/admin/me` `{ "email": "…", "reauthIdToken": "…" }` | 要件定義書 §6.2 A-12、04 §5.1 D04-23・D04-47 改・D04-57 |
| 役割 | `owner` →「オーナー」、`admin` →「管理者」、`super_admin` →「スーパーアドミン」。表示のみ（変更は運用者が `scripts/set-admin-role` で行う。00 §4.2、§5） | `GET /api/v1/admin/me` | 00 §5 |
| 求職者用回答リンク | `{appBaseUrl()}/exam?q={organizationId}&p=user`（基底 URL はサーバの `appBaseUrl()`。01 §4.3） | `GET /api/v1/admin/me` の `links.applicant`（04 §5.1 が生成。00 §4.2「受検リンク 2 種」） | 要件定義書 §6.2 A-12、D-12 |
| 既存スタッフ用回答リンク | `…/exam?q={organizationId}&p=executives`。説明文「責任者のみ回答データを閲覧できます」（T-08。10 K-19） | `links.executive` | 要件定義書 §4、§6.2 A-12 |
| 管理者追加用リンク | `owner`／`super_admin` にのみ節を表示（`admin` には出さない）。常時表示するのは「発行日時: {links.adminInviteIssuedAt}」（未発行なら「未発行」）だけで、リンクの平文は `GET /api/v1/admin/me` では返らない（`links.adminInvite` は常に `null`。04 §5.1 D04-56。Firestore には `organizations.inviteTokenHash` だけを保存する。02 D02-35）。平文は下の「発行する」の応答でだけ得られ、その場で表示・コピーさせる。ページを離れると再表示できない旨の注記（T-39）を添える。要件定義書 §6.2 A-12「アカウント画面に管理者追加用リンク」（常時表示）との差異は依頼主確認事項（00 D-35、09 §6.1） | `GET /api/v1/admin/me` の `links.adminInviteIssuedAt` | 要件定義書 §6.2 A-12、00 §2.2、04 D04-56 |
| コピーする | `navigator.clipboard.writeText`。成功で `Toast`「コピーしました」。失敗時は入力欄を選択状態にして手動コピーを促す | | 要件定義書 §6.2 A-12 |
| 管理者追加用リンクを発行する | `owner`／`super_admin` のみ。P-07「新しいリンクを発行すると、これまでの管理者追加用リンクは使えなくなります。」→ 実行。成功で応答の `adminInvite` を「コピーする」付きで表示し、発行日時を `rotatedAt` に更新して `Toast`「発行しました」。表示した平文は React state にだけ置き、ストレージに保存しない | `POST /api/v1/admin/organization/invite-token`（04 §5.2 D04-25 で確定） | 00 §4.2、04 §5.2、D04-56 |
| 管理者一覧 | `owner`／`super_admin` のみ表示（`admin` には節ごと出さない。API は `admin` に 403 `ROLE_REQUIRED` を返す）。同一組織の `adminUsers`（削除済みを除く）の氏名（`name`。`adminUsers.displayName`）・メールアドレス（`email`。04 §5.11 が Admin SDK `getUsers` で引いて返す。`null` なら「—」）・役割（`role` → §10.1 のラベル）・状態（`isSuspended`: 「有効」／「停止中」）・登録日（`createdAt`）。`createdAt` 昇順（04 の返却順のまま）。役割変更・停止の操作は本フェーズでは置かない（04 §5.12 D04-50。運用者が `scripts/set-admin-role` で行う。00 §4.2）。データは Server Component（`account/page.tsx`）が `listAdminUsers(ctx)`（04 §8.3）で取得し、`AccountPage` に渡す | `GET /api/v1/admin/admin-users`（04 §5.11 で確定。D04-49） | 要件定義書 §4（管理者追加）、依頼範囲「アカウント（管理者一覧、招待、役割）」。設計判断 D06-20 |
| パスワードを変更する | 現在のパスワード（必須。04 D04-23）、新しいパスワード（8〜72 文字）、確認（一致しなければ送信しない）。`reauthenticate(currentPassword)`（§10.4。クライアント SDK `signInWithEmailAndPassword` → `getIdToken()` → `signOut(auth)`。04 D04-47 改）で `reauthIdToken` を得て `updateMe({ password, reauthIdToken })`。Firebase Auth クライアント SDK の `updatePassword` をブラウザから直接呼ばない（04 がサーバ側で `reauthIdToken` の `uid` 一致と `auth_time` 5 分以内を検証してから Admin SDK `updateUser({ password })` と `revokeRefreshTokens` を行い、監査ログ `account.update` を書く）。成功（`reloginRequired: true`。Cookie は削除済み）で `Toast`「パスワードを変更しました。もう一度ログインしてください」（T-37）を出して `/admin/login?reset=1` へ遷移する（他端末のセッションも失効する旨を P-08 で事前に案内）。失敗: 再認証失敗は T-38 を現在のパスワード欄の下に表示し API を呼ばない、422 `CURRENT_PASSWORD_MISMATCH` も同じ欄に `error.message`、422 `VALIDATION_ERROR`（Firebase のパスワードポリシー違反を含む）は該当欄の下に表示、429 `RATE_LIMITED`（再認証の `auth/too-many-requests` を含む）は `Toast` | `PATCH /api/v1/admin/me` `{ "password": "…", "reauthIdToken": "…" }` | 要件定義書 §6.2 A-12、04 §5.1 D04-23・D04-47 改・D04-57 |
| ログアウト | `logout()`（§10.4）= `DELETE /auth/session`（Cookie 削除 + `revokeRefreshTokens`。00 §4.2、04）→ `/admin/login`。ブラウザ側の Firebase Auth にはログイン後の状態を残していない（§3.1 の `signOut(auth)`）ため、クライアント SDK の呼び出しは不要 | `DELETE /auth/session` | 要件定義書 §6.2 A-12 |
...
付録C の文言をコード内定数として持ちます（00 D-01）。文言本文は付録C からそのまま転記し、本書には複製しません。すべて Next.js・Firebase 非依存の純粋な TypeScript です（00 §3.3）。
...
export interface PositionView {
  ...
  readonly imagePath: string;                    // /images/positions/{key}.svg
}
...
- `resolveResultTexts` は `lib/masters/` と 03 の `lib/presentation/trait-highlights.ts` だけに依存します。Firestore・API に触れません（03 §8.1）。
...
既存の画像ファイルは依頼主から提供されず、**実装者が SVG で作成** します（10 K-05、00 D-13、08 D08-20）。次の規約で配置します（00 §3.8 2.1 版と一致）。

| 種別 | パス | 枚数 | 使用画面 | 推奨サイズ（`viewBox`） |
|---|---|---|---|---|
| 適性タイプ・組織内分類のキャラクター（同一ファイル。K-05） | `public/images/types/{AptitudeTypeKey}.svg` | 16 | M-05 サマリー・個人特性（512×512 相当で表示）、M-06・P-05（256×256 相当で表示） | 512×512 |
| 資質 | `public/images/aptitudes/{AptitudeKey}.svg` | 4 | M-05 育成方法（見出し横。任意） | 256×256 |
| ソーシャルスタイル（偉人） | `public/images/styles/{SocialStyleKey}.svg` | 4 | M-05 ソーシャルスタイル | 512×512 |
| 立ち位置 | `public/images/positions/{PositionKey}.svg` | 5 | M-05 セクション 1・4 | 600×400 |

- ファイル名は識別子そのもの（例: `attendant.svg`、`sensory_open.svg`、`driving.svg`、`strong_leader.svg`）です。日本語ファイル名・空白・大文字は使いません。
- 適性タイプとキャラクターは同一ファイル `public/images/types/` を使います（K-05 で確定。`public/images/characters/` は作らない。1.3 版までの別パス案は取り下げ）。表示サイズは使用画面側の `width`／`height` で変えます。依頼主が後日正式な画像を提供する場合は同名の SVG（または `img` の `src` を変えて PNG）で差し替えます。
...
```text
public/images/types/       attendant.svg follower.svg specialist.svg creator.svg professional.svg generalist.svg scientist.svg pioneer.svg conductor.svg controller.svg artist.svg reviewer.svg promoter.svg actor.svg receptionist.svg freelancer.svg
public/images/aptitudes/   sensory_open.svg environment_receptive.svg self_actualizing.svg inquiry_logical.svg
public/images/styles/      driving.svg expressive.svg analytical.svg amiable.svg
public/images/positions/   strong_leader.svg cooperative_leader.svg follower.svg passive_follower.svg unfit.svg
```
...
| 画面 | 初期表示（Server Component → `requireAdmin()` → `lib/services/`。04 §8.3） | ブラウザからの呼び出し（Route Handler・Firebase Auth クライアント SDK） |
|---|---|---|
| M-01 | なし（セッション Cookie が有効なら `/admin` へ。§1.3） | クライアント SDK `signInWithEmailAndPassword` → `getIdToken()` → `POST /auth/session` → `signOut(auth)` → `POST /api/v1/admin/me/login-events`（04 §5.1） |
| M-02 | `validateInviteToken(token)`（04 のサービス。Admin SDK で `organizations.inviteTokenHash` を照合） | `POST /auth/invite`（04）→ クライアント SDK `sendPasswordResetEmail` |
| M-03 | なし（`oobCode` クエリの有無で表示切替） | クライアント SDK `sendPasswordResetEmail`、`verifyPasswordResetCode`、`confirmPasswordReset` → `signInWithEmailAndPassword` → `POST /auth/session` → `signOut(auth)` → `POST /api/v1/admin/me/login-events` |
...
| M-07 | `getMe(ctx)`、`listAdminUsers(ctx)`（owner／super_admin のとき） | `PATCH /api/v1/admin/me`（氏名・メール・パスワード）、`POST /api/v1/admin/organization/invite-token`、`DELETE /auth/session` |
| 共通（M-04〜M-07） | `getMe(ctx)`（役割・組織名。`AdminShell` の表示に使う。`requireAdmin()` の `AdminContext` と同じ取得を 1 回にまとめる） | `DELETE /auth/session`（ログアウト） |
...
| `GET /api/v1/admin/me`（04 §5.1） | `adminUserId`、`name`、`email`（`string \| null`）、`role`、`canViewExecutives`、`organization`（`organizationId`、`name`、`code`、`customerNumber`）、`links`（`applicant`、`executive`、`adminInvite`（常に `null`。D04-56）、`adminInviteIssuedAt`） | 1.4 版: 管理者追加用リンクは常時表示せず、発行日時と「発行する」ボタンだけを置く（§3.7）。`canViewExecutives` は API 応答にも含まれるが、画面は §10.1 のヘルパーで `role` から求めてもよい（同じ規則） |
| `PATCH /api/v1/admin/me`（04 §5.1） | `{ name?, email?, password?, reauthIdToken? }`（`email`／`password` を変えるときは `reauthIdToken` 必須。D04-47 改）。応答は `MeUpdatedDto`（`MeDto` + `reloginRequired`）。`reloginRequired: true` のとき Cookie は削除済み。422 `CURRENT_PASSWORD_MISMATCH`、409 `EMAIL_ALREADY_REGISTERED`、429 `RATE_LIMITED` | 1.4 版: `currentPassword` を API に送らず、ブラウザの再認証で得た `reauthIdToken` を送る。`pendingEmail`・確認メール（T-32）は廃止（D04-57）。変更後は再ログイン（§3.7、§10.4） |
| `POST /api/v1/admin/me/login-events`（04 §5.1） | 本文なし、204。`admin.login` を記録 | ログイン成功後・招待受理後に必ず呼ぶ（§3.1、§3.2） |
| `GET /api/v1/admin/admin-users`（04 §5.11。D04-49 改で確定） | `{ items: [{ adminUserId, name, email, role, isSuspended, createdAt }], total }`。`admin` は 403 `ROLE_REQUIRED`。`email` は Admin SDK `getUsers` から（`null` あり） | `id` → `adminUserId`。1.4 版: メールアドレスを表示する（§3.7）。D06-20 を更新 |
| `POST /api/v1/admin/organization/invite-token`（04 §5.2。D04-25 で採用） | `{ adminInvite, rotatedAt }`。平文を得られる唯一の手段（D04-56）。トークンは base64url 43 文字 | 1.4 版: 応答の `adminInvite` をその場で表示・コピーさせ、ページを離れると再表示できない（§3.7 P-07、T-39） |
| `POST /auth/invite`（04。00 §4.2） | `{ inviteToken, name, email }` → `{ organizationName, email, nextUrl }`。404 `INVITE_TOKEN_INVALID`、409 `EMAIL_ALREADY_REGISTERED` | M-02 の送信先（§3.2）。1.3 版で `password` を本文から外した（D06-30。パスワードは M-03 の再設定メールで設定）。04 §6.3（2.0〜2.1 版）で同じ形に確定（回答済み (1)） |
| `POST /auth/session`（04。00 §4.1） | `{ idToken }` → 200 とセッション Cookie（`Set-Cookie`）。ID トークンの検証失敗は 401 | M-01・M-03 の `establishSession()`（§10.4）。**画面は「ID トークンが有効なら 200」を前提**にし、停止中・未所属の判定は遷移先の `requireAdmin()` → E-01 に委ねる（§1.3）。04 D04-54 で「有効な ID トークンなら常に Cookie を発行し、判定は `requireAdmin()` に委ねる」と確定（回答済み (2)。応答は 200 `{ expiresAt }`。有効期間は 7 日・延長なし。D04-53） |
| `DELETE /auth/session`（04。00 §4.2） | 本文なし、204。Cookie 削除と `revokeRefreshTokens` | M-07 のログアウトと E-01 の「ログアウト」（§3.7、§1.3） |
| `validateInviteToken(token)`（04 の `lib/services/`） | 招待トークンの平文 → `{ organizationId, organizationName }`。無効・期限切れ・組織削除済みは `ApiError(404, INVITE_TOKEN_INVALID)` | M-02 の Server Component が事前検証に使う（§3.2）。04 §8.1 `lib/services/invite-acceptance.ts` の `validateInviteToken()`（無効なら `null`。例外を投げない）で確定（回答済み (3)） |
...
1.3 版（K-06）で 04 に確認・依頼した事項と回答（1.4 版ですべて反映済み）:

1. `POST /auth/invite` の入力からパスワードを外し `{ inviteToken, name, email }` とする → **確定**（04 §6.3。`createUser` は乱数パスワード、`emailVerified: true`。02 D02-39）。再設定メールの送信は **画面（クライアント SDK `sendPasswordResetEmail`）** が行う（04 D04-58）。§3.2 の (1) はそのまま。
2. `POST /auth/session` は有効な ID トークンに対して常に Cookie を発行し、判定は `requireAdmin()` に委ねる → **確定**（04 D04-54）。§3.1 の失敗時の表示は変更なし。
3. 招待トークンの事前検証関数 → `lib/services/invite-acceptance.ts` の `validateInviteToken()`（04 §8.1・§8.3）で **確定**。
4. `PATCH /api/v1/admin/me` のメールアドレス変更 → **即時反映・`pendingEmail` なし**（04 D04-57）。現在のパスワードの検証は **ブラウザの再認証で得た `reauthIdToken`** をサーバで検証する方式（04 D04-47 改）。変更後は全セッションを失効させ `reloginRequired: true`（§3.7、§10.4 に反映。T-32 は廃止、T-37〜T-39 を追加）。
5. `admin.login` の監査記録 → **`POST /api/v1/admin/me/login-events` を維持**（04 D04-24 改。`POST /auth/session` では書かない）。§3.1・§3.3・§8.3 の呼び出しはそのまま。
6. 利用履歴の幹部除外 → **`usageLogs.respondentKind` の複製**（02 §3.7、04 §5.8 D04-62）で確定（§3.4.7）。

1.4 版で新たに反映した 04・02 の確定事項: 管理者追加用リンクは再発行の応答でだけ表示（04 D04-56、02 D02-35。§3.7）、管理者一覧に `email`（04 D04-49 改。§3.7）、セッション Cookie 7 日・延長なし（04 D04-53）、イラストは SVG・同一ファイル（10 K-05。§7）。

### 8.3 監査ログ

画面は監査ログを直接書きません。`auditLogs`（00 §2.2）への `result.view`、`result.comparison`、`result.pdf_export`、`result.ai_generate`、`classification.view`、`usage_log.view`、`respondent.update_team`、`respondent.update_exclusion`、`respondent.delete`、`account.update`、`organization.rotate_invite_token`、`admin.signup`（`POST /auth/invite` が Admin SDK で書く。04 D04-36 改）の記録はサービス（04）またはリポジトリ（02）が書きます（`action` の一覧は 04 §2.6・02 §11）。

例外は `admin.login` です。`POST /auth/session` はクレーム・`adminUsers` を検証せず `organizationId` を確定できない（04 D04-54）ため、04 D04-24 改のとおり **画面がセッション Cookie の発行直後に `POST /api/v1/admin/me/login-events` を 1 回呼ぶ** 形で確定しました（§3.1、§3.3。§8.2 の回答 (5)）。呼び忘れると要件定義書 §9 の「アクセスログ」のうちログイン記録が残らないため、E2E（08）で「ログイン後に `admin.login` が 1 件記録される」ことを確認対象にします（§11）。
...
```ts
// lib/auth/admin-role.ts（表示用。判定関数の実体は 02 §9.2 の lib/auth/claims.ts）
import type { AdminRole } from "@/lib/db/types";   // 02 §5.3
export { canViewExecutives } from "@/lib/auth/claims"; // 02 §9.2 の実装を再エクスポート（owner / super_admin → true）。本書で再定義しない
...
import type {
  MeDto, MeUpdatedDto, UpdateMeInput, ResultListItemDto, ComparisonDto, RespondentUpdatedDto, AiAnalysisDto,
...
export function acceptInvite(input: { inviteToken: string; name: string; email: string }): Promise<{ organizationName: string; email: string; nextUrl: string }>;   // POST /auth/invite（04）。1.3 版で password を外した（D06-30）
export function recordLoginEvent(): Promise<void>;                                   // POST /api/v1/admin/me/login-events（04 §5.1）。失敗は呼び出し側が握りつぶす（§3.1）
export function logout(): Promise<void>;                                             // DELETE /auth/session（04）。応答にかかわらず /admin/login へ遷移する（§3.7）

// lib/utils/admin-auth.ts（ブラウザ専用。firebase/auth を import してよいのはこのファイルと lib/firebase/client.ts だけ。00 §3.3）
/**
 * signInWithEmailAndPassword → getIdToken → POST /auth/session → signOut(auth) の順に行い、
 * セッション Cookie を発行させる。失敗は AdminAuthError（reason: "invalid_credential" | "session_rejected" | "network"）。
 * ID トークンはこの関数の外に出さない（00 §4.1）。M-01（§3.1）と M-03（§3.3）が使う
 */
export function establishSession(email: string, password: string): Promise<void>;
/**
 * 現在のパスワードによる再認証（04 D04-47 改）: signInWithEmailAndPassword(auth, email, currentPassword) → getIdToken() → signOut(auth)。
 * 戻り値の ID トークンは PATCH /api/v1/admin/me の reauthIdToken にだけ使い、保存しない。失敗は AdminAuthError（reason: "invalid_credential" | "too_many_requests" | "network"）。M-07（§3.7）が使う
 */
export function reauthenticate(email: string, currentPassword: string): Promise<string>;
export function requestPasswordReset(email: string): Promise<void>;                  // sendPasswordResetEmail(auth, email)。auth/user-not-found は成功扱い（§3.3）
export function verifyResetCode(oobCode: string): Promise<string>;                   // verifyPasswordResetCode。戻り値はメールアドレス
export function completePasswordReset(oobCode: string, newPassword: string): Promise<void>;   // confirmPasswordReset
...
// アカウント
export function updateMe(patch: { name?: string; email?: string; password?: string; reauthIdToken?: string }): Promise<MeUpdatedDto>;   // PATCH /api/v1/admin/me（04 §5.1）。reloginRequired が true なら Cookie は削除済みで、呼び出し側が /admin/login?reset=1 へ遷移する（§3.7）
...
- `lib/utils/admin-auth.ts` の関数は `AdminApiError` ではなく `AdminAuthError` を投げます。Firebase Auth のエラーコード（`auth/…`）は `reason` に丸め、画面には出しません（T-24・T-23・T-33 の文言だけを使う）。Auth Emulator を使うローカル・CI では `lib/firebase/client.ts` が `NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_HOST`（00 §3.2）で `connectAuthEmulator` を呼びます（この関数群は変更不要）。
...
| T-08 | 責任者のみ回答データを閲覧できます | 要件定義書 §4（「院長先生」を「責任者」に置き換え。10 K-19） |
| T-09 | ※ 診断スコア（16特性・ソーシャルスタイル・リスク指標）をもとにAIが自動生成しています。生成結果は責任者が内容をご確認のうえご活用ください。 | 付録D §1（「院長」を「責任者」に置き換え。10 K-19） |
...
| T-23 | 認証リンクが無効か期限切れです。もう一度お試しください | 設計判断（§3.3。`verifyPasswordResetCode` の失敗） |
| T-24 | メールアドレスまたはパスワードが正しくありません | 設計判断（§3.1） |
| T-25 | 登録が完了しました。ログインしてください | 1.3 版で未使用（M-02 は T-35 に変更。定数は互換のため残してよい） |
...
| T-32 | （1.4 版で廃止。確認メール方式は採らない。04 D04-57） | — |
| T-37 | {メールアドレス／パスワード}を変更しました。もう一度ログインしてください | 設計判断（§3.7。04 D04-57 の `reloginRequired`） |
| T-38 | 現在のパスワードが正しくありません | 04 §2.4 `CURRENT_PASSWORD_MISMATCH` と同じ文言（§3.7 の再認証失敗） |
| T-39 | このリンクはこの画面を離れると再表示できません。必要な相手にすぐに共有してください | 設計判断（§3.7。04 D04-56） |
| T-33 | 通信に失敗しました。ネットワーク接続を確認して再度お試しください | 設計判断（§10.4 ネットワーク例外） |
| T-34 | 除外中 | 設計判断（P-05 のバッジ。§3.6.2） |
| T-35 | 登録しました。パスワード設定用のメールを送信しました。メール内のリンクからパスワードを設定してください | 設計判断（§3.2、D06-30。1.3 版で追加） |
| T-36 | パスワードを変更しました。ログインしてください | 設計判断（§3.1 `?reset=1`、§3.3。1.3 版で追加） |
...
| E2E（Playwright、1440×900） | ログイン（`POST /api/v1/admin/me/login-events` が 1 回呼ばれる）→ 一覧 → 詳細 → 比較選択 → AI 解説生成（モック provider。同期応答）→ PDF ダウンロード → 組織内分類 → アカウントの受検リンクコピー → 管理者追加用リンクの発行と応答リンクの表示（owner） → 管理者追加（招待リンク → `POST /auth/invite` → T-35 の案内。自動ログインはしない。08 E-18 で Emulator の `oobCode` からパスワードを設定してログイン）→ メールアドレス変更（再認証 → `reloginRequired` でログイン画面へ）。信頼係数 90% のゲージ色が緑系、リスク 90% が赤であることを DOM の `stroke` 属性で確認（要件定義書 §11 の 5 番の受入。08 E-14） |
| 不具合対応の受入（00 §1.11） | 3・10 番: レーダーの比較対象系列の値と合致度に使う平均が同一 API 応答から来ていること（比較選択 1 回あたり `GET …/comparison` のネットワークリクエストが 1 本。08 E-13 と同じ観点）。6 番: 比較選択後に `adminUsers` 文書が変化しない、2 つのタブで別のチームを選んでも互いに影響しない。5 番: 上記の色確認 |
...
| D-13 | 決定済み（10 K-05。1.4 版で §7 を更新） | 適性タイプとキャラクターの画像が同一か | 依頼主は提供せず実装者が SVG で作成。同一ファイル `public/images/types/{AptitudeTypeKey}.svg`（§7） | 06、07 |
...
| D06-19 | 設計判断 | 組織内分類の分類人数の数え方 | `results.aptitudeType` の所属分類で数える（`socialStyle` では数えない） | 04、06 |
| D06-20 | 設計判断（04 で確定済み。1.4 版で更新） | アカウント画面の管理者一覧・招待リンク再発行（要件になし。依頼範囲の指示） | オーナーのみ閲覧・再発行。API は 04 §5.11 `GET /api/v1/admin/admin-users`（D04-49 改。`email` を含む）と 04 §5.2 `POST …/invite-token`（D04-25）で確定。管理者一覧にメールアドレスを表示する。招待リンクの平文は再発行の応答でだけ表示する（D04-56。依頼主確認事項 00 D-35）。役割変更・停止は本フェーズでは運用者（`scripts/set-admin-role`。00 §4.2、04 D04-50） | 02、04、06 |
...
| D06-24 | 設計判断 | 比較組織の選択状態の置き場所 | URL クエリ（`scope`、`teamCode`）。Cookie・ストレージ・Firestore に置かない。応答は `ComparisonProvider` の React state だけに置く | 04、06、07 |
...
| D06-30 | 設計判断（1.3 版。10 K-06） | 管理者の認証画面での Firebase Auth の使い方 | ログインは `signInWithEmailAndPassword` → `getIdToken` → `POST /auth/session` → `signOut(auth)`（00 §4.1・§4.2）。招待（M-02）ではパスワードを入力させず、`POST /auth/invite` 後に再設定メール（`sendPasswordResetEmail`）でパスワードを設定させる。パスワード再設定（M-03）は `verifyPasswordResetCode` → `confirmPasswordReset` → 自動ログイン。`/auth/callback` と `redirectTo` は廃止し、メールのアクション URL は Firebase コンソールで設定する（01）。ログアウトは `DELETE /auth/session`。理由は §3.2 の補足。メール送信の主体は画面（クライアント SDK）で確定（02 D02-39、04 D04-58）。アクション URL のカスタマイズ可否は実装時確認（§3.3、09 §6.4 の 8） | 01、02、04、06、08 |
| D06-32 | 設計判断（1.4 版。04 D04-47 改・D04-57 に従う） | メール・パスワード変更時の再認証と変更後の再ログイン | 現在のパスワードはクライアント SDK の再認証（`reauthenticate()`。§10.4）で検証し、得た ID トークンを `reauthIdToken` として `PATCH /api/v1/admin/me` に送る。変更後は Cookie が削除されるため T-37 を表示してログイン画面へ遷移する（他端末も失効。P-08 で事前に案内）。確認メール方式（`pendingEmail`、T-32）は廃止 | 04、06、08 |
| D06-31 | 設計判断（1.3 版。10 K-06） | 画面が参照する保存先の表記 | 1.x 版のテーブル・列名（`admin_users.name`、`results.aptitude_type` など）を 00 §2.2 のコレクション・フィールド名（`adminUsers.displayName`、`results.aptitudeType` など）に置き換えた。API の JSON キー（`name`、`resultId` など）は 04 の Dto のままで変更しない。RLS による絞り込みはサーバ側クエリの `organizationId` 等価条件とクレーム照合（00 §4.1）に読み替えた | 04、06 |
...
|---|---|---|
| 1.0 | 2026-09-17 | 初版 |
| 1.3 | 2026-09-21 | K-06（Supabase → Firebase。10）に伴う部分改版。§0・§0.2、§1.1（M-03 の URL、E-01 の条件）、§1.2 の遷移図、§1.3（Cookie 検証、クレーム照合）、§3.1（`establishSession`: `signInWithEmailAndPassword` → `POST /auth/session` → `signOut`。`?error=auth_callback` を廃止し `?reset=1` を追加）、§3.2（招待の入力からパスワードを外し、再設定メールで設定。D06-30）、§3.3（`sendPasswordResetEmail`・`verifyPasswordResetCode`・`confirmPasswordReset`。`/auth/callback`・`redirectTo` を廃止）、§3.4.2・§3.4.7・§3.5.9・§3.6.3・§3.7 のフィールド名（D06-31）、§3.7 のログアウト（`DELETE /auth/session`）とメール・パスワード変更の注記、§8.1〜§8.3 の API 対応と 04 への確認事項 6 件、§10.4 に `lib/utils/admin-auth.ts` と `logout()` を追加、§10.5 に T-35・T-36 を追加（T-25 は未使用）、§11 の受入観点、D06-19・D06-20・D06-24 の表記。最終確認で §2.4 の Client Component の import 禁止対象（`firebase-admin`・`lib/firebase/admin.ts` を追加、`firebase/auth` の許可先を明記）と §3.4.7 の削除済み受検者の履歴行（物理削除は行わない。10 K-04）を改めた。レイアウト・グラフ設定・文言の出し分け・色規則は変更なし |
...
| 1.4 | 2026-09-21 | Firebase 化後の分冊間整合。04 2.0〜2.1 版・02 2.1 版の確定に合わせ、§1.3（`verifyAdminSessionCookie`、クレーム不一致）、§3.2（`createAdminAccount`、`emailVerified: true`、再送で回復、メール送信は画面で確定）、§3.4.7（`usageLogs.respondentKind`）、§3.7（管理者追加用リンクは発行の応答でだけ表示・発行日時の常時表示、管理者一覧にメールアドレス、メール・パスワード変更は `reauthenticate()` + `reauthIdToken` と変更後の再ログイン。`pendingEmail`・確認待ち・T-32 を廃止し T-37〜T-39 を追加）、§7（イラストは実装者作成の SVG、適性タイプとキャラクターは同一ファイル。10 K-05）、§8.2（04 の確定形と 1.3 版の依頼 6 件への回答）、§8.3（`login-events` 確定）、§10.1（`canViewExecutives` は 02 の実装を再エクスポート、`AdminRole` は `lib/db/types`）、§10.4（`reauthenticate`、`updateMe` の型、`MeUpdatedDto`）、§11（E2E: 招待後は自動ログインしない、リンク発行、再認証）、D-13・D06-20・D06-30 を更新し D06-32 を追加。レイアウト・グラフ設定・文言の出し分け・色規則は変更なし |
| 1.5 | 2026-09-24 | 本番デプロイで判明した事項の反映（10 K-19 アイリストのサロン向け）。§3.4.1・§3.5.2・§3.5.4・§3.7 の画面例の文言（職業名、「院長（先生）」→「責任者」）、§3.4.2 の職業（コード 1〜8。旧コード 9 は「その他」と表示。PR #13・#14）、§3.5.9 の AI 解説の見出し（「このサロンで活きる強み」）と免責表示、§3.7 の既存スタッフ用回答リンクの説明文、§10.5 の T-08・T-09 を改めた |
