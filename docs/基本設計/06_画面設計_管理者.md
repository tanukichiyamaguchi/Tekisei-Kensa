...
| 版 | 1.3（1.1 版: レビュー指摘 20 件への対応。1.2 版: 最終点検で 07 D07-15・01 D01-35・00 1.1 版との整合を反映。1.3 版: K-06（Supabase → Firebase）に伴う部分改版。変更点は §13） |
| 作成日 | 2026-09-17（1.1 版: 2026-09-21、1.2 版: 2026-09-21、1.3 版: 2026-09-21） |
...
| ログイン済みで M-01 にアクセス | `/admin` へリダイレクト（`login/page.tsx` の Server Component が 04 のセッション Cookie 検証（`verifySessionCookie`。`lib/auth/session-cookie.ts`）で有効な Cookie を確認したときだけ。無効・期限切れならログイン画面をそのまま表示する） | 設計判断 |
...
| 項目 | 内容 | 根拠 |
|---|---|---|
| 事前検証 | Server Component が `q` を 04 のサービス `validateInviteToken(token)`（`lib/services/`。トークンの SHA-256 と `organizations.inviteTokenHash` の一致、`deletedAt == null` を Admin SDK で確認し、組織名を返す。関数名は 04 が確定）に渡し、組織名を得る。無効・欠落なら案内文 T-22「このリンクは無効です。管理者追加用リンクを組織の管理者から受け取ってください」だけを表示し、フォームを出さない | 00 §2.2 `organizations.inviteTokenHash`、04 §8.5 |
...
| 送信 | **`POST /auth/invite`**（04。00 §4.2）を `acceptInvite({ inviteToken, name, email })`（§10.4）で呼ぶ。サーバが招待トークンを検証し、Admin SDK `createUser` でユーザーを作成し、カスタムクレーム（`organizationId`、`role: "admin"`）と `adminUsers` 文書（ID = uid）を作る（00 §4.2）。ブラウザから Firebase Auth の `createUserWithEmailAndPassword` は **呼ばない**（公開サインアップは無効化。00 §4.2）。200 を受けたら (1) `sendPasswordResetEmail(auth, email)`（§3.3 と同じ関数。Firebase がパスワード設定用のメールを送る）を呼び、(2) フォームを閉じて T-35「登録しました。パスワード設定用のメールを送信しました。メール内のリンクからパスワードを設定してください」を表示する（自動ログインはしない。パスワードが無いため）。(1) が失敗しても T-35 を表示し、利用者は M-01「パスワードを忘れた方」から同じメールを再送できる。確認メールは送られない（`emailVerified` の扱いは 04）ため「確認メールを送信しました」の表示は置かない | 00 §4.2、04、D06-30 |
...
- 設計判断 D06-30: パスワードの設定を Firebase Auth の再設定メール（`sendPasswordResetEmail` → `confirmPasswordReset`）に統一する理由: (1) サーバの `createUser` にパスワードを渡さずに済み、招待経由の初期パスワードが API の本文とログに載る経路を無くせる、(2) 初期オーナー（`scripts/create-owner`。00 §3.3）も同じ経路でパスワードを設定でき、画面と運用の手順が 1 つになる、(3) メールアドレスの実在確認を兼ねる（リンクを開けた人だけがパスワードを持てる）。メールの送信を画面（クライアント SDK）から行うかサーバ（Admin SDK `generatePasswordResetLink` + 送信）から行うかは 00 §4.2 のとおり 02・04 が確定する。サーバ送信に確定した場合、画面は (1) の `sendPasswordResetEmail` を呼ばず T-35 だけを表示する（04 への確認事項。§8.2）。
...
| 幹部 | `admin` には幹部の履歴を返さない（04 が絞る。`usageLogs` に `respondentKind` を複製して条件にするか、`respondents` を突き合わせるかは 02・04 が確定） | 00 §2.2 `usageLogs`、00 §5 |
...
| メールアドレス | Firebase Auth の `email`（`GET /api/v1/admin/me` の `email`。`adminUsers` 文書には持たない。00 §2.2）。「変更」で新アドレスを入力 → `updateMe({ email })`。Firebase Auth クライアント SDK の `updateEmail`／`verifyBeforeUpdateEmail` をブラウザから直接呼ばない（04 がサーバ側で Admin SDK `updateUser({ email })` を行い、監査ログ `account.update` を書く。00 §4.2）。Admin SDK の変更は即時に反映されるため、応答に `pendingEmail` が無い（`undefined`）場合は「確認待ち」行を出さず、応答の `email` を表示に反映して `Toast`「変更しました」を出す。04 が確認メール方式（`pendingEmail` あり）を採る場合は従来どおり「確認待ち: {pendingEmail}」と T-32「新しいメールアドレス宛に確認メールを送信しました。メール内のリンクを開くと変更が反映されます」を表示する（方式は 04 が確定。§8.2）。失敗: 409 `EMAIL_ALREADY_REGISTERED` は `error.message` を欄の下に表示、422 `VALIDATION_ERROR` は `details.issues` の `message` を表示 | `PATCH /api/v1/admin/me` `{ "email": "…" }` | 要件定義書 §6.2 A-12、04 §5.1 |
...
| 管理者追加用リンク | `{appBaseUrl()}/admin/signup?q={inviteToken}`（トークンの平文はサーバが再発行時にだけ生成し、Firestore には `organizations.inviteTokenHash` を保存する。平文を `GET /api/v1/admin/me` の応答に含めるか、再発行時の応答にだけ含めるかは 04 が確定） | `links.adminInvite` | 要件定義書 §6.2 A-12、00 §2.2 |
| コピーする | `navigator.clipboard.writeText`。成功で `Toast`「コピーしました」。失敗時は入力欄を選択状態にして手動コピーを促す | | 要件定義書 §6.2 A-12 |
| 招待リンクを再発行する | `owner`／`super_admin` のみ。P-07「再発行すると、これまでの管理者追加用リンクは使えなくなります。」→ 実行。成功で応答の `adminInvite` を表示に反映し `Toast`「再発行しました」 | `POST /api/v1/admin/organization/invite-token`（04 §5.2 D04-25 で確定） | 00 §4.2、04 §5.2 |
| 管理者一覧 | `owner`／`super_admin` のみ表示（`admin` には節ごと出さない。API は `admin` に 403 `ROLE_REQUIRED` を返す）。同一組織の `adminUsers`（削除済みを除く）の氏名（`name`。`adminUsers.displayName`）・役割（`role` → §10.1 のラベル）・状態（`isSuspended`: 「有効」／「停止中」）・登録日（`createdAt`）。`createdAt` 昇順（04 の返却順のまま）。メールアドレスは応答に含まれないため表示しない（04 §5.11。`adminUsers` 文書にも無く、Admin SDK `getUser` で引く必要があるため。00 §2.2）。役割変更・停止の操作は本フェーズでは置かない（04 §5.12 D04-50。運用者が `scripts/set-admin-role` で行う。00 §4.2）。データは Server Component（`account/page.tsx`）が `listAdminUsers(ctx)`（04 §8.3）で取得し、`AccountPage` に渡す | `GET /api/v1/admin/admin-users`（04 §5.11 で確定。D04-49） | 要件定義書 §4（管理者追加）、依頼範囲「アカウント（管理者一覧、招待、役割）」。設計判断 D06-20 |
| パスワードを変更する | 現在のパスワード（必須。04 D04-23）、新しいパスワード（8〜72 文字）、確認（一致しなければ送信しない）。`updateMe({ password, currentPassword })`。Firebase Auth クライアント SDK の `updatePassword` をブラウザから直接呼ばない（04 がサーバ側で現在のパスワードを検証してから Admin SDK `updateUser({ password })` を行い、監査ログ `account.update` を書く。D04-47。現在のパスワードの検証方法（Admin SDK にはパスワード照合が無いため、Firebase Auth の REST API でのサインイン試行など）は 04 が確定し、実装時確認とする）。成功で `Toast`「パスワードを変更しました」と入力欄のクリア。失敗: 422 `CURRENT_PASSWORD_MISMATCH` は `error.message` を現在のパスワード欄の下に表示、422 `VALIDATION_ERROR` は該当欄の下に表示、429 `RATE_LIMITED` は `Toast` | `PATCH /api/v1/admin/me` `{ "password": "…", "currentPassword": "…" }` | 要件定義書 §6.2 A-12、04 §5.1 D04-23・D04-47 |
...
| `POST /auth/invite`（04。00 §4.2） | `{ inviteToken, name, email }` → `{ organizationName, email, nextUrl }`。404 `INVITE_TOKEN_INVALID`、409 `EMAIL_ALREADY_REGISTERED` | M-02 の送信先（§3.2）。1.3 版で `password` を本文から外した（D06-30。パスワードは M-03 の再設定メールで設定）。04 の K-06 改版と整合をとる（下記の依頼 (1)） |
| `POST /auth/session`（04。00 §4.1） | `{ idToken }` → 200 とセッション Cookie（`Set-Cookie`）。ID トークンの検証失敗は 401 | M-01・M-03 の `establishSession()`（§10.4）。**画面は「ID トークンが有効なら 200」を前提**にし、停止中・未所属の判定は遷移先の `requireAdmin()` → E-01 に委ねる（§1.3）。04 が `POST /auth/session` で 403 `ADMIN_SUSPENDED`／`ADMIN_NOT_REGISTERED` を返す設計にした場合、画面はその `error.message` をフォームの上に表示する（下記の依頼 (2)） |
| `DELETE /auth/session`（04。00 §4.2） | 本文なし、204。Cookie 削除と `revokeRefreshTokens` | M-07 のログアウトと E-01 の「ログアウト」（§3.7、§1.3） |
| `validateInviteToken(token)`（04 の `lib/services/`） | 招待トークンの平文 → `{ organizationId, organizationName }`。無効・期限切れ・組織削除済みは `ApiError(404, INVITE_TOKEN_INVALID)` | M-02 の Server Component が事前検証に使う（§3.2）。1.x 版の `validate_admin_invite_token()`（SQL 関数）の置き換え（下記の依頼 (3)） |
...
1.3 版（K-06）で 04 に確認・依頼する事項:

1. `POST /auth/invite` の入力からパスワードを外し、`{ inviteToken, name, email }` とする（D06-30）。Admin SDK `createUser` はパスワード無し（または画面に出さないランダム値）で作成し、パスワードは再設定メールで設定させる。再設定メールの送信を画面（クライアント SDK `sendPasswordResetEmail`）とサーバ（Admin SDK `generatePasswordResetLink` + 送信）のどちらが行うかを 02・04 で確定し（00 §4.2）、サーバ送信なら本書 §3.2 の (1) を省く。
2. `POST /auth/session` は有効な ID トークンに対して常に Cookie を発行し、停止中・未所属の判定は `requireAdmin()` に委ねる（画面の前提）。異なる場合は §3.1 の失敗時の表示を 04 の応答に合わせる。
3. 招待トークンの事前検証関数 `validateInviteToken(token)` を `lib/services/` に置く（名称は 04 が確定してよい。本書は読み替える）。
4. `PATCH /api/v1/admin/me` のメールアドレス変更で、Admin SDK `updateUser({ email })` の即時反映（`pendingEmail` なし）か、確認メール方式（`pendingEmail` あり）かを確定する（§3.7 は両方に対応済み）。現在のパスワードの検証方法（D04-47）は Admin SDK にパスワード照合が無いため実装時確認。
5. `admin.login` の監査記録: `POST /auth/session` はサーバで成功を検知できるため、04 が `POST /auth/session` 内で `admin.login` を書く方式に変えてもよい。その場合は `POST /api/v1/admin/me/login-events` と本書 §3.1・§3.3 の呼び出しを廃止する（§8.3）。00 §4.2（2.0 版）は `login-events` を維持しているため、本書は当面 `login-events` を呼ぶ形のままとする。
6. 利用履歴（P-01）で `admin` に幹部を返さない条件のために `usageLogs` に `respondentKind` を複製するか、`respondents` と突き合わせるかを 02・04 で確定する（§3.4.7）。

### 8.3 監査ログ

画面は監査ログを直接書きません。`auditLogs`（00 §2.2）への `result.view`、`result.comparison`、`result.pdf_export`、`result.ai_generate`、`classification.view`、`usage_log.view`、`respondent.update_team`、`respondent.update_exclusion`、`respondent.delete`、`account.update`、`organization.rotate_invite_token`、`admin.signup`（`POST /auth/invite` が Admin SDK で書く。04 D04-36 改）の記録はサービス（04）が書きます（`action` の一覧は 04 が確定）。

例外は `admin.login` です。Firebase Auth のログイン（`signInWithEmailAndPassword`）はブラウザと Firebase の間で完結しますが、続く `POST /auth/session` はサーバで処理されるため、K-06 後はサーバ側でもログイン成功を検知できます。00 §4.2（2.0 版）は `POST /api/v1/admin/me/login-events` を維持しているため、本書は当面 **画面がセッション Cookie の発行直後に `POST /api/v1/admin/me/login-events` を 1 回呼ぶ** 形を維持します（§3.1、§3.3）。04 が `POST /auth/session` 内で `admin.login` を書く方式に変えた場合は、画面の呼び出しを廃止します（§8.2 の依頼 (5)）。呼び忘れると要件定義書 §9 の「アクセスログ」のうちログイン記録が残らないため、E2E（08）で「ログイン後に `admin.login` が 1 件記録される」ことを確認対象にします（§11）。
...
import type { AdminRole } from "@/lib/auth/claims"; // 00 §3.3（organizationId・role の型。02）
...
| D06-20 | 設計判断（04 で確定済み） | アカウント画面の管理者一覧・招待リンク再発行（要件になし。依頼範囲の指示） | オーナーのみ閲覧・再発行。API は 04 §5.11 `GET /api/v1/admin/admin-users`（D04-49）と 04 §5.2 `POST …/invite-token`（D04-25）で確定。メールアドレスは応答に含まれないため表示しない。役割変更・停止は本フェーズでは運用者（`scripts/set-admin-role`。00 §4.2、04 D04-50） | 02、04、06 |
...
| D06-30 | 設計判断（1.3 版。10 K-06） | 管理者の認証画面での Firebase Auth の使い方 | ログインは `signInWithEmailAndPassword` → `getIdToken` → `POST /auth/session` → `signOut(auth)`（00 §4.1・§4.2）。招待（M-02）ではパスワードを入力させず、`POST /auth/invite` 後に再設定メール（`sendPasswordResetEmail`）でパスワードを設定させる。パスワード再設定（M-03）は `verifyPasswordResetCode` → `confirmPasswordReset` → 自動ログイン。`/auth/callback` と `redirectTo` は廃止し、メールのアクション URL は Firebase コンソールで設定する（01）。ログアウトは `DELETE /auth/session`。理由は §3.2 の補足。メール送信の主体（画面かサーバか）とアクション URL のカスタマイズ可否は実装時確認（§3.3、§8.2） | 01、02、04、06、08 |
