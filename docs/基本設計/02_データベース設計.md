...
| # | 決定事項 | 本書での対応 |
|---|---|---|
| 1 | Next.js + Supabase（PostgreSQL、Auth、Storage）+ Vercel | §5 で `auth.users` と `admin_users` の対応、§6 で RLS、§7 で Supabase Auth の利用方法、§9 で Storage バケットのポリシー（07 分冊が利用する場合）を定める |
| 2 | 不具合 10 件の修正 | 6 番: 比較計算の列・テーブルを一切持たない（§3.2、§8）。9・10 番: `results.scoring_version` と母集団取得関数 `fetch_population()`（§8.4）。1・2・4・8 番: 内部名・識別子は共通定義に統一（enum 値は `sensory_open` など）。5・7 番は他分冊 |
| 3 | 出題は Q1〜Q144 のみ | `questions.is_active` / `is_scored` と `answers.question_no` の CHECK（1〜144）（§3.5、§3.8） |
...
flowchart LR
    subgraph respondentSide["受検者側（ログイン不要）"]
        R1["POST respondent/sessions"] -->|service_role| T1["respondents / assessment_sessions / usage_logs INSERT"]
...
- 受検者側の書き込みはサーバ（Route Handler）がサービスロールキーで行い、組織 ID とセッショントークンをサーバで検証します（共通定義 D-23、本書 §7.4）。
...
| `ai_generation_error` | text | 可 | | | 直近の失敗理由（`failed` のとき）。個人情報・API キーを含めない |
| `latest_ai_analysis_id` | uuid | 可 | | FK `ai_analyses(id)`（後付け） | 最新 AI 分析（要件定義書 §8.4） |
| `created_at` / `updated_at` | timestamptz | 不可 | `now()` | | |
| `deleted_at` | timestamptz | 可 | | | 受検者の論理削除に連動 |
...
| `provider` | text | 不可 | | 50 文字以内 | 連携先名（`AiProvider.name`。初期値 `anthropic`。決定事項 5） |
...
| `registered_at` | timestamptz | 不可 | `now()` | | 登録日時（既存の「作成日時」） |
| `submitted_at` | timestamptz | 可 | | | 回答日時（利用履歴ポップアップの「回答日時」。要件定義書 §6.2 A-06） |
| `created_at` / `updated_at` | timestamptz | 不可 | `now()` | | |
...
- 保存期間: 仮置き 1 年（設計判断 D02-17。依頼主確認事項）。
...
| 管理者追加 | `signUp({ email, password, options: { data: { invite_token, name } } })` | トリガーで `admin_users` を作成（§7.3） |
| 初期オーナー | Auth Admin API `auth.admin.createUser` または `inviteUserByEmail` に `app_metadata: { organization_id, role: 'owner' }` | トリガーで `admin_users` を作成（§7.2） |
| 受検者 | 使わない（ログイン不要。要件定義書 §3） | セッショントークン（§7.4） |
| 役割の判定 | 使わない（JWT のカスタムクレームは使わない） | RLS ヘルパー関数が `admin_users` を参照（§6.2） |

- 設計判断 D02-03: 役割・組織の決定に `raw_user_meta_data`（利用者が signUp 時に自由に設定できる）を信用しません。利用者由来の値は `invite_token` だけを受け取り、組織は DB の照合で決め、役割は必ず `admin`。`raw_app_meta_data`（サービスロールでしか設定できない）に `organization_id` と `role` があるときだけそれを採用します。
- Supabase Auth の設定（01 分冊で反映）: Email プロバイダ有効、メール確認 仮置き「有効」（設計判断 D02-22）、パスワード最小長 仮置き 8、リダイレクト URL に `{NEXT_PUBLIC_APP_BASE_URL}/auth/callback` を登録、サインアップの公開は「有効」（招待トークン無しの signUp はトリガーで失敗させる）。
...
| 母集団取得 `fetch_population()` | ○（幹部も含む数値のみ） | ○ | ○ | ○ |
...
| `admin_users` | UPDATE | `admin_users_update_self` | using: `id = auth.uid() and deleted_at is null`。with check: 同じ（列は `name` のみ許可。§6.4） |
...
| `results` | UPDATE | `results_update_ai_same_org` | using: SELECT と同じ。with check: `organization_id = current_admin_organization_id()`（列は AI 4 列のみ） |
...
| `ai_analyses` | INSERT | `ai_analyses_insert_same_org` | with check: `organization_id = current_admin_organization_id() and generated_by = auth.uid() and exists (select 1 from results r where r.id = result_id)` |
...
sequenceDiagram
    ...
    B->>N: GET /admin/signup?q=トークン
    N->>D: validate_admin_invite_token(トークン)
    D-->>N: 組織名（無効なら null）
    N-->>B: サインアップ画面（組織名を表示）／無効なら案内
    B->>A: signUp(email, password, data: { invite_token, name })
    A->>D: insert into auth.users（トリガー handle_new_auth_user）
    D->>D: トークンで organizations を照合 → admin_users(role='admin') を INSERT
    D-->>A: 成功（無効トークンなら例外 → signUp 失敗）
    A-->>B: 確認メール送信（設定による）
```
...
- トリガーが例外を投げると Supabase は signUp を 500（`Database error saving new user`）で失敗させます。画面側は事前に `validate_admin_invite_token()` で検証し、無効なトークンではフォームを出しません（06 分冊）。
...
- 招待トークンの再発行 API は共通定義 §4.2 に無いため、04 分冊が `POST /api/v1/admin/organization/invite-token` として追加するか、本フェーズでは運用者対応とするかを決めます（本書からの提案。§12 D02-02）。
...
- 04 分冊はこの関数を `supabase.rpc("fetch_population", { p_scoring_version: SCORING_VERSION, p_team_code })` で呼び、行を `PopulationMember[]` に変換して `compareWithPopulation()` に渡します。`result_id` は本人を含むかどうかの判定やデバッグ用で、応答には含めません。
...
| 組織の削除 | `organizations.deleted_at` を設定すると、ヘルパー関数は組織を返さなくなり（§6.2 は `organizations.deleted_at` を見ないため、`admin_users` 側も同時に `deleted_at` を設定する運用とする）、受検リンクも無効（04 分冊が登録時に `organizations.deleted_at is null` を確認） |
...
- 物理削除関数 `purge_deleted_respondents()` の本体は §11.16 に掲載します。
...
|---|---|---|---|---|---|
| `respondent.register` | respondent | 受検者登録（U-01） | respondents | `{ "kind": "applicant" }` | 04（service_role） |
| `session.submit` | respondent | 送信・採点（U-08） | results | `{ "scoringVersion": "1.0.0" }` | 04（service_role） |
| `admin.signup` | admin | 招待リンクからの登録 | admin_users | | 04（初回ログイン時に補完） |
...
## 9. Storage（PDF を保存する場合）

PDF を Storage に保存するかどうかは 07 分冊が決めます（共通定義 §6）。保存する場合のバケットとポリシーを用意しておきます。保存しない（都度生成してストリーム返却）なら本節は適用しません。
...
- 正は `lib/masters/questions.ts`（03 分冊）。`supabase/seed/questions.sql` はそこから生成し、`insert ... on conflict (question_no) do update` で冪等にします。生成スクリプトの置き場所は 01 分冊（提案: `scripts/generate-questions-seed.ts`）。
...
- ローカルでは `supabase db reset` 後に `supabase/seed/questions.sql` を適用します（`supabase/config.toml` の `db.seed.sql_paths` に登録。01 分冊）。本番は 08 分冊のリリース手順で適用します。
...
ファイルは番号順に適用します。各ファイルの内容をそのまま保存してください。本書の全 SQL は PostgreSQL 16.13 上で、Supabase 相当のスタブ（`auth.users`、`auth.uid()`、`anon` / `authenticated` / `service_role` ロール、`extensions` / `storage` スキーマ）を置いて番号順に適用し、§7.3 のトリガー、§6.3 の RLS（admin・owner・停止中・anon）、列権限、`finalize_assessment_session()`・`soft_delete_respondent()`・`purge_deleted_respondents()`・`fetch_population()` の動作を確認済みです（08 分冊はこれを結合テストとして移植してください）。
...
create table public.results (
  ...
  constraint ck_results_ai_error_length check (ai_generation_error is null or length(ai_generation_error) <= 2000)
);
...
comment on table public.ai_analyses is 'AI 解説の生成履歴。成功時のみ行を作る。raw_json は付録D §2 のスキーマ';
```
...
§8.4 の `fetch_population()`、§8.5 の `soft_delete_respondent()`、下記の `provision_organization()`、`finalize_assessment_session()`、`purge_deleted_respondents()` を収めます。
...
- `alter table ... disable trigger` は関数所有者（テーブル所有者 `postgres`）の権限で実行されます。`security definer` のため `service_role` から呼んでも動作します。この関数は運用者が手動または定期ジョブ（01 分冊）で実行します。
- `finalize_assessment_session()` に渡す `p_result` は `lib/db/mappers/result.ts` の `toResultInsertJson(scoreResult)` が作ります（§12）。

### 11.17 `20260917000017_create_storage_pdf_exports.sql`（07 分冊が Storage を使う場合のみ）

§9 の SQL をそのまま収めます。使わない場合はこのファイルを作りません。
...
| バックアップ・PITR | 01 分冊 |
| 保持期間 | 論理削除の物理削除 180 日（仮置き）、監査ログ 1 年（仮置き）。依頼主確認事項 |
...
| D02-03 | 設計判断 | 役割・組織の決定は `auth.users` の INSERT トリガーで行い、利用者由来の値は `invite_token` のみ信用する | `raw_app_meta_data`（サービスロール専用）に `organization_id` / `role` があれば優先 | 01、04 |
...
| D02-07 | 設計判断 | 母集団は RLS を通さない `fetch_population()` で取得し、幹部・本人を含める（共通定義 D-05、D-06） | 返す列は 16 尺度と 5 軸のみ | 03、04 |
...
| D02-10 | 未確認（要件定義書 §12） | 削除の論理／物理と保持期間 | 論理削除（共通定義 D-11）。物理削除は 180 日後に運用者が `purge_deleted_respondents()` を実行 | 04、08 |
...
| D02-17 | 設計判断 | `audit_logs` は追記専用、閲覧は owner のみ。`audit_actor_kind` enum を追加。保持 1 年（仮置き） | 記録対象は §8.6 | 04、07 |
| D02-18 | 未確認（要件定義書 §8.1） | 組織の「コード」「顧客番号」の用途・一意性 | 任意入力、一意制約なし | 06 |
| D02-19 | 設計判断 | PDF を Storage に保存する場合のバケット `pdf-exports` とポリシーを用意 | 使うかどうかは 07 が判断 | 07 |
...
| D02-22 | 未確認（要件定義書 §12） | サインアップ・パスワード再設定の画面文言とメール設定 | メール確認あり、パスワード最小 8 文字 | 01、06 |
