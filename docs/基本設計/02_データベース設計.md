...
| 版 | 2.0（技術構成の変更（Supabase → Firebase。10 K-06）に伴う全面改版。1.x 版の PostgreSQL・RLS・SQL 関数前提の記述はすべて置き換えた。変更点は §16） |
| 作成日 | 2026-09-17（1.1 版: 2026-09-19、1.2 版: 2026-09-21、2.0 版: 2026-09-21） |
...
| Admin SDK の初期化 | `lib/firebase/admin.ts` の単一インスタンス（`getFirestore()`）。`lib/db/` 以外から `firebase-admin/firestore` を import しない（共通定義 §3.3） |
...
export function organizationsRef(): CollectionReference<OrganizationDoc> {
  return getAdminFirestore().collection(COLLECTIONS.organizations).withConverter(docConverter<OrganizationDoc>());
}
export function adminUsersRef(): CollectionReference<AdminUserDoc> {
  return getAdminFirestore().collection(COLLECTIONS.adminUsers).withConverter(docConverter<AdminUserDoc>());
}
export function respondentsRef(): CollectionReference<RespondentDoc> {
  return getAdminFirestore().collection(COLLECTIONS.respondents).withConverter(docConverter<RespondentDoc>());
}
export function assessmentSessionsRef(): CollectionReference<AssessmentSessionDoc> {
  return getAdminFirestore().collection(COLLECTIONS.assessmentSessions).withConverter(docConverter<AssessmentSessionDoc>());
}
export function resultsRef(): CollectionReference<ResultDoc> {
  return getAdminFirestore().collection(COLLECTIONS.results).withConverter(docConverter<ResultDoc>());
}
export function aiAnalysesRef(): CollectionReference<AiAnalysisDoc> {
  return getAdminFirestore().collection(COLLECTIONS.aiAnalyses).withConverter(docConverter<AiAnalysisDoc>());
}
export function usageLogsRef(): CollectionReference<UsageLogDoc> {
  return getAdminFirestore().collection(COLLECTIONS.usageLogs).withConverter(docConverter<UsageLogDoc>());
}
export function auditLogsRef(): CollectionReference<AuditLogDoc> {
  return getAdminFirestore().collection(COLLECTIONS.auditLogs).withConverter(docConverter<AuditLogDoc>());
}
```

- 実装時確認: `withConverter` を付けた参照に対する `update()` の部分更新（`FieldValue.serverTimestamp()` やフィールドパス指定）の型付け。型が合わない場合は `update()` だけコンバータ無しの参照（`getAdminFirestore().collection(...).doc(id)`）で行います。
...
export const REPOSITORY_ERROR_CODES = [
  ...
  "ADMIN_USER_NOT_FOUND",        // adminUsers 文書が無い（401 ADMIN_NOT_REGISTERED）
...
| Q11 | `auditLogs` | `action ==`, `ipAddress ==`, `createdAt >= 現在 − 10 分`（`count()`） | 受検者登録のレート制限（04 §2.8、D02-28） | `action, ipAddress, createdAt` |
| Q12 | `auditLogs` | `organizationId ==` → `createdAt desc` | 運用者の参照・結合テスト | `organizationId, createdAt desc` |
| Q13 | `results` | `organizationId ==`, `deletedAt == null`（`count()`） | 一覧の `total`（04 D04-04）。Q3 / Q4 と同じ条件の集計 | Q3 / Q4 と共用 |

- 一覧のページング（04 が確定）は `limit` + `offset` または `startAfter(submittedAt, id)` のカーソル方式のどちらでも Q3 / Q4 のインデックスで処理できます。`offset` は読み飛ばした文書も読み取り課金の対象になる（実装時確認）ため、1 組織あたり数百件規模で `pageSize` 上限 200 の前提では影響は小さいものの、カーソル方式を推奨します（§12）。
...
- `count()` 集計クエリ（Q11、Q13）の課金は「インデックスエントリ 1,000 件ごとに 1 読み取り」（実装時確認）で、通常のクエリより安価です。
...
- デプロイは Firebase CLI（`firebase deploy --only firestore:indexes,firestore:rules`。01 §6、08 リリース手順）。本番のインデックス作成には時間がかかるため、リリース手順ではルール・インデックスをアプリより先にデプロイします。
...
| | `listResults()` | 読み取り（Q3 / Q4 + Q13） | `GET /api/v1/admin/results` | 一覧 SELECT |
...
| `ai-analyses-repository.ts` | `markAiGenerationStarted()` | トランザクション（状態検査） | `POST …/ai-analysis` | UPDATE AI 列 |
...
| `usage-logs-repository.ts` | `listUsageLogs()` | 読み取り（Q5 / Q6） | `GET /api/v1/admin/usage-logs` | RLS `usage_logs_select_same_org` |
...
| | `countRecentAuditLogs()` | `count()` 集計（Q11） | 04 §2.8 レート制限 | `audit_logs` の COUNT |
...
export interface ListResultsInput {
  readonly viewer: Viewer;
  readonly limit: number;                   // 1〜200（04 D04-04）
  readonly cursor: { readonly submittedAt: Date; readonly resultId: string } | null;  // 前ページ最終行。null なら先頭
}
export interface ListResultsOutput {
  readonly items: readonly Result[];        // submittedAt 降順（同時刻は resultId 昇順で安定化）
  readonly total: number;                   // count() 集計（Q13）
  readonly nextCursor: { readonly submittedAt: Date; readonly resultId: string } | null;
}
/**
 * 回答一覧。Q3（owner / super_admin）または Q4（admin: respondentKind == "applicant"）。
 * 氏名などは含まれないため、呼び出し元が items の respondentId で getRespondentsByIds() を呼び突き合わせる（04）。
 * orderBy は submittedAt desc, __name__ asc（カーソルの一意性のため。実装時確認: 複合インデックスに __name__ を
 * 明示する必要の有無）
 */
export function listResults(input: ListResultsInput): Promise<ListResultsOutput>;
...
/**
 * 組織内分類用。Q3 / Q4 と同じ条件で全件を select("respondentId", "respondentKind", "aptitudeType", "socialStyle", "submittedAt")
 ...
 * select("traits", "compatibility") で射影し、toPopulationMember() で変換する。
 * 個人情報は含まない。結果は保存しない（要件定義書 §11 の 6 番）
 */
export function fetchPopulation(input: { readonly organizationId: string; readonly scope: ComparisonScope }): Promise<readonly PopulationMember[]>;
```
...
export async function fetchPopulation({ organizationId, scope }: { organizationId: string; scope: ComparisonScope }) {
  ...
  return snapshot.docs.map((doc) => toPopulationMember(doc.data()));
}
```
 ...
 * generating / aiGenerationStartedAt = now に更新する。completed なら何もせず { alreadyCompleted: true } を返す
 * （07 §6: 再表示時は再生成しない）。generating（滞留判定は 07）の扱いも 07 の規則に従う。
 * 見えなければ RepositoryError("RESULT_NOT_FOUND")
 */
export function markAiGenerationStarted(input: { readonly resultId: string; readonly viewer: Viewer }): Promise<{ readonly alreadyCompleted: boolean; readonly latestAiAnalysisId: string | null }>;
...
export interface ListUsageLogsInput {
  ...
  readonly cursor: { readonly registeredAt: Date; readonly usageLogId: string } | null;
}
/** 利用履歴。Q5（owner / super_admin）または Q6（admin: respondentKind == "applicant"）。registeredAt 降順 */
export function listUsageLogs(input: ListUsageLogsInput): Promise<{ readonly items: readonly UsageLog[]; readonly total: number; readonly nextCursor: ListUsageLogsInput["cursor"] }>;
```
...
/** 他のリポジトリがバッチ／トランザクションに監査ログを含めるための内部関数 */
export function addAuditLogToBatch(batch: WriteBatch | Transaction, entry: AuditEntry): string;
/** レート制限用（Q11、count() 集計）。action と ipAddress が一致し createdAt >= since の件数 */
export function countRecentAuditLogs(input: { readonly action: string; readonly ipAddress: string; readonly since: Date }): Promise<number>;
```

- `countRecentAuditLogs()` は `ipAddress` が `null` のリクエストには使えません（04 が `null` のときの扱いを決める）。実装時確認: `count()` 集計クエリが不等号条件（`createdAt >=`）と等価条件の組み合わせで使えること。
...
- クレームは ID トークン・セッション Cookie に含まれ、`verifySessionCookie()` の戻り値（`DecodedIdToken`）から読みます。`lib/auth/claims.ts` の `parseAdminClaims(decoded): AdminClaims | null` が `organizationId`（英数字 1〜128 文字）と `role`（3 値）の形式を検証し、満たさなければ `null`（04 は 401）。
...
- 認可の判定はクレームだけで行えますが、利用停止・削除の即時反映のため `requireAdmin` は `adminUsers` 文書も毎回読みます（§9.5。1 読み取り／リクエスト。§12）。クレームと文書の `organizationId`・`role` が食い違う場合（`set-admin-role` の途中失敗）は **クレームを正** とし、監査ログに `admin.claims_mismatch`（actorKind: system）を残して 04 が 401 にします（運用者が `scripts/set-admin-role --sync` で文書をクレームに合わせる）。
...
| 文書が無い（招待処理の途中失敗） | 成功 | なし | 401 `ADMIN_NOT_REGISTERED` |
| クレーム無し・不正 | 成功 | — | 401 |

- 停止解除は `updateUser(uid, { disabled: false })` と `isSuspended = false` です（トークンの再発行は不要。再ログインで新しい Cookie を得る）。
- 実装時確認で `checkRevoked` が `disabled` を拒否しない場合でも、文書の `isSuspended` により管理 API は拒否されます。Cookie の有効期間中（最大 14 日）にログインだけができてしまう可能性は無く（`disabled` はログイン自体を拒否する。実装時確認）、既存 Cookie による API 利用は文書の確認で止まります。
...
`POST /auth/invite { inviteToken, displayName, email }`（入出力の確定は 04 §6.2。パスワードは受け取らない。D02-39）の処理:
...
sequenceDiagram
    ...
    B->>N: POST /auth/invite { inviteToken, displayName, email }
    N->>D: findOrganizationByInviteTokenHash（再検証。null なら 404 INVITE_TOKEN_INVALID）
    N->>A: createUser({ email, displayName, emailVerified: true, password: 乱数 })
...
- 代替案（採用しない）: 1.x 版と同じく `POST /auth/invite` でパスワードも受け取り `createUser({ password })` で設定する。フォーム 1 回で完了する利点があるが、メールアドレスの実在確認ができない（`emailVerified: true` で作るため）。再設定メール方式なら「メールを受け取れる人だけがログインできる」ことが担保されるため、こちらを採用します。06 M-02 の項目（パスワード入力欄を無くす）と 04 §6.2 の入力（`password` を外す）への申し送りです（§15.1）。
...
| メールアドレス変更（`PATCH /api/v1/admin/me`） | サーバの Admin SDK `updateUser(uid, { email })` | 確認メールは送られない（Admin SDK の更新は即時。実装時確認）。誤入力の防護として 04 が現在のパスワードの再入力（クライアント SDK `reauthenticateWithCredential` で得た ID トークンを添付）を要求するかを決める。監査ログ `account.update`（`fields: ["email"]`。値は入れない） |
| パスワード変更（同上） | サーバの Admin SDK `updateUser(uid, { password })` + `revokeRefreshTokens(uid)`（他端末のセッションを失効） → 自端末は新しい Cookie を再発行 | 最小 8 文字（04 のスキーマ）。Firebase 側のパスワードポリシー（実装時確認: コンソールで設定可能な最小長・文字種）と一致させる |
...
| 項目 | 内容 |
|---|---|
| 発行 | `POST /auth/session { idToken }` → `verifyIdToken(idToken, true)` → `auth_time` が直近 5 分以内であることを確認（古い ID トークンの再利用防止。実装時確認: `DecodedIdToken.auth_time` の単位）→ `parseAdminClaims` が `null` なら 401 → `createSessionCookie(idToken, { expiresIn })` → `Set-Cookie`（名前は `SESSION_COOKIE_NAME`、初期値 `admin_session`。HttpOnly、Secure、SameSite=Lax、Path=/）。発行後、ブラウザは `signOut()` してクライアント SDK の状態を捨てる（認証状態を Cookie に一本化） |
| 有効期限 | `expiresIn` は 14 日（Firebase の上限。実装時確認: 5 分〜14 日）。04 が短縮を決めてよい |
...
| 監査 | ログイン成功は `POST /api/v1/admin/me/login-events`（04 D04-24）で `admin.login` を記録する。`POST /auth/session` 自体でも `admin.login` を書けるが、04 の既定に合わせる（どちらか 1 箇所。04 が確定） |

```ts
// lib/auth/session-cookie.ts（契約。実装は 04 分冊）
import type { Viewer } from "@/lib/auth/claims";

export const SESSION_COOKIE_MAX_AGE_MS: number;   // 14 日
export function createAdminSessionCookie(idToken: string): Promise<{ readonly cookie: string; readonly expiresAt: Date; readonly viewer: Viewer }>;
export function verifyAdminSessionCookie(cookie: string): Promise<Viewer | null>;   // 失効・クレーム不正は null
export function revokeAdminSessions(uid: string): Promise<void>;
```
...
| I-9 | `adminUsers` の文書 ID = Auth の `uid`、`organizationId`・`role` はクレームと一致 | `createAdminAccount()` の手順順序（クレーム → 文書）。`changeAdminRole()` が両方を更新。不一致は 401 + `admin.claims_mismatch` | 08: 招待後のクレームと文書の一致 |
...
| `account.update` | admin | 氏名・メール・パスワード変更（A-12） | adminUsers | `{ "fields": ["displayName"] }`（値は入れない） | service |
...
| 回答一覧（1 ページ。`pageSize` = P、N 件中） | 2 + P + P | 1 | `adminUsers` 1、`count()` 1（N ≤ 1,000 のとき）、`results` P（カーソル方式。`offset` 方式は読み飛ばし分も課金される。実装時確認）、`respondents` P（`getAll`）／監査 `result.list`。P = 200、N = 300 なら約 402 読み |
| 組織内分類 | 1 + N | 1 | `results` 全件（`select` で射影） |
| 利用履歴（1 ページ） | 2 + P | 1 | `count()` 1、`usageLogs` P |
...
### 15.1 他分冊への改版依頼・申し送り（2.0 版）

本書の Firestore 化で確定した事項のうち、他分冊（同時に改版中の 01・04・08 を含む）の記述に影響するものです。00 共通定義の変更を要するものはありません（00 §2.2 の「主要フィールド」に本書が追加したフィールドは、00 が「詳細は 02」としている範囲内です）。

| 宛先 | 内容 | 本書の根拠 |
|---|---|---|
| 04 §4.2、§4.4、§4.5 | 受検者 API は `registerRespondent()`・`saveAnswers()`・`submitSession()` を呼ぶ。`submitSession()` は永続化済みの回答をトランザクション内で採点する（service で `scoreAnswers` を呼んでから渡す 1.x 版の手順は不要。`deps` で注入）。`RepositoryError` のコードと HTTP ステータスの対応（§5.6） | §8.4、§5.6 |
| 04 §4.4、05 | `assessmentSessions.lastSavedPageNo`（1〜20）に統合した（1.x 版の `last_saved_step` / `last_saved_page` は廃止）。`exam-pages.ts` で `step` / `page` に変換する | §3.4 D02-37 |
| 04 §5.1（一覧）、§5.8（利用履歴） | ページングはカーソル方式（`submittedAt` + `resultId`）を推奨。`total` は `count()` 集計 | §8.5、§7.1、§12 |
| 04 §5.2、06 M-07 | 管理者追加用リンクは **再発行時の応答でのみ平文を 1 回表示** し、アカウント画面に常時表示しない（`inviteTokenHash` のみ保存）。`GET /api/v1/admin/me` の `links.adminInvite` は返さず、`POST /api/v1/admin/organization/invite-token`（owner）の応答に `inviteUrl` を含める。採用しない場合の代替は §3.1 | §3.1 D02-35、§9.7 |
| 04 §6.2、06 M-02 | `POST /auth/invite` の入力からパスワードを外し（`{ inviteToken, displayName, email }`）、成功後に画面がクライアント SDK `sendPasswordResetEmail` を呼ぶ。途中失敗の再実行規則（同じメールで再要求 → クレーム・文書が無ければ補完） | §9.7 D02-39、D02-40 |
| 04 §2.5（`requireAdmin`） | `verifySessionCookie(cookie, true)` → `parseAdminClaims` → `getAdminUser(uid)` で `isSuspended` / `deletedAt` を毎回確認する（1 読み取り）。クレームと文書の不一致は 401 + `admin.claims_mismatch` | §9.2、§9.5 |
| 04 §2.6 | `AuditAction` に `organization.create`、`organization.rotate_invite_token`、`admin.create`、`admin.role_change` / `admin.suspend` / `admin.unsuspend` / `admin.delete`、`admin.claims_mismatch`、`session.submit`、`respondent.delete` を追加（リポジトリ・スクリプトが書くものを含む）。`purge` 系は削除 | §11 |
| 04 §6.3、06 | パスワード再設定はクライアント SDK `sendPasswordResetEmail` + カスタムアクション URL `/admin/password-reset`（`confirmPasswordReset`）。サーバ側の `generatePasswordResetLink` は使わない | §9.8 |
| 01 §5（Firebase プロジェクト設定） | Auth: メール／パスワードのみ、自己登録の無効化、メールテンプレート（再設定・メールアドレス変更）、アクション URL のカスタマイズ、パスワードポリシー（最小 8 文字）、メール列挙保護の設定値。Firestore: `(default)`、ロケーション、ルール・インデックスのデプロイ。Storage: バケットを作らない（作る場合は全拒否ルール） | §6、§9.7、§9.8 |
| 01 §2、§4 | `scripts/create-owner`（`--org-name` ほか）、`scripts/set-admin-role`（`--role` / `--suspend` / `--delete` / `--sync`）、`scripts/seed-local`（`SEED_OWNER_EMAIL` / `SEED_OWNER_PASSWORD`）の引数と環境変数。Emulator 使用時は `FIREBASE_SERVICE_ACCOUNT_KEY` 不要 | §9.6、§9.9、§13.2 |
| 07 §7.1 | `aiAnalyses` のフィールド定義（§3.6）と `saveAiAnalysis()` / `markAiGenerationStarted()` / `markAiGenerationFailed()` の契約。`organizationId` / `respondentId` は `results` 文書から写す | §3.6、§8.6 |
| 08 §2、§5 | 結合テストの前提を本書の関数名（`registerRespondent()` など）・エラーコード・不変条件 I-1〜I-14・実装時確認事項（§13.4）に合わせる。Emulator のクリア方法（§13.3）。PF-05 の `results` 直接 `set` を許容 | §8、§10、§13 |
| 03 | `submitSession()` が `assertAnswerMap`（`Record<number, number>` → `AnswerMap`）と `scoreAnswers` を `deps` として受け取る。両関数の名称は 03 が正 | §8.4 |
...
| D02-35 | 設計判断（2.0 版で追加） | 招待トークンは平文を保存せずハッシュのみ保存し、リンクは再発行時の応答で 1 回だけ表示する | 既存の「固定リンクを常時表示」（A-12）との差。採用しない場合は `inviteToken`（平文）を保存し `GET /admin/me` で返す（§3.1）。依頼主確認事項 | 04、06 |
...
| D02-42 | 実装時確認（2.0 版で追加） | `verifySessionCookie(cookie, true)` が `disabled` ユーザーと `revokeRefreshTokens` 後の Cookie を拒否すること、`expiresIn` の上限 14 日 | 拒否しない場合も `adminUsers.isSuspended` の毎回確認で管理 API は止まる（§9.5） | 01、04、08 |
| D02-43 | 設計判断（2.0 版で追加） | `requireAdmin` が `adminUsers` 文書を毎回読む（1 読み取り／リクエスト） | 停止・削除の即時反映とクレーム不一致の検出のため。コストは §12 に計上 | 04 |
