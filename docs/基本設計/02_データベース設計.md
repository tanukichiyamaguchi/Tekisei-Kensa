...
- 本番プロジェクトに対する実行は依頼主または運用責任者が行います（01 §3.4、08）。本番では GitHub Actions の firebase-ops ワークフローの create-owner で実行し、Environment production の必須レビュアーの承認を経ます。リポジトリは公開のため、メールアドレスは env に置かず、ジョブの最初のステップで `$GITHUB_EVENT_PATH` の `.inputs.email` を読んで `::add-mask::` します（10 K-17、PR #12）。初期オーナーは 2026-09-23 にこの方法で作成し、パスワード再設定メールで初期パスワードを設定してログインできました。実装者はローカル Emulator と検証プロジェクトに対してのみ実行します。
