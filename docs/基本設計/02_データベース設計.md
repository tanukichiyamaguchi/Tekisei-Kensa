...
| 版 | 2.2（2.2 版: M2 実装時の I-50 の結果により利用履歴の昇順インデックスを追加。2.0 版: 技術構成の変更（Supabase → Firebase。10 K-06）に伴う全面改版。2.1 版: Firebase 化後の分冊間整合（04・01・08 の 2.0 版と突き合わせ、04 が必要とする関数・インデックスを追加）。変更点は §16） |
| 作成日 | 2026-09-17（1.1 版: 2026-09-19、1.2 版: 2026-09-21、2.0 版: 2026-09-21、2.1 版: 2026-09-21、2.2 版: 2026-09-23） |
...
| `occupationCode` | number | ○ | 職業コード（共通定義 §1.10。1 アイリスト〜8 その他。10 K-19） | 新規登録は整数 1〜8（`occupationCodeSchema`）。保存済み文書の読み取り（`storedOccupationCodeSchema`）は K-19 より前の旧データの 9 も受け付け、「その他」と表示する（10 K-19、PR #13・#14） | |
...
| 職業（8 種。旧コード 9 は「その他」と表示する。10 K-19） | コード定数 `lib/masters/occupations.ts` | `respondents.occupationCode` は整数、表示名はコード側 |
...
export interface RespondentDoc extends SoftDeletableDoc {
  ...
  readonly occupationCode: number;          // 新規は 1〜8。K-19 より前の旧データは 9 もある（「その他」と表示。10 K-19）
...
| Q5 | `usageLogs` | `organizationId ==` → `registeredAt desc`（`order=asc` のときは `asc`） | 利用履歴（owner / super_admin） | `organizationId, registeredAt desc` と `organizationId, registeredAt asc`（2.2 版で昇順を追加。D02-46） |
| Q6 | `usageLogs` | `organizationId ==`, `respondentKind == "applicant"` → `registeredAt desc`（`order=asc` のときは `asc`） | 利用履歴（admin） | `organizationId, respondentKind, registeredAt desc` と同 `asc`（2.2 版で昇順を追加。D02-46） |
...
export interface RegisterRespondentInput {
  ...
  readonly occupationCode: number;          // 1〜8（occupationCodeSchema。10 K-19）
  readonly diagnosisExperience: DiagnosisExperience;
...
手順（`scripts/create-owner.ts`。`pnpm owner:create --org-name "..." --org-code "..." --customer-number "..." --email {オーナーのメールアドレス} --display-name "責任者"`。既存組織にオーナーを追加する場合は `--organization-id`）:
...
- 本番プロジェクトに対する実行は依頼主または運用責任者が行います（01 §3.4、08）。本番では GitHub Actions の firebase-ops ワークフローの create-owner で実行します（2026-09-23 の実行時は Environment production の必須レビュアーの承認を経ましたが、リポジトリを非公開にしたため、現在はリポジトリ全体の Secret・Variable で動き、承認はありません。10 K-17）。実行ログに個人情報を残さないため、メールアドレスは env に置かず、ジョブの最初のステップで `$GITHUB_EVENT_PATH` の `.inputs.email` を読んで `::add-mask::` します（10 K-17、PR #12）。初期オーナーは 2026-09-23 にこの方法で作成し、パスワード再設定メールで初期パスワードを設定してログインできました。実装者はローカル Emulator と検証プロジェクトに対してのみ実行します。
...
| D02-46 | 設計判断（2.2 版で追加） | 利用履歴（Q5 / Q6）の複合インデックスを `registeredAt` の降順と昇順の 2 本ずつ定義する | 04 §5.8 の `order`（`asc` / `desc`）で両方向の並びを発行する。Firestore の自動作成の単一フィールドインデックスが昇順・降順を別に持つことから、複合インデックスも方向ごとに必要と判断した（M2 の I-50 がクエリとの照合で不足を検出）。本番での要否は検証用プロジェクトで確認する | 04、08 |
...
| 2.2 | 2026-09-23 | M2 実装時の結合テスト I-50 の結果を反映: Q5 / Q6 に `registeredAt` 昇順の複合インデックスを追加（§7.1、§7.3。複合インデックスは 14 本）。D02-46 を追加 |
| 2.3 | 2026-09-24 | 本番デプロイで判明した事項の反映（職業コードの検証範囲を新規 1〜8・旧データ 9 は「その他」表示に更新、`scripts/create-owner` の表示名の例を「責任者」に、本番での初期オーナー作成を firebase-ops ワークフローで行う手順と作成済みの旨を追記。10 K-17・K-19、PR #12・#13・#14） |
| 2.4 | 2026-09-24 | §9.6 の本番での実行方法を、リポジトリの非公開化（10 K-17 の改定。Environment を使わずリポジトリ全体の Secret・Variable、承認なし）に合わせて更新 |
