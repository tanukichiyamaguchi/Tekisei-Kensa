...
| D-06 | 未確認（要件定義書 §12）＋依頼主確認事項 | 母集団に既存スタッフ（幹部）の結果を含めるか | 含める（除外フラグで制御可能）。ただし `admin` が母集団経由で幹部の値を推定できるリスクがあり、02 D02-07 は「`admin` のときは幹部を除く」案を推奨（採用時は本行と 02 `fetch_population()`、08 I-25 を改版） | 02、03、04、08 |
...
| D-17 | 未確認（要件定義書 §12）→ 07 で確定 | AI 連携のモデル名・生成パラメータ | Claude API を使用（決定事項 5）。初期値 `claude-opus-5`、`max_tokens 16000`、適応思考、`effort high`、構造化出力（07 §4.1、§4.2）。コスト重視の代替 `claude-sonnet-5` は依頼主判断（07 D07-08） | 07 |
