...
| 版 | 1.1（レビュー指摘反映: T-14 境界値、§4.2・§5.10 誤記、§7.1 母集団条件、§7.3 未確認表記、§8.2 色注釈、D3-06・D3-14・D3-18） |
| 作成日 | 2026-09-17（改版 2026-09-19） |
...
- `lib/scoring/` と `lib/masters/` は Next.js・Supabase に依存しない純粋な TypeScript とし、I/O（DB、HTTP、ファイル）を一切行いません（00 §3.3）。DB からの母集団取得や結果保存は 04 分冊（`lib/services/`、`lib/db/`）の範囲です。
...
| # | 決定事項 | 本書での対応 |
|---|---|---|
| 1 | Next.js + Supabase + Vercel、ApexCharts | 採点エンジンはフレームワーク非依存の純関数（§1.2）。グラフ用の系列データは `lib/presentation/` で `ScoreResult` / `ComparisonResult` から整形する（§8） |
| 2 | 要件定義書 §11 の不具合 10 件を修正 | §6 に反映点一覧。優劣性は計算値を保存、思考の傾向は `compat_thinking_tendency` を参照、母集団は 1 種類、比較値は非永続化、同点順は定数化、Q145〜Q204 非採点、内部名統一、`scoring_version` 付与 |
...
- `scripts/` は 01 分冊が定義済みのディレクトリ（`scripts/generate-question-seed.ts` など）です。`generate-masters.ts` はそこに追加します。
- 設問文の正は `lib/masters/questions.ts`（実体は `data/questions.json`）であり、`supabase/seed/questions.sql` は 01 分冊の `scripts/generate-question-seed.ts` がこれから生成します（00 §6 分冊間の境界）。
...
| 1〜204 以外のキーが存在する | 無視する（設計判断 D3-14: 純関数としての防御的な扱い。API 層は回答保存時に `questionNo` 1〜144 以外を 422 で拒否し（04 §4.4）、送信 API は DB の `answers` から `AnswerMap` を組み立てるため、通常運用ではこのようなキーは採点エンジンに到達しない） |

- 回答の型はオブジェクトのキーが文字列に変換される点に注意します。`assertAnswerMap` は `Number(key)` で正規化して検証します。
...
補足: 00 §1.1 および §3.4 のコメントはソーシャルスタイル 4 値を「0〜30」と記載しています（付録B §7 の見出し「（0〜30）」を踏襲した値域）。一方、付録B §7 の式（所属 4 タイプの得点の最大値 ÷ 2）から導出すると、16 タイプ得点が −58〜58 のため計算値は −29〜29 になり得ます（全問「そう思わない」で −29。§5.10、§10.4 の全問 5 の列）。本書は 00 を読み替えず、この差異を **00 の改版が必要な事項** として §12 の D3-06 に記録します。改版までの本書の扱いは「採点エンジンではクランプせず、DB にも計算値のまま保存する（02 の `numeric(7,3)` は負値を許容）。レーダー描画時に 0 に丸める（06）」で、02・06 分冊と揃っています（§8.4）。
...
| 値域・刻み | −100〜100、整数 |
| 出力 | `ScoreResult.compatibility`（`COMPATIBILITY_KEYS` 順の 5 値）。**DB には負値のまま保存**（00 §1.3） |
...
| 内部名 | JSON マスタの `internalName` は「感性開放型」。「感性解放型」はコード・マスタ・DB に一切出さない（00 §1.4） |
...
| 出力 | `ScoreResult.aptitudeTypeScores`（16 値。新システムでは `results.type_*` に保存）、`aptitudeType` |
...
| 値域・刻み | 計算値は −29〜29、0.25 刻み（§5.10）。00 §1.1・§3.4 の「0〜30」とは食い違うため 00 の改版を要請する（§2.2 補足、§12 の D3-06）。設計判断: クランプせず計算値を保存 |
...
| 浮動小数 | `0.49 × (−1.5n)` は 2 進小数で誤差を持つ（例: n = 14 で 89.71000000000001 になり得る）。採点エンジンでは丸めず、DB の `numeric(7,3)` への保存で小数第 3 位に丸まる（数学的な真値 89.71 と一致する。§9.1）。テストは許容誤差 1e-9 で比較する |
...
- 戻り値は凍結し、呼び出し側（04 の `lib/db/mappers/`）が `results` の列へ変換します。列との対応は 00 §2.1 の接頭辞規則（`trait_*`、`compat_*`、`aptitude_*`、`risk_*`、`type_*`、`style_*`、`reliability`、`aptitude_first`、`aptitude_second`、`aptitude_type`、`social_style`、`scoring_version`）です。
...
| 6 | 比較値を閲覧者アカウントに書き込み | `ComparisonResult` を返すだけで永続化しない。DB へ書く関数を `lib/scoring/` に置かない | `compare.ts`、04 分冊 | 08 分冊（結合テスト: 比較 API 呼び出し後に `admin_users` が更新されないこと） |
...
| 条件 | 内容 | 担当 |
|---|---|---|
| 同一組織 | `results.organization_id` が閲覧対象と一致 | 04（取得） |
| 除外でない | `respondents.is_excluded = false` | 04 |
| 現行ロジックで採点済み | `results.scoring_version = SCORING_VERSION` | 04 |
| 論理削除されていない | `results.deleted_at is null` かつ `respondents.deleted_at is null`（02 §8.4 の `fetch_population()` と同じ条件）。`assessment_sessions.deleted_at` は条件に含めないが、02 §8.5 の `soft_delete_respondent()` が `respondents`・`assessment_sessions`・`results` の `deleted_at` を同一時刻で設定するため、結果として同等に担保される | 04（02 の `fetch_population()` を呼ぶ） |
| チーム選択時 | `respondents.team_code = scope.teamCode` | 04 |
| 本人の包含 | 閲覧対象の受検者自身も条件を満たせば含める（**仮置き D-05**） | 04（含めて渡す） |
| 幹部の包含 | `respondent_kind = executive` も条件を満たせば含める（**仮置き D-06**）。閲覧者が `admin` で幹部の結果を見られない場合でも、母集団の平均には含める（00 §5 補足） | 04 |

- `compareWithPopulation` は渡された `population` をそのまま母集団として扱い、**フィルタも重複排除もしません**。`scope` は結果に転記するためだけに受け取ります（呼び出し側が正しい条件で取得した責任を持つ）。
- 母集団の各要素は `PopulationMember`（16 尺度と 5 軸のみ。00 §3.4）で、`results` の `trait_*` と `compat_*` 列から `lib/db/mappers/` が作ります。
...
| 保存 | 比較計算の結果は保存しない（要件定義書 §11 の 6 番）。`ScoreResult` は `numeric(7,3)` に保存し、真値が小数第 3 位以内（§9.1）なので丸め損失はない |
...
`lib/presentation/` は `ScoreResult` / `ComparisonResult` と `lib/masters/` だけに依存し、DB・API には触れません（00 §3.3）。
...
| DB 保存 | `numeric(7,3)`（00 §2.1）。真値は最大 3 桁なので損失なし。浮動小数の誤差（例: 89.71000000000001）は保存時に小数第 3 位で丸まり真値に戻る |
...
| T-11 | 6. 比較計算 | §7.7 の表。既存の観測値は「思考の傾向に適応力を使う」不具合込みの値なので、新システムの `computeAxisDeviations` に受検者の `compat_thinking_tendency = 28` を渡すと 53.195、偏差値 69.189、評価 E、立ち位置 `strong_leader` になることを検証。合致度 49.8409… と他 4 軸の偏差は観測値と一致 | §7.7 |
...
| 送信 API 全体 2 秒以内 | 採点はボトルネックにならない。DB 書き込みを含む計測は 08 分冊 | － |
...
| D3-06 | 設計判断（**00 改版要請**） | ソーシャルスタイル 4 値は付録B §7 の式から計算上 −29〜29 になり得るが、00 §1.1・§3.4 は「各 0〜30、0.25 刻み」と記載している。00 §0 により分冊側で 00 を読み替えることはできないため、00 §1.1・§3.4 を「計算値は −29〜29（保存値）、表示レンジ 0〜30」に改版する必要がある（リスクの 00 §1.1「各 0〜100」と §1.5「負値を保存」も同じ書き方に揃えることが望ましい） | 00 改版まで: クランプせず保存（02 の `numeric(7,3)` は負値を許容）。レーダー描画時に 0 に丸める（06）。改版後は §2.2 補足を 00 への参照に置き換える | 00、02、03、06 |
