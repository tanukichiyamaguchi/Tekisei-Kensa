...
| 1.3 | 2026-09-21 | K-06（Supabase → Firebase。10）に伴う部分改版。§0.2 の決定事項 1、§1.3 の設問シード廃止、§2.1 の `assessmentSessions.answers`（map）、§2.2・§5.3・§5.4・§5.6・§5.8 の Firestore 保存の記述、§5.9 の `results` 文書への保存形（列名変換を廃止し `ScoreResult` をそのまま map で保存）、§6・§7.1 の母集団条件を `results` の複製フィールドによる Firestore クエリ（00 §1.11）と 02 の `fetchPopulation()` に置換、§7.6・§9.1 の保存型を number（倍精度）に変更（保存時の丸めが無いことを明記。D3-19）、§10.7・§11 の識別子を camelCase に統一。採点式・同点処理・値域・テスト観点は変更なし |
