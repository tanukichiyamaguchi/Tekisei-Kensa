...
| 版 | 1.3（1.1 版: レビュー指摘反映: T-14 境界値、§4.2・§5.10 誤記、§7.1 母集団条件、§7.3 未確認表記、§8.2 色注釈、D3-06・D3-14・D3-18。1.2 版: 最終点検で D3-06 の 00 改版要請が 00 1.1 版に反映されたことを §2.2・§5.7・§12 に反映。1.3 版: K-06（Supabase → Firebase）に伴う部分改版。1.4 版: Firebase 化後の分冊間整合（02 2.1 版の関数名に合わせた）。変更点は §13） |
| 作成日 | 2026-09-17（改版 2026-09-19、1.2 版 2026-09-21、1.3 版 2026-09-21、1.4 版 2026-09-21） |
...
flowchart LR
    ...
    SS --> SA
    SA --> R["ScoreResult"]
    R -->|"02 submitSession(): results 文書に保存 (toResultDocFields, map のまま)"| DB[("Firestore results")]
    DB -->|"02 fetchPopulation(): 母集団を取得"| P["PopulationRow[] (PopulationMember + resultId)"]
    R --> CW
    P --> CW
...
|---|---|---|---|
| `scoreAnswers(answers)` | `AnswerMap`（Q1〜Q144 → 1〜5） | `ScoreResult`（全指標、候補、タイプ、スタイル、信頼係数、`scoringVersion`） | 受検者の送信 API（`POST /api/v1/respondent/sessions/{sessionId}/submit`）の service が 02 の `submitSession(input, deps)` に `deps.scoreAnswers`・`deps.assertAnswerMap` として渡し、02 がトランザクション内で永続化済みの回答に対して呼ぶ（02 §8.4、04 §4.5）。結果は `results` に 1 文書で保存（map のまま。00 §2.1） |
...
- 回答の型はオブジェクトのキーが文字列に変換される点に注意します。`assertAnswerMap` は `Number(key)` で正規化して検証します（Firestore の `answers` map は文字列キーで読み戻される。02 の mapper `toAnswerMap()`（02 §5.5）が数値キーの `Record<number, number>` に変換してから渡すが、文字列キーのまま渡されても本関数は同じ結果を返す）。
...
| 6 | 比較値を閲覧者アカウントに書き込み | `ComparisonResult` を返すだけで永続化しない。Firestore へ書く関数を `lib/scoring/` に置かない | `compare.ts`、04 分冊 | 08 分冊（結合テスト: 比較 API 呼び出し後に `adminUsers` 文書が更新されないこと） |
...
- 母集団の各要素は `PopulationMember`（16 尺度と 5 軸のみ。00 §3.4）で、`results` 文書の `traits`・`compatibility` map から 02 の `toPopulationMember()`（`lib/db/mappers/`）が作ります。02 の `fetchPopulation({ organizationId, scope })` はこれに文書 ID を添えた `PopulationRow`（`PopulationMember & { resultId }`。02 §8.5）の配列を返し、04 が `includesSubject` の判定に使います。`PopulationRow` は `PopulationMember` の部分型なので、`compareWithPopulation` にはそのまま渡せます（本関数は `traits`・`compatibility` 以外を読みません）。`fetchPopulation()` は `select("traits", "compatibility")` で射影して読み（実装時確認: map フィールドへの射影が機能すること。機能しなければ全フィールドを読む。00 §1.11）、`TRAIT_KEYS`・`COMPATIBILITY_KEYS` の全キーが number で存在することを mapper が検証します（欠けていれば `compareWithPopulation` の平均が `NaN` になるため、mapper で `Error` にする）。
...
| 宛先 | 事項 |
|---|---|
| 02（Firestore データモデル） | (1) `results` 文書は `ScoreResult` の形をそのまま持つ（§5.9。map 6 つ + 単値 6 つ）。指標の値は §2.2 の値域（負値を含む）を number のまま保存し、書き込み前のスキーマ検証は「number であること・キー集合が `*_KEYS` と一致すること」までにとどめ、値域でのクランプや丸めを行わない。`aptitudeTypeScores` の 16 キーを必ず持つ。`scoringVersion` は string 必須。(2) `fetchPopulation({ organizationId, scope })` を `lib/db/repositories/results-repository.ts` に置き、00 §1.11 のクエリで `PopulationRow[]`（`PopulationMember` + `resultId`）を返す。`toPopulationMember()`（`lib/db/mappers/`）は `traits`・`compatibility` の全キーが number であることを検証する（§7.1）。(3) `ScoreResult` ↔ `results` 文書の mapper（`toScoreResult()` / `toResultDocFields()`）は値を変換・丸めしない。**1.4 版: (1)〜(3) は 02 2.1 版 §3.5、§5.5、§8.5 に反映済み** |
| 04（API・services） | 送信サービスは `assertAnswerMap` の `InvalidAnswerMapError` を 422 に、`EmptyPopulationError` を「比較不能」応答（409 相当。コードは 04 が定める）に変換する。`AnswerMap` は `assessmentSessions.answers`（文字列キーの map）を 02 の `toAnswerMap()` で数値キーにしてから渡す（そのまま渡しても動く。§2.1）。採点は 02 の `submitSession()` のトランザクション内で行うため、`scoreAnswers`・`assertAnswerMap` は `deps` として 02 に渡す（04 §4.5）。母集団は 02 の `fetchPopulation()` を 1 回呼んで取得し（§7.1 の条件を 04 で再定義しない）、本人（D-05）・幹部（D-06）を含めて `compareWithPopulation` に渡す。`ComparisonResult` は丸めずに返し、`populationSize` を含める。**1.4 版: 04 2.1 版 §4.5、§5.5 に反映済み** |
...
| D-05 | 決定済み（10 K-01） | 母集団に閲覧対象の受検者自身を含めるか | 含める。`compareWithPopulation` は渡された配列をそのまま平均する | 03、04 |
| D-06 | 決定済み（10 K-01） | 母集団に幹部の結果を含めるか | 含める | 03、04 |
...
| D3-06 | 設計判断（00 改版要請 → 00 1.1 版 D-26 で反映済み） | ソーシャルスタイル 4 値は付録B §7 の式から計算上 −29〜29 になり得る。00 §1.1・§1.7・§3.4 は 1.1 版で「計算値 −29〜29（保存値）、表示レンジ 0〜30」に、リスクも「計算値 −40〜100、表示 0〜100」に改版された | クランプせず保存（Firestore の number は負値をそのまま保持する。00 §2.1）。レーダー描画時に 0 に丸める（06 §6.4）。§2.2 補足は 00 への参照に置き換え済み | 00、02、03、06 |
...
| D3-19 | 設計判断（1.3 版。10 K-06） | Firestore の number（倍精度）には 1.x 版の `numeric(7,3)` のような保存時の丸めが無く、信頼係数の浮動小数誤差（例: 89.71000000000001）がそのまま保存される | 丸めの方針は変えない: 採点エンジンでも mapper でも丸めず、表示層（§9.2）だけで丸める。閾値判定は `THRESHOLD_EPSILON`（§7.6）で吸収し、テストは許容誤差 1e-9 で比較する。保存値を小数第 3 位で丸める案は、採点エンジン外に数値変換を持ち込み「そのまま読み戻せる」（00 §2.1）を崩すため採らない | 02、03、08 |
...
|---|---|---|
| 1.0 | 2026-09-17 | 初版 |
| 1.1 | 2026-09-19 | レビュー指摘反映（T-14 境界値、§4.2・§5.10 誤記、§7.1 母集団条件、§7.3 未確認表記、§8.2 色注釈、D3-06・D3-14・D3-18） |
| 1.2 | 2026-09-21 | 最終点検（09）。D3-06 の 00 改版要請が 00 1.1 版 D-26 に反映されたことを §2.2・§5.7・§12 に反映 |
| 1.3 | 2026-09-21 | K-06（Supabase → Firebase。10）に伴う部分改版。§0.2 の決定事項 1、§1.3 の設問シード廃止、§2.1 の `assessmentSessions.answers`（map）、§2.2・§5.3・§5.4・§5.6・§5.8 の Firestore 保存の記述、§5.9 の `results` 文書への保存形（列名変換を廃止し `ScoreResult` をそのまま map で保存）、§6・§7.1 の母集団条件を `results` の複製フィールドによる Firestore クエリ（00 §1.11）と 02 の `fetchPopulation()` に置換、§7.6・§9.1 の保存型を number（倍精度）に変更（保存時の丸めが無いことを明記。D3-19）、§10.7・§11 の識別子を camelCase に統一。最終確認で §1.1 の保存単位（「1 行」→「1 文書」）と §6 の 6 番（「DB」→「Firestore」）を改めた。採点式・同点処理・値域・テスト観点は変更なし |
| 1.4 | 2026-09-21 | Firebase 化後の分冊間整合。02 2.1 版・04 2.1 版に合わせ、§1.1・§5.9・§11 の呼び出し元を 02 の `submitSession()`（`deps` で `scoreAnswers`・`assertAnswerMap` を受け取る）と mapper `toResultDocFields()`・`toAnswerMap()` に、§7.1・§11 の `fetchPopulation({ organizationId, scope })` の戻り値を `PopulationRow[]`（`PopulationMember` + `resultId`）に更新。採点式・同点処理・値域・テスト観点は変更なし |
