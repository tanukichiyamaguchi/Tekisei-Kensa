...
flowchart LR
    ...
    SS --> SA
    SA --> R["ScoreResult"]
    R -->|"04: results 文書に保存 (map のまま)"| DB[("Firestore results")]
    DB -->|"02 fetchPopulation(): 母集団を取得"| P["PopulationMember[]"]
    R --> CW
    P --> CW
...
|---|---|---|---|
| `scoreAnswers(answers)` | `AnswerMap`（Q1〜Q144 → 1〜5） | `ScoreResult`（全指標、候補、タイプ、スタイル、信頼係数、`scoringVersion`） | 受検者の送信 API（`POST /api/v1/respondent/sessions/{sessionId}/submit`）。結果は `results` に 1 文書で保存（map のまま。00 §2.1） |
...
| 宛先 | 事項 |
|---|---|
| 02（Firestore データモデル） | (1) `results` 文書は `ScoreResult` の形をそのまま持つ（§5.9。map 6 つ + 単値 6 つ）。指標の値は §2.2 の値域（負値を含む）を number のまま保存し、書き込み前のスキーマ検証は「number であること・キー集合が `*_KEYS` と一致すること」までにとどめ、値域でのクランプや丸めを行わない。`aptitudeTypeScores` の 16 キーを必ず持つ。`scoringVersion` は string 必須。(2) `fetchPopulation(organizationId, scope)` を `lib/db/repositories/results-repository.ts` に置き、00 §1.11 のクエリで `PopulationMember[]` を返す。`toPopulationMember()`（`lib/db/mappers/`）は `traits`・`compatibility` の全キーが number であることを検証する（§7.1）。(3) `ScoreResult` ↔ `results` 文書の mapper（`toScoreResult()` / `toResultFields()`）は値を変換・丸めしない |
| 04（API・services） | 送信サービスは `assertAnswerMap` の `InvalidAnswerMapError` を 422 に、`EmptyPopulationError` を「比較不能」応答（409 相当。コードは 04 が定める）に変換する。`AnswerMap` は `assessmentSessions.answers`（文字列キーの map）をそのまま渡してよい（§2.1）。母集団は 02 の `fetchPopulation()` を 1 回呼んで取得し（§7.1 の条件を 04 で再定義しない）、本人（D-05）・幹部（D-06）を含めて `compareWithPopulation` に渡す。`ComparisonResult` は丸めずに返し、`populationSize` を含める |
