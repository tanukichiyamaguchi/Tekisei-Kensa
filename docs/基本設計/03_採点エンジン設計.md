...
5. **負値はクランプ指示のある指標だけ 0 にする**: 付録B に「負なら 0」と書かれている資質 4 型と信頼係数だけ `clampMin = 0` を持ち、相性 5 軸・リスク 7 項目・16 タイプ得点・ソーシャルスタイル 4 値は負値のまま返します。負値の表示は `lib/presentation/`（§8.4）で扱います。
...
│   │   └── social-styles.ts      # SOCIAL_STYLE_DEFINITIONS
│   ├── texts/                    # 付録C の文言（06 分冊）
│   └── occupations.ts
├── presentation/
...
| 1〜204 以外のキーが存在する | 無視する（設計判断: 将来の設問追加時に旧クライアントからの送信を落とさないため） |
...
補足: 00 §1.7 に記載のソーシャルスタイル 4 値の範囲「0〜30」は表示レンジ（付録E §1 の `yaxis max 30`）であり、計算上は負値になり得ます（全問「そう思わない」で −29）。設計判断として採点エンジンではクランプせず、DB にも計算値のまま保存します（§8.4、§12 の D3-06）。
...
- Q1〜Q143 のうち、Q64 と Q144 は指標に使われず信頼係数だけに使われます。
...
| 値域・刻み | −29〜29、0.25 刻み。00 §1.7 の「0〜30」は表示レンジ。設計判断: クランプせず計算値を保存（§12 の D3-06） |
...
|---|---|---:|---|---:|---:|
| 16 尺度 | 加算 7 問全て「そう思わない」、減算 7 問全て「そう思う」 | 0 − 14 + 14 = 0 | 加算 8 問全て「そう思う」、減算全て「そう思わない」 | 16 − 0 + 14 = 30 | 0.5 |
...
| 論理削除されていない | `results.deleted_at is null`（結合する `respondents`、`assessment_sessions` も同様） | 04 |
...
- `compatibilityAverages` は結果に含めますが、既存画面ではスライダーに比較対象マーカーを表示していません（付録E §4）。表示するかは 06 分冊の判断です。
...
色分けの値域は付録C §7 のとおりですが、信頼係数は逆順（高いほど緑系）にします（要件定義書 §11 の 5 番、00 §1.11）。合致度は既存踏襲（D-09、依頼主確認）。関数形だけ本書で定め、色の具体値と実装は 06 分冊が定めます。
...
export function reliabilityColor(displayValue: number): string; // band を逆順（高いほど緑系）
...
| T-14 | （追加） | 評価・立ち位置の境界: (合致度, 偏差値) = (80, 60) → A、(79.999999999, 60) → B、(60, 30) → D、(60, 29.9999999999) → E、(59.9999999999, 60) → E。偏差値 60 → `strong_leader`、59.9999999999 → `cooperative_leader`（許容誤差 1e-9 の内側は閾値成立、外側は不成立） | §7.4〜§7.6 |
...
| D3-06 | 設計判断 | ソーシャルスタイル 4 値は計算上 −29〜29 になり得る（00 §1.7 の「0〜30」は表示レンジ） | クランプせず保存。レーダー描画時に 0 に丸める | 02、03、06 |
...
| D3-14 | 設計判断 | `AnswerMap` に 1〜204 以外のキーがあっても無視する | エラーにしない | 03、04 |
