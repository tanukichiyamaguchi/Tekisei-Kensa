...
│   │   └── firebase-ops.yml          # 運用作業（create-owner など）。Environment production（Secret と必須レビュアー）で実行（§7.6、10 K-17、PR #12）
...
- `tests/fixtures/existing_results.json` は依頼主の既存システム由来です（tests/fixtures/README.md）。リポジトリは **公開（Public）** のまま運用します（10 K-17、§8.1）。
...
1. GitHub の Organization（または個人アカウント）でリポジトリを作成する（本リポジトリ `Tekisei-Kensa` をそのまま使う場合はこの手順は不要。本リポジトリは公開（Public）のまま運用する。10 K-17）。
...
- 本番の初期オーナーは、2026-09-23 に GitHub Actions の `firebase-ops` の `create-owner` で作成しました。オーナーはパスワード再設定メールで初期パスワードを設定し、ログインできています。`firebase-ops` は Environment `production`（Secret と必須レビュアー）で動きます。メールアドレスは `env` に置かず、ジョブの最初のステップで `$GITHUB_EVENT_PATH` の `.inputs.email` を読み、`::add-mask::` で伏せ字にします（以前は `env` に渡していたため、ステップ見出しにメールアドレスが表示されました。PR #12、10 K-17）。以下は PC から実行する場合の手順です。
...
| 検証用データ | `tests/fixtures/existing_results.json`（匿名化済み） | リポジトリは公開（Public）のまま運用する（10 K-17）。取り扱いは tests/fixtures/README.md に従う |
