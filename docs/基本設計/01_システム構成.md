...
| 要件定義書 §6.6、§10、付録D | 生成 AI 連携（API キーとモデル名の管理、既存のリクエスト形式は Gemini 形式だった） |
...
flowchart LR
    ...
    subgraph Vercel["Vercel（東京リージョン hnd1）"]
        ...
        AC["app/auth/callback（Supabase Auth コールバック）"]
...
├── .github/
│   ├── workflows/
│   │   ├── ci.yml                    # lint・typecheck・unit test（§6.2）
│   │   ├── e2e.yml                   # E2E（08 分冊が内容を定める。PR 時は手動起動可）
...
│   ├── seed/
│   │   └── questions.sql             # questions のシード（03 の lib/masters/questions.ts から生成）
│   ├── seed.sql                      # supabase db reset 時に読み込む入口（seed/ を \i で読む）
...
- `supabase/seed/questions.sql` は手書きせず、`scripts/generate-question-seed.ts` で `lib/masters/questions.ts` から生成します。設問文の正は 03 分冊の `lib/masters/questions.ts` です（00 §6 分冊間の境界）。
...
| `no-console: error`（`lib/utils/logger.ts` を除く） | ログは §8.6 の logger 経由に限定し、個人情報の混入経路を減らす |
...
| `NEXT_PUBLIC_APP_BASE_URL` | 受検リンク・管理者追加用リンクの生成（要件定義書 §6.2 A-12）、Auth のリダイレクト先の組み立て | あり | なし | ローカル `http://localhost:3000`、Preview はデプロイ URL を自動で使うため未設定可（§4.2）、Production は本番ドメイン | `https://{本番ドメイン}` |
...
| `AI_MODEL` | 使用モデル名。`ai_analyses.model` に保存 | なし | なし | 全環境 | 初期値は 07 分冊が確定する（D-17）。`.env.example` にはその値を記載する |
| `AI_PROMPT_VERSION` | プロンプト版。`ai_analyses.prompt_version` に保存 | なし | なし | 全環境 | `2026-09-17.1` のような文字列（形式は 07 分冊） |
...
- 環境変数を **追加する場合は 00 §3.2 に追記してから** 使います。本書の表は 00 と同じ名前のみを扱います。
...
- `publicEnv()` は Next.js のビルド時置換に合わせて `process.env.NEXT_PUBLIC_*` を個別に参照します（オブジェクト全体を渡すとブラウザ側で置換されないため）。
- `scripts/check-env.ts` は同じスキーマで `.env.local` を検証し、`pnpm dev` の前に実行できるようにします。

### 4.4 タイムゾーンとロケール

- DB は `timestamptz`（UTC）、API は ISO 8601 UTC（00 §2.1、§4.1）。表示は `lib/presentation/format-date.ts` の 1 関数に集約し、`Intl.DateTimeFormat("ja-JP", { timeZone: "Asia/Tokyo", ... })` で変換します。
...
ANTHROPIC_API_KEY=
# モデル名の初期値は docs/基本設計/07_AI解説とPDF.md が定める
AI_MODEL=
AI_PROMPT_VERSION=
...
| Deployment Protection | Preview を Vercel Authentication で保護（§8.8） | 個人情報を扱う画面を無防備に公開しない。検証データのみとはいえ保護する |
...
- 関数ごとの `maxDuration` と `memory` は `vercel.json` ではなく各 Route Handler の `export const maxDuration = 300;` で指定します（Next.js App Router のセグメント設定）。対象は §5.4。
...
| Auth: Allow new users to sign up | **無効**（設計判断 D01-09） | 管理者追加は招待トークン経由のみ。公開サインアップを止め、サーバが招待トークンを検証したうえで Auth Admin API でユーザーを作成する。この受理処理の配置は §5.6 |
| Auth: Confirm email | 無効（招待経由で作成するため）。パスワード再設定は有効 | 02 分冊がフローを確定 |
...
| Storage | バケット `pdf-exports`（非公開）。PDF を保存する設計を 07 が採用した場合のみ作成（§8.5） | 00 §2.1 の命名 |
...
- Auth セッションの Cookie 更新は Next.js の `middleware.ts`（`app/` と同階層）で `@supabase/ssr` の推奨手順に従い行います。`middleware.ts` は `/admin/**` と `/api/v1/admin/**` にだけ適用し、未認証なら `/admin/login` へリダイレクト（画面）または 401（API）を返します。認可（役割・`is_suspended`）の判定は middleware ではなく `lib/auth/` で行います（04 分冊）。
...
| **招待の受理（管理者追加）** | `app/auth/invite/route.ts`（`POST`。`/api/v1` の外、`app/auth/` 配下に置く。設計判断 D01-11） | **サービスロール**（招待トークンを `organizations` で検証 → `auth.admin.createUser` → `admin_users` に `role = admin` で作成 → ログイン用のセッション発行方法は 02 分冊） | 02（フロー・トークン）、04（入出力）、06（画面） |
| メール・パスワード変更（ログイン中） | `PATCH /api/v1/admin/me`（00 §4.2） | 利用者セッション（`updateUser`）＋必要なら サービスロール | 04 |
| 初期オーナーの作成 | 運用作業（§7.6）。Supabase ダッシュボードで Auth ユーザーを作成し、`organizations` と `admin_users` を SQL で投入 | サービスロール相当（ダッシュボード） | 02 |
...
| 有効期限 | 24 時間（設計判断 D01-12。受検の中断・再開（D-16）を同日中に限る。延長の要否は 05 分冊の再開仕様に合わせて調整可） |
| 発行 | `POST /api/v1/respondent/sessions` の応答で `Set-Cookie` |
| 検証 | `/api/v1/respondent/sessions/{sessionId}/**` で、Cookie のトークンと `{sessionId}` の組が `assessment_sessions` に存在し、`status = draft` であること |
    ...
    steps:
      ...
      - name: build（環境変数はダミー。実 API・実 DB には接続しない）
        ...
        env:
          ...
          AI_PROVIDER: stub
          AI_MODEL: dummy
          AI_PROMPT_VERSION: dummy
      - name: サービスロールキーがブラウザ用バンドルに含まれていないことを確認
        run: |
          if grep -R "dummy-service-role-key-do-not-ship" .next/static >/dev/null 2>&1; then
            echo "サーバ専用の値がクライアントバンドルに混入しています"; exit 1; fi
```
...
jobs:
  e2e:
    ...
    steps:
      ...
      - run: pnpm install --frozen-lockfile
      - run: supabase start                       # ローカル Supabase（Docker）。migrations と seed が適用される
...
- `supabase status -o env` が出力する変数名（`API_URL`、`ANON_KEY`、`SERVICE_ROLE_KEY` など）とアプリの変数名（`NEXT_PUBLIC_SUPABASE_URL` など）は異なるため、実装時に `scripts/` で読み替えるステップを追加します。
...
| ローカル適用 | `supabase db reset`（全適用＋ `seed.sql`） |
...
4. 最初のデプロイは環境変数が無いためビルドで失敗してもよい。Project Settings → Environment Variables で §4.1 の変数を **Production と Preview のそれぞれ** に登録する（値は §7.4、§7.5 で取得）。秘匿値（`SUPABASE_SERVICE_ROLE_KEY`、`ANTHROPIC_API_KEY`）は「Sensitive」を有効にする。
5. Project Settings → General → Node.js Version を `.nvmrc` と同じ版にする。
6. Project Settings → Deployment Protection で Preview を「Vercel Authentication」で保護する。依頼主が Preview を見る場合は Vercel のチームメンバーであること。
7. Project Settings → Domains で本番ドメインを追加し、表示される DNS レコード（CNAME または A）をドメイン事業者側に設定する（§7.3）。
8. （任意）Firewall で §8.4 のレート制限ルールを作成する。
...
7. Authentication → Providers で Email を有効、他を無効にする。Authentication → Settings（概ね「Sign In / Up」）で「Allow new users to sign up」を **オフ** にする（§5.3）。
...
新システムは空から開始する（決定事項 4）ため、最初の組織とオーナーは運用作業で作ります。SQL の実体は 02 分冊です。

1. Supabase ダッシュボード → Authentication → Users → 「Add user」で、オーナーのメールアドレスと初期パスワードを入力して作成する（「Auto Confirm User」を有効にする）。作成された User の UUID を控える。
2. SQL Editor で 02 分冊の「組織とオーナーの投入 SQL」を実行する（`organizations` に組織を作り、`admin_users` に手順 1 の UUID を `id`、`role = owner` で登録する）。
3. オーナーにログイン URL（`https://{本番ドメイン}/admin/login`）と初期パスワードを **別経路** で伝え、初回ログイン後にアカウント画面（S-08）でパスワードを変更してもらう。
4. 以降の管理者追加は、オーナーがアカウント画面の「管理者追加用リンク」を使って行う（要件定義書 §6.2 A-12）。運用作業は不要。
...
| 3 | Vercel Pro のプロジェクトが GitHub と連携し、Preview／Production の環境変数が揃っている | 環境変数一覧に §4.1 の 8 個がある |
| 4 | 本番ドメインが Vercel に割り当てられ HTTPS で開ける | ブラウザで確認 |
| 5 | Supabase の本番・検証プロジェクトが東京リージョンで作成され、Auth の設定（サインアップ無効、Site URL、Redirect URLs、SMTP、テンプレート）が済んでいる | ダッシュボードで確認 |
| 6 | GitHub Environment Secrets に Supabase のトークン類が登録され、`db-migrate.yml` が `staging` で成功する | Actions の実行結果 |
| 7 | Anthropic の API キーが Vercel に登録され、利用上限が設定されている | Console の Limits |
| 8 | 初期オーナーでログインでき、アカウント画面に 3 種のリンクが表示される | 本番でログイン |
...
- 第 3 層と第 4 層を **両方** 置きます。管理者 API は利用者セッションのクライアント（RLS 有効）で DB にアクセスするため、アプリの認可に漏れがあっても他組織のデータは返りません。受検者 API はサービスロール（RLS バイパス）のため、第 3 層のトークン検証が唯一の防御になります。したがって受検者 API は **`sessionId` とトークンの一致、`status = draft`、組織 ID の一致** を必ず検証し、更新は該当セッション 1 件に限定します（04 分冊）。
...
| 対象 | 方式 | 目安 |
|---|---|---|
| 受検者登録 `POST /api/v1/respondent/sessions` | Vercel Firewall のレート制限ルール（パスと IP 単位）。加えてアプリ側で、同一組織・同一 IP からの直近 10 分の作成件数を `assessment_sessions.created_ip_hash`（IP のハッシュ。02 分冊が列を確定）で数え、超過時は 429 を返す | IP あたり 10 分に 20 件（設計判断 D01-16。歯科医院の求職者登録は同時に多数発生しない） |
...
| 受検リンクの組織 ID | UUID のため推測は困難だが、リンクを知る者は誰でも受検登録できる（要件定義書 §9 で許容）。組織の `organizations.is_active`（02 分冊）で受付停止できるようにする | |
...
- 07 分冊が「保存せずに応答ストリームで返す」方式を選んだ場合、このバケットは作りません。
...
- PDF 生成時に Chromium が印刷用ページを開く際も同じヘッダーが適用されます。印刷用ページで外部リソースを使わない前提です。
...
| | 受検リンクに組織 ID が含まれる点は許容 | 既存踏襲（`?q={organizationId}`、D-12）。組織の受付停止フラグで対処可能に（§8.4） | 02、05 |
...
| | 回答をページ単位で保存し、離脱時に再開できることが望ましい | `PUT …/answers` でページ単位保存。セッショントークン Cookie（24 時間）で再開（§5.7、D-16） | 04、05 |
...
- ローカルの初期オーナーは `supabase/seed.sql` に **ローカル専用** の組織とオーナーを含めます（メールアドレスは `owner@example.com` のような例示用ドメイン、パスワードは README に記載）。本番のシード（`questions.sql`）とは別ファイルにし、本番には投入しません（`db-migrate.yml` の `seed` は `questions.sql` のみを実行）。
...
| D01-09 | 設計判断 | 公開サインアップ | Supabase Auth の「Allow new users to sign up」を無効化。管理者追加は招待トークンをサーバで検証し Auth Admin API で作成（§5.3、§5.6） | 02、04、06 |
...
| D01-12 | 未確認（要件定義書 §12: 受検の中断・再開可否） | 受検者セッション Cookie の有効期限 | 24 時間。Cookie 名 `tk_session`、`HttpOnly; Secure; SameSite=Lax; Path=/`（§5.7） | 04、05 |
...
| D01-16 | 設計判断 | 受検者登録のレート制限 | Vercel Firewall + アプリ側（IP ハッシュ、10 分 20 件）。`assessment_sessions` に IP ハッシュ列を持つ（列名は 02 が確定。本書案 `created_ip_hash`）（§8.4） | 02、04 |
...
| D01-27 | 設計判断 | 組織の受付停止 | `organizations.is_active`（列名は 02 が確定）で受検リンクの受付を止められるようにする（§8.4） | 02、04 |
