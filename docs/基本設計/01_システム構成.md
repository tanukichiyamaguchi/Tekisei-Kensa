...
| 版 | 1.1 |
| 作成日 | 2026-09-17 |
| 更新日 | 2026-09-19（レビュー指摘への対応。02〜08 分冊との整合: 管理者追加方式の一本化、初期オーナー作成手順、受検者 Cookie 有効期限、環境変数の追加、middleware の除外パス、CI・スクリプト名の統一。§12 の改版履歴を参照） |
...
| `PDF_TOKEN_SECRET` | PDF 印刷トークン（HMAC-SHA256）の署名鍵（04 §7.2、D04-40。07 §9.9）。`SUPABASE_SERVICE_ROLE_KEY` から派生させず専用に持つ | なし | **秘匿** | ローカル `.env.local`、Vercel（Preview／Production で **別の値**、Sensitive 指定）、GitHub Actions（E2E ではジョブ内で生成。§6.3） | `openssl rand -base64 32` で生成した 44 文字の文字列（32 バイト以上の乱数）。**00 §3.2 に未掲載（追記依頼。§12）** |
| `PDF_CHROMIUM_EXECUTABLE_PATH` | ローカルで PDF 生成に使う Chrome／Chromium の実行ファイルのパス（07 §9.12）。未設定ならローカルでは PDF 生成のみ「未設定」エラーになり、他の機能は動く | なし | なし | ローカル `.env.local` のみ（任意）。Vercel・CI には置かない（Vercel は `@sparticuz/chromium` を使う） | `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome` のような絶対パス。**00 §3.2 に未掲載（追記依頼。§12）** |
...
- 環境変数を **追加する場合は 00 §3.2 に追記してから** 使います。本書 1.1 版で追加した `PDF_TOKEN_SECRET`（04 D04-40）と `PDF_CHROMIUM_EXECUTABLE_PATH`（07 §9.12）は、他分冊からの追加依頼を受けて本書が先行して確定したものです。**00 §3.2 への追記を §12 の引き渡し事項として依頼します**（本書は 00 を編集しない）。`VERCEL`・`VERCEL_AUTOMATION_BYPASS_SECRET` は Vercel の自動設定変数のため、`VERCEL_ENV`・`VERCEL_URL` と同じく派生表の扱いとし、00 §3.2 への追記対象にはしません（設計判断 D01-30）。
...
jobs:
  e2e:
    ...
    steps:
      ...
      - name: Chromium と日本語フォント（PDF の E2E 用。08 §3.4.2）
...
- CI 上の PDF 生成は、`VERCEL` が未設定のため `lib/pdf/browser.ts` がローカル経路（`PDF_CHROMIUM_EXECUTABLE_PATH`）を選びます。E2E で PDF を検証する場合は、`playwright install` が入れた Chromium のパスを `PDF_CHROMIUM_EXECUTABLE_PATH` に渡すステップを 08 §3.4.2 の内容に従って追加します（本書の枠には含めない。07 §9.12）。
...
- ローカルの初期オーナーは `supabase/seed.sql` に **ローカル専用** の組織とオーナーを含めます（メールアドレスは `owner@example.com` のような例示用ドメイン、パスワードは README に記載）。本番のシード（`questions.sql`）とは別ファイルにし、本番には投入しません（`db-migrate.yml` の `seed` は `questions.sql` のみを実行。ローカルは `config.toml` の `sql_paths` で両方を読む。§6.4）。
...
select
  ...
  'owner@example.com', crypt('{README に記載するローカル用パスワード}', gen_salt('bf')), now(),
...
| D01-30 | 設計判断（04 D04-40、07 §9.12 の追加依頼） | 環境変数の追加 | `PDF_TOKEN_SECRET`（必須・秘匿・環境ごとに別の値）と `PDF_CHROMIUM_EXECUTABLE_PATH`（ローカル専用・任意）を §4.1 に追加し、起動時検証に加えた（§4.3）。`VERCEL`・`VERCEL_AUTOMATION_BYPASS_SECRET` は Vercel 自動設定のため派生表の扱い。**00 §3.2 への追記は引き渡し事項**（本書は 00 を編集しない） | 00、04、07、08 |
...
### 12.1 他分冊への引き渡し事項（1.1 版）

本書の改版により、次の分冊に記述の改版を依頼します。本書は指定されたファイル以外を編集しないため、各分冊の担当が反映してください。
