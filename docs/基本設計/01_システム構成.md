...
操作ごとの読み書き回数（推定。02・04 の確定内容で更新する）:
...
| 送信・採点 | 1 | 4 | セッションの `get` + `results`・`assessmentSessions`・`respondents`・`usageLogs` |
| 管理者 API 1 回あたりの共通コスト | 0〜1 | 0〜1 | クレームは Cookie に含まれ Firestore を読まない。`isSuspended` を毎回確認する場合は `adminUsers` を 1 回（00 §5 の実装時確認の結果による）。監査対象の操作は `auditLogs` に 1 回 |
| 結果詳細の初期表示 | 2 | 1 | `results` + `respondents` + 監査ログ |
| 比較 API（1 回） | **N**（母集団の件数） | 0 | `results` を等価条件でクエリ。N = 組織内の有効な結果件数（チーム選択時はそのチームの件数） |
| 回答一覧（1 ページ 50 件） | 50 + 50 | 0 | `results` のクエリ + `respondents` の `getAll()`（氏名の突き合わせ。00 §2.2）。全件表示にすると 2N |
| 組織内分類 | N + N | 0 | 分類 × タイプごとの該当者名を出すため `results` 全件と `respondents` 全件 |
...
| 回答一覧 200 回 × 100 | **20,000** | 0 |
| 組織内分類 50 回 × 600 | **30,000** | 0 |
| 管理者 API 共通 600 回 | 600 | 数十 |
| **1 組織の合計** | **約 112,000** | **約 1,000** |
| 10 組織の合計 | 約 1,120,000（1 日あたり約 37,000） | 約 10,000 |
...
- N が数千件に達する組織が現れた場合の対策（04 が性能要件に対して評価。00 D-30）: (1) 比較 API の母集団を `select("traits", "compatibility")` で射影してもコストは変わらないため、組織ごとの平均値を短時間キャッシュする、(2) 一覧のページネーションを必須にする（全件表示を提供しない）、(3) 組織内分類は `results` だけで人数を集計し、該当者名は分類を選んだときに読む。本書はこれらを **D01-44** として 04 に申し送ります（§12.1）。
...
| D01-16 | 設計判断（2.0 版で更新） | 受検者登録のレート制限 | Vercel Firewall + アプリ側（IP あたり 10 分 20 件）。判定は `auditLogs`（`organizationId`、`action == "respondent.register"`、`ipAddress`、直近 10 分）の件数で行う。Firestore の `count()` 集計クエリか取得件数かは 04、複合インデックスは 02（§8.4） | 02、04 |
| D01-17 | 設計判断（依頼主確認事項。10 K-03 で仮置き継続） | AI 生成の組織あたり日次上限 | 200 回を仮置き。`aiAnalyses` の件数で判定（§8.4） | 04、07 |
...
| D01-28 | 設計判断（2.0 版で更新） | 管理者追加（招待）の方式 | ブラウザは `POST /auth/invite` を呼び、Route Handler が招待トークン検証後に Admin SDK `createUser({ emailVerified: true })` → `setCustomUserClaims({ organizationId, role: "admin" })` → `adminUsers/{uid}` 文書を **明示的に作成** する（DB トリガーは存在しない）。確認メールは送らない（§5.7） | 02、04、06 |
...
| D01-41 | 設計判断（2.0 版） | ブラウザの Firebase Auth の永続化 | `inMemoryPersistence` にし、ID トークンを `POST /auth/session` に渡した直後に `signOut()` する。認証状態はセッション Cookie に一本化（§5.5）。Cookie の属性は `HttpOnly; Secure; SameSite=Lax; Path=/`、有効期限は 14 日を上限に 04 が定める | 04、06 |
...
| D01-44 | 設計判断（2.0 版。04 への申し送り） | 読み取り回数の見積もりと上限 | 比較・組織内分類・一覧の読み取りは母集団の件数 N に比例する（§10.1）。N が数千件に達する場合の対策（平均のキャッシュ、一覧のページネーション必須、組織内分類の該当者名の遅延読み込み）は 04 が性能要件に対して評価する（00 D-30） | 02、04 |
...
| D01-47 | 設計判断（2.0 版。推奨。確定は 02・04・06） | パスワード再設定の方式 | ブラウザのクライアント SDK `sendPasswordResetEmail`（Firebase がメールを送る）を推奨。アクション URL を `/admin/password-reset` にカスタマイズし `confirmPasswordReset` で完了する方式を推奨（§5.3、§5.7）。Admin SDK `generatePasswordResetLink` + 自前送信は送信手段が無いため採らない | 02、04、06 |
| D01-48 | 推定（2.0 版。04 への申し送り） | セッション Cookie の失効チェックの性能 | `verifySessionCookie(cookie, true)` は Firebase Auth への問い合わせを伴い、リクエストごとに遅延が加わり得る。00 §4.1 のとおり失効チェックありを既定とし、性能への影響と緩和策は 04 が評価する（§5.7） | 04 |

### 12.1 他分冊への引き渡し事項（2.0 版）

本書 2.0 版の内容のうち、他分冊（同時に Firebase 前提へ改版中）が取り込むべき事項です。1.x 版の引き渡し事項（Supabase 前提）はすべて廃止し、下表に置き換えます。

| 宛先 | 事項 | 本書の根拠 |
|---|---|---|
| 00（共通定義） | (1) §2.1「ロケーションは 01 が定める」→ `asia-northeast1` で確定。(2) §3.3 の `firebase.json` の「実装時確認」→ `firebase/` 配置 + `--config firebase/firebase.json` で確定（D01-37）。(3) §3.2 の派生表に `GOOGLE_APPLICATION_CREDENTIALS`・`FIREBASE_CI_SERVICE_ACCOUNT_KEY` が CI 専用として存在する旨の注記（追記不要と判断。D01-30）。(4) §3.3 の `scripts/` に `check-env` が含まれていることの確認（既載） | §2、§5.3、§5.6、D01-37 |
| 02（データモデル） | (1) `lib/firebase/admin.ts` の初期化関数（`adminFirestore()`、`adminAuth()`）と Emulator 分岐は本書 §5.5 の形を前提にする。(2) `scripts/create-owner.ts`・`scripts/set-admin-role.ts`・`scripts/seed-local.ts` の処理内容（組織文書、`adminUsers` 文書、クレーム）を定義し、本書 §7.6・§11 の引数（`--org-name` 等、`--organization-id`、`--email`、`--display-name`、Emulator 限定の `--password`）に合わせる。(3) `firestore.indexes.json` にレート制限用（`auditLogs`: `organizationId + action + ipAddress + createdAt`）と AI 日次上限用（`aiAnalyses`: `organizationId + createdAt`）の複合インデックスを含める（§8.4）。(4) 認可 3 段階の共通関数（`organizationId` の一致検証）を `lib/auth/`・`lib/db/` に置く（D01-42）。(5) `Doc` 型に `undefined` を使わず `null` を明示する（§5.5）。(6) パスワード再設定の方式は D01-47 の推奨を前提に確定する | §5.5、§5.7、§7.6、§8.4、§11、D01-42、D01-47 |
| 04（API） | (1) `POST /auth/session`・`DELETE /auth/session`・`POST /auth/invite` の入出力と、`middleware.ts` が Cookie の有無だけを見る前提（§5.5）。(2) セッション Cookie の `expiresIn`（14 日以内）と失効チェックの性能評価（D01-48）。(3) レート制限と AI 日次上限の件数判定方式（`count()` か取得件数か。§8.4）。(4) 読み取り回数の見積もり（§10.1）に対する一覧のページネーション・組織内分類の読み取り方針（D01-44）。(5) 受検者トークンの検証条件（§5.8）。(6) `PATCH /api/v1/admin/me` のパスワード変更はサーバの入力検証で D01-10 の強度を課す | §5.5、§5.7、§5.8、§8.4、§10.1、D01-44、D01-48 |
| 05（受検者画面） | 変更依頼なし。§5.8 の有効期限（7 日・延長）と検証条件（GET は `submitted` でも可）は 05 の確定内容に合わせた | §5.8、D01-12 |
| 06（管理者画面） | (1) ログイン画面は `lib/firebase/client.ts` の `clientAuth()` を使い、`setPersistence` 完了後に `signInWithEmailAndPassword` → `getIdToken()` → `POST /auth/session` → `signOut()` → `/admin` の順に処理する（§5.5、D01-41）。エラー文言はメール列挙保護を前提に 1 種類にする（§5.3）。(2) サインアップ画面は `POST /auth/invite` 成功後に (1) と同じ手順でログインする。(3) パスワード再設定画面は `sendPasswordResetEmail`（戻り先は `window.location.origin` 基準）と、アクション URL カスタマイズ時の `confirmPasswordReset` の 2 画面構成を前提にする（D01-47。確定は 02・04 と合わせる）。(4) メールテンプレートの日本語文言（件名・本文）を定め、依頼主に渡す（§7.4 手順 11） | §5.3、§5.5、§5.7、D01-41、D01-47 |
| 07（AI・PDF） | (1) `lib/pdf/browser.ts` は `isVercel()`（`lib/utils/env.ts`）で経路を選ぶ（§3.5）。(2) `VERCEL_AUTOMATION_BYPASS_SECRET` と `PDF_CHROMIUM_EXECUTABLE_PATH` は `serverEnv()` から読む。(3) Cloud Storage for Firebase は本フェーズで有効化しない（D01-45）。将来保存する場合の条件は §8.5。(4) `next.config.ts` の `serverExternalPackages` に `puppeteer-core`・`@sparticuz/chromium` を含めた（§8.7。実装時確認: 要否） | §3.5、§4.1、§8.5、§8.7 |
| 08（実装・テスト） | (1) CI は `ci.yml` で Emulator 上の結合テスト（`pnpm test:integration:emu`）を実行し、`e2e.yml` は `firebase emulators:exec` の中で `pnpm seed:local && pnpm test:e2e` を実行する（§6.2、§6.3）。Java のセットアップと Emulator の JAR キャッシュを含める。(2) テストごとの Emulator データクリア（REST API）の実装。(3) 全エンドポイントの他組織アクセス 404 テストと `firestore.rules` が全拒否の固定文字列であることの検査（D01-42）。(4) リリース前確認に「検証用プロジェクトへのインデックスのデプロイ → Preview で全クエリの実行」「スケジュールバックアップからの復元訓練」「Vercel Firewall ルールの運用文書化」を含める（§5.6、§6.4、§8.4、§9.3）。(5) `scripts/create-owner.ts` の結合テスト（Emulator に対する `owner:create` → ログイン）を検討。(6) コールドスタートを含む送信 API の応答時間を Preview で計測する（§5.4） | §5.4、§5.6、§6.2、§6.3、§6.4、§8.2、§9.3、D01-42 |
| 09（要件対応表） | §10 の対応表を Firebase 前提に更新した（RLS → サーバコードの組織検証、Supabase Auth → Firebase Auth + セッション Cookie、日次バックアップ → Firestore スケジュールバックアップ） | §10 |
