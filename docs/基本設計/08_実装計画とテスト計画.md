...
| ワークフロー | 実行するもの | 契機 |
|---|---|---|
| `ci.yml`（01 §6.2） | lint、format、typecheck、`vitest --project unit`（M-、T-、P-、X-、U-、PF-01、PF-02）、**Emulator 上の結合テスト**（`pnpm test:integration:emu`。I-、I-50）、build、バンドル検査（サーバ専用の値がブラウザ用バンドルに無いこと。`firebase-admin` の import 制限は ESLint。00 §3.3）、**§7.6 の再生成検査** | 全 push、PR |
...
| # | 準備物 | 用途 | 期限 | 手順の参照 | 備考 |
|---|---|---|---|---|---|
| 1 | GitHub リポジトリ（private）と実装者の Write 権限、Branch protection、Environments（`staging`、`production`。`firebase-deploy.yml` の対象） | 開発全般、CI | M0 着手時 | 01 | 本リポジトリをそのまま使う場合は権限設定のみ |
...
| 7 | 初期組織と初期オーナーを `pnpm owner:create`（`scripts/create-owner.ts`。本番プロジェクトのサービスアカウント鍵を実行時の環境変数で渡し、実行後に端末から消す。02 §9.6、01 §7.6）で作成する。受入用組織も同時に作る。Firebase コンソールからのユーザー追加は使わない（クレームと `adminUsers` 文書ができないため。§2.3） | 依頼主または運用責任者（実装者は手順の案内のみ。01） | オーナーがログイン画面の「パスワードをお忘れの方」から再設定メール（Firebase Auth の標準メール）を受け取り、設定後にログインできる。標準出力の管理者追加用リンクは運用者が安全な経路でオーナーに渡す |
