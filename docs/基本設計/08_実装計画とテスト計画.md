...
| 版 | 2.3 |
| 作成日 | 2026-09-18（1.1 版: 2026-09-21、1.2 版: 2026-09-21 最終点検、2.0 版: 2026-09-21 技術構成の変更（Supabase → Firebase。10 K-06）を反映、2.1 版: 2026-09-21 Firebase 化後の分冊間整合（01・02・04 の 2.0〜2.1 版に合わせた）。§11） |
...
| 01 分冊（2.1 版）§2、§3.4、§4、§5、§6、§7、§9 | ライブラリ（vitest、Playwright、`firebase`、`firebase-admin`、`firebase-tools`）、`package.json` のコマンド、環境変数の管理と起動時検証、Firebase プロジェクトの環境分離、CI（Emulator の起動、ルール・インデックスのデプロイ）、サービスアカウントの発行と保管、監視・バックアップ（Firestore のエクスポート） |
| 02 分冊（2.1 版）§3、§5、§7、§8、§9、§10、§13 | 各コレクションのフィールド定義と検証スキーマ、`Doc` 型、複製フィールドの同期手順（バッチ／トランザクション）、`firestore.rules`・`storage.rules`（全拒否）、`firestore.indexes.json`、`lib/db/`（collections、repositories、mappers）と `fetchPopulation()`、セッション Cookie の発行・検証・失効、初期オーナー作成（`scripts/create-owner`）、Emulator 用シード（`scripts/seed-local`）、監査ログの記録対象 |
...
| 04 分冊（2.1 版）§2、§4、§5、§6、§7、§8、§10 | エラーコード、電話番号の規則（D04-06）、長時間処理、`lib/services/`・`lib/auth/` のファイル構成、`/auth/session`・`/auth/invite`、認可チェックの実装箇所、結合テストの観点 |
| 09 分冊（1.2 版）§6.4 | K-06 で新たに「実装時確認」となった 19 件と、01・02・04・08 の 2.0〜2.1 版で追加された 20〜46 の確認時期（本書 §3.3.4 で M0〜M2 の確認項目に割り当てる） |
...
| M2 Firestore データモデル・認証・アクセス層 | `firebase/firestore.rules`（全拒否）、`firestore.indexes.json`、`lib/firebase/admin.ts`、`lib/db/`（collections、repositories、mappers）、`lib/auth/`、`scripts/create-owner`、`scripts/seed-local`、結合テスト基盤、検証用 Firebase プロジェクトへのルール・インデックスのデプロイ | 02、01 | M1（`questions.ts`・`ScoreResult` が正） |
...
| PR-0.3 | CI | `.github/workflows/ci.yml`（01 §6.2。lint／format／typecheck／unit／Emulator 上の結合テスト `pnpm test:integration:emu`／build／バンドル検査）、`e2e.yml`（01 §6.3。`firebase emulators:exec` の中で `pnpm seed:local && pnpm test:e2e`。§3.4.2）、`firebase-deploy.yml`（ルール・インデックスのデプロイ。手動・承認付き。01）、`dependabot.yml`、PR テンプレート（§1.2） | — |
...
- 空の Next.js アプリが `pnpm ci` と `pnpm build` に合格し、Vercel の Preview デプロイが作成される（Preview の環境変数は検証用 Firebase プロジェクトの値。01 が環境分離を確定）。
- ローカルと CI（`ci.yml` の `pnpm test:integration:emu`）で Firebase Emulator Suite（Auth、Firestore）が起動し、結合テストの最小 1 本（I-00）が Emulator に対して合格する。CI の Emulator 実行には Java ランタイムが必要なため、`ci.yml`・`e2e.yml` に `setup-java`（temurin 21）のステップを置く（01 §6.2。実装時確認: 必要な Java の版）。
...
- リポジトリ直下から `pnpm firebase`（`firebase --config firebase/firebase.json`）で Firebase CLI が動き、`.firebaserc` の別名が解決される（01 D01-37、09 §6.4 の 16・32。解決されない場合は `--project` を明示する）。
...
- Firestore のロケーションが作成後に変更できない点（09 §6.4 の 14）を踏まえ、作成済みプロジェクトのロケーション（`asia-northeast1`。00 §2.1、01 §5.3）を記録し、01 の構成と一致していることの承認。
...
| PR | 内容 | 主な成果物 | テスト ID |
|---|---|---|---|
| PR-2.1 | 文書型・ルール・インデックス | `lib/db/collections.ts`（`COLLECTIONS` 定数と型付きコレクション参照。00 §3.1）、`lib/db/types.ts`（8 コレクションの `Doc` 型と列挙の型。02 §5.3）、`lib/db/domain.ts`（ドメイン型。02 §5.4）、`lib/db/errors.ts`（`RepositoryError`。02 §5.6）、`lib/db/schemas/`（書き込み前の検証スキーマ。02 §2）、`firebase/firestore.rules`（全拒否。PR-0.2 の内容を確定）、`firebase/firestore.indexes.json`（02 §7 の Q1〜Q14 に対応する複合インデックス 12 本と単一フィールド除外 12 件） | I-01、I-50 |
| PR-2.2 | アクセス層 | `lib/db/mappers/`（`Doc` ↔ ドメイン型。`Timestamp` ↔ `Date`、`ScoreResult` ↔ `results` 文書）、`lib/db/repositories/`（コレクションごとの読み書き。02 §8.2 の関数一覧: `registerRespondent`、`saveAnswers`、`submitSession`、`updateRespondentFlags`、`softDeleteRespondent`、`listResults`、`fetchPopulation`（00 §1.11 のクエリ）、`markAiGenerationStarted`、`saveAiAnalysis`、`appendAuditLog` ほか） | U-01（mappers の往復）、I-00、I-36、I-37 |
| PR-2.3 | 認証・認可 | `lib/auth/session-cookie.ts`（`createAdminSessionCookie`／`verifyAdminSessionCookie`／`revokeAdminSessions`。02 §9.11）、`lib/auth/claims.ts`（`parseAdminClaims`、`canViewExecutives`。02 §9.2）、`lib/auth/respondent-token.ts`（乱数トークンの発行・SHA-256・照合。02 §9.10）、`lib/auth/invite-token.ts`（02 §9.7）、`lib/auth/admin-accounts.ts`（`createAdminAccount` ほか。02 §9.9）、`lib/auth/pdf-token.ts`（04 §7.2）、`lib/auth/admin-context.ts`（Cookie 検証 → クレーム → `adminUsers` 文書の取得。認可 3 段階の (1)(2)。04 §2.5.1）、`lib/auth/respondent-session.ts`（04 §2.5.2）、`middleware.ts`（Cookie の有無だけで遮断。`/admin/results/*/print` を除外。00 §3.3）、`app/auth/session/route.ts`（`POST`／`DELETE`。04） | U-02、I-02〜I-08 |
...
| PR-2.5 | 運用スクリプトと開発環境 | `scripts/create-owner.ts`（`pnpm owner:create`。`createOrganization()` → `createAdminAccount()`（Auth ユーザー作成（乱数パスワード）→ カスタムクレーム付与 → `adminUsers` 文書（ID = uid）→ 監査ログ）。メールは送らず、オーナーはログイン画面の「パスワードをお忘れの方」から再設定メールで初期パスワードを設定する。02 §9.6、01 §7.6）、`scripts/seed-local.ts`（Emulator 用の組織・オーナー・送信済み受検者の投入。本番には投入しない。02 §13.2）、`scripts/set-admin-role.ts`（役割変更・利用停止・削除・`--sync`。02 §9.9）、`scripts/verify-firebase.ts`（`pnpm firebase:verify`。検証用プロジェクトに対する実装時確認の実行。D08-33、01 §2）。各スクリプトは `main()` と処理関数を分ける（I-09）。`firebase-deploy.yml` を `staging` で実行し、ルール（全拒否）とインデックスを検証用プロジェクトにデプロイ。Vercel Preview の環境変数を検証用プロジェクトの値に設定 | I-09、H-01 |
...
- 検証用 Firebase プロジェクトで `firebase-deploy.yml` によりルール・インデックスがデプロイされ、Firebase コンソールでルールが全拒否になっている（H-01）。同プロジェクトで `pnpm owner:create` により初期オーナーを作成し、クライアント SDK の `sendPasswordResetEmail` で届いた再設定メールからパスワードを設定した後、クライアント SDK のログイン → `POST /auth/session` が成立する（画面はまだ無いため Node のスクリプトで確認）。検証用・本番用プロジェクトに対する `owner:create` の実行は依頼主または運用責任者が行う（実装者は Emulator に対してのみ実行する。01）。

依頼主の確認ポイント:

| ID | 内容 |
|---|---|
| 02、00 §5、00 §4.2 | 組織と初期オーナーは運用スクリプト `pnpm owner:create`（`scripts/create-owner.ts`。Admin SDK）で作成し、本番・検証プロジェクトに対しては依頼主または運用責任者が実行する手順の承認。Firebase コンソールからユーザーを追加した場合はカスタムクレームと `adminUsers` 文書が無いため管理 API を利用できない（00 §5）。コンソールでの追加は行わない運用とする |
...
| 01 | Firebase の料金プラン（従量制。Emulator はローカルのため課金なし）と、本番用・検証用の 2 プロジェクト構成、Preview が検証用プロジェクトに接続する構成（01 が確定） |
...
| 00 D-31、04 D04-53 | 管理者セッション Cookie の有効期限は 7 日・延長なし（04 で確定。既存のログイン保持期間は未確認のため依頼主確認事項。09 §6.1） |
| 02 D02-35、04 D04-56 | 管理者追加用リンクの平文はアカウント画面に常時表示せず、owner が「再発行」したときの応答でだけ 1 回表示する（要件定義書 §6.2 A-12 との差異。依頼主確認事項。09 §6.1） |
...
| PR-6.3 | 運用文書 | `docs/運用/`（初回リリース手順 §8.1、通常リリース §8.2、ロールバック §8.3、月次点検（01）、初期オーナー作成（`pnpm owner:create` の実行手順を運用者向けに転記）、役割変更・利用停止（`scripts/set-admin-role`）、招待トークン再発行、サービスアカウント鍵のローテーション（01）、Vercel Firewall のレート制限ルール（01 §8.4。`POST /auth/session`・`POST /auth/invite` を含む）、Firestore のスケジュールバックアップからの復元手順（01 §9.3、§8.3）） | — |
| PR-6.4 | 受入指摘の修正 | 受入テストで見つかった不具合の修正（複数 PR） | 該当テスト ID の追加・修正 |

完了条件:

- §4 の受入テスト A-01〜A-12 がすべて合格し、「意図的に異なる箇所」（§4.1）以外の差異がない。
- 本番環境で初期オーナーがログインでき、アカウント画面の受検リンク 2 種が本番ドメインで表示され、管理者追加用リンクを再発行できる（01 §7.8 のチェックリスト）。
- 検証用 Firebase プロジェクトで Firestore のエクスポート → インポートによる復元を 1 回試した記録がある（01、H-09。手順の詳細は実装時確認: エクスポート先の Cloud Storage バケットと `gcloud firestore export`／`import` 相当の操作）。
...
flowchart TB
    ...
    subgraph integ["結合（vitest, Firebase Emulator Suite（Auth 9099・Firestore 8080）, ci.yml）"]
    ...
    subgraph e2e["E2E（Playwright, Emulator + seed:local + next start, e2e.yml）"]
        E1["受検者: モバイル E-0x"]
        E2["管理者: PC E-1x, E-2x"]
    end
    subgraph manual["手動 H-xx（Preview（検証用 Firebase プロジェクト）／本番）"]
...
| 結合 | Firebase Emulator Suite（Auth、Firestore。`firebase emulators:exec` の子プロセス）+ Next.js の Route Handler を直接呼ぶ（`vitest --project integration`）。Admin SDK は `FIRESTORE_EMULATOR_HOST`・`FIREBASE_AUTH_EMULATOR_HOST` で Emulator に接続する（00 D-33） | すべての push（`ci.yml` の `pnpm test:integration:emu`。01 §6.2）、および開発者のローカル | API 契約・認可 3 段階（Cookie 検証・役割・組織一致）・トランザクション（二重送信拒否）・複製フィールドの同期・`fetchPopulation()` のクエリ結果・監査ログ |
| E2E | Emulator + `pnpm seed:local` + `next start`（`pnpm build` 済みのアプリ。01 §6.3）+ Playwright | `run-e2e` ラベル付き PR と `main`（`e2e.yml`）、および開発者のローカル | 画面遷移・表示・色・二重送信などブラウザでしか確認できないもの |
| 手動 | Vercel Preview（検証用 Firebase プロジェクト、実 API キー）と本番 | マイルストーン完了時、受入時 | Firebase Auth のメール、実 AI、PDF の見た目、実機、エクスポート／インポートによる復元 |

- 設計判断 D08-28 改（2.1 版）: E2E は 01 §6.3 に合わせ、`e2e.yml` が `pnpm build` → `firebase emulators:exec ... "pnpm seed:local && pnpm test:e2e"` の順に実行し、`playwright.config.ts` の `webServer` が `pnpm start`（ビルド済みの Next.js）を起動します。2.0 版の `next dev` 案（CI のステップ削減）は、01 が `pnpm build` を CI に含めている（ビルド失敗の早期検出）ため取り下げました。ローカルで開発サーバに対して E2E を回すときは `PLAYWRIGHT_BASE_URL` を `pnpm dev` の URL に向けます。
- 本番・検証の Firebase プロジェクトには自動テストから接続しません（00 D-33）。Emulator 専用のプロジェクト ID `demo-tekisei`（01 D01-38。`demo-` 接頭辞の扱いは実装時確認）を `NEXT_PUBLIC_FIREBASE_PROJECT_ID` に与え、万一 Emulator 用の環境変数が欠けても本物のプロジェクトに書き込まれないようにします。
...
- 10 K-19（PR #13・#14）で、付録D §5 に対応する `recruitment-v2` を `lib/ai/prompts/` に追加しました。本番・`.env.example`・CI の `AI_PROMPT_VERSION` は `recruitment-v2` です。`recruitment-v1` は残しており、U-11・U-15 は v1 を対象とした記述です。
...
/** Auth の disabled + revokeRefreshTokens + adminUsers.isSuspended を 00 §5 の手順どおりに行う（02 §9.9 の setAdminSuspended()。scripts/set-admin-role と同じ関数） */
...
- `tests/integration/setup/global-setup.ts` は Emulator が起動済みであること（`FIRESTORE_EMULATOR_HOST` に接続できること）を確認し、起動していなければ「`pnpm emulators` を先に起動するか `pnpm test:integration:emu`（`firebase emulators:exec` 経由）を使う」と案内して失敗します。CI では `ci.yml` が `pnpm test:integration:emu` で起動します（01 §6.2）。
...
| I-02 | 認可 (1) Cookie 検証: `GET /api/v1/admin/me` を (a) Cookie なし、(b) 改ざんした Cookie、(c) 有効期限切れ（`createSessionCookie` の `expiresIn` を最小値で発行し、時刻を進めるか短い期限の失効を待つ。方法は実装時確認）、(d) `revokeRefreshTokens` 後の Cookie、(e) 利用停止（`suspendAdmin()`。Auth の `disabled`）後の Cookie で呼ぶ | (a)〜(e) いずれも 401 `UNAUTHENTICATED`（04 §2.4。(e) で `verifySessionCookie(cookie, true)` が拒否しない場合は 403 `ADMIN_SUSPENDED` を `adminUsers.isSuspended` の確認で返す。00 §5、09 §6.4 の 6）。`loginAgain()` で作り直した Cookie では 200。`createAdminSessionCookie` が 7 日（`SESSION_COOKIE_MAX_AGE_MS`。04 D04-53）で発行できる | 00 §4.1、§5、D-31、04 §2.5.1、§6.1 |
...
| I-05 | クレーム不備: `organizationId` の無いクレーム、`role` が 3 値以外のクレーム、クレームなしの Auth ユーザーで作った Cookie で `GET /admin/me`（`POST /auth/session` はクレームを検査せず Cookie を発行する。04 D04-54） | 403 `ADMIN_NOT_REGISTERED`（00 §5「ログインできても管理 API を利用できない」。Cookie 自体は有効なため 401 ではない。04 §2.5.1） | 00 §5、04 §2.4 |
| I-06 | Auth にはあり正しいクレームもあるが `adminUsers` 文書が無いユーザー、および文書の `organizationId`／`role` がクレームと異なるユーザー（`writeDocForTest` で文書だけを書き換える例外書き込み）で管理者 API | いずれも 403 `ADMIN_NOT_REGISTERED`。不一致の場合は `auditLogs` に `admin.claims_mismatch`（`actorKind: "system"`）が 1 件残る（02 §9.2） | 04 §2.4、§2.5.1、02 §9.2 |
...
| I-09 | Emulator に対して `scripts/create-owner.ts`（`pnpm owner:create` と同じ処理関数を import して実行。02 §13.2。スクリプトはメールを送らない） | `organizations` 文書 1 件（`inviteTokenHash` あり。標準出力に招待リンクの平文が 1 回出る）、Auth ユーザー 1 件（カスタムクレーム `{ organizationId, role: "owner" }`、`emailVerified == true`）、`adminUsers/{uid}` 文書（`role == "owner"`、`isSuspended == false`、`deletedAt == null`）、`auditLogs` に `organization.create` と `admin.create`（`actorKind: "system"`）。テストがパスワードを設定（Admin SDK `updateUser`）した後、ID トークン → `POST /auth/session` で Cookie が得られ `GET /admin/me` が 200。同じメールアドレスで 2 回目の実行は `auth/email-already-exists` で失敗し文書が増えない。`scripts/seed-local.ts` を実行すると送信済み受検者 3 件（全問 1、全問 3、周期回答）と幹部 1 件が投入され、`GET /admin/results` が owner で 4 件を返す | 00 §3.3、§5、02 |
...
| I-11 | `POST sessions`（`kind = applicant`）→ `respondents`・`assessmentSessions`・`usageLogs` に各 1 文書（同一バッチ。02 `registerRespondent()`。`organizationId` が一致、`deletedAt: null` が明示的に存在、`respondents.resultId == null`・`teamCode == null`・`isExcluded == false`・`sessionId`・`usageLogId` が相互参照（02 §10 I-1）、`assessmentSessions.lastSavedPageNo == null`、`usageLogs.respondentKind == "applicant"`）、`auditLogs` に `respondent.register`、`Set-Cookie`（HttpOnly の受検者トークン。名前は 04）。`assessmentSessions.sessionTokenHash` が Cookie 値の SHA-256 と一致し、平文が Firestore のどこにも無い | 201 | 04 §4.2、00 §2.2、D-32 |
| I-12 | `PUT …/answers` をページ 1〜20 で呼ぶ → `assessmentSessions.answers` の map に `"1"`〜`"144"` のキーだけがあり値が 1〜5。ページ保存がフィールドパス指定の部分更新であること（ページ 2 を保存してもページ 1 の値が残る）。`questionNo = 145` を含む本文、`choiceCode = 6`、キーが数字でない本文 | 不正な本文は 422 `VALIDATION_ERROR`（サーバのスキーマ検証。Firestore には型制約が無いため到達させない）。`tokenExpiresAt` が保存のたびに延長され、`lastSavedPageNo` が保存したページ番号になる（02 D02-37） | 00 §2.1、§2.2、D-29、04 §4.4、D04-10 |
| I-13 | `POST …/submit`（144 問保存済み）→ `results` 1 文書（`scoringVersion == "1.0.0"`、指標 map 6 つのキー数が 16・5・4・7・16・4、`aptitudeFirst` 等が `AptitudeKey`／`AptitudeTypeKey`／`SocialStyleKey` の識別子）、複製フィールド `respondentKind == "applicant"`・`teamCode == null`・`isExcluded == false`・`deletedAt == null`、`assessmentSessions.status == "submitted"`・`resultId`、`respondents.resultId`、`usageLogs.resultId`・`submittedAt`、`auditLogs` に `session.submit` | 200。`results` の指標が `scoreAnswers(answers)` と完全一致（保存時の丸めなし。03 D3-19）。5 書き込み（`results` 作成 + 3 文書更新 + `auditLogs`）が同一トランザクションで反映される（途中状態が観測されない。02 `submitSession()`）。3 文書の `submittedAt` が同じ `Timestamp` | 04 §4.5、00 §2.2、§2.4、02 §8.4、03 §5.9 |
...
| I-18 | `kind = executive` で送信 → `results.respondentKind`・`usageLogs.respondentKind`（02 §3.7）が `executive` | サーバコードで複製される（トリガーは存在しない。00 §2.2、D-34） | 00 §2.2、02 |
...
| I-22 | `GET /admin/classification`・`GET /admin/usage-logs` を `admin` と `owner` で | 幹部が `admin` の人数・履歴に含まれない（`results.respondentKind`・`usageLogs.respondentKind` の複製フィールドで絞り込む。02 Q4・Q6）。利用履歴の応答が `{ items, total, page, pageSize }` で `total` が `count()` と一致（02 Q13） | 04 D04-31、§5.8、00 §5、§2.2 |
...
| I-33 | `POST /auth/invite { inviteToken, name, email }` を無効トークン・有効トークン（base64url 43 文字。`POST /admin/organization/invite-token` の応答から取る）で | 無効 404 `INVITE_TOKEN_INVALID`。有効で Auth ユーザーが作られ（02 `createAdminAccount()`。`emailVerified == true`、乱数パスワードが設定されるが応答・ログに現れない。02 D02-39）、カスタムクレーム `{ organizationId, role: "admin" }` と `adminUsers/{uid}` 文書（`role == "admin"`）、`auditLogs` に `admin.signup` ができる。応答に `password` を含めない。同じメールで 2 回目は 409 `EMAIL_ALREADY_REGISTERED`。途中失敗の回復（02 D02-40）: Auth ユーザーだけがある状態（テストが `createUser` のみ実行）から同じメールで要求すると 200 で完了しクレーム・文書が補完される。トークンは使い回せる（招待リンクは再発行まで有効）。再発行後は旧トークンが 404 になる | 04 §6.3、§5.2、02 §9.7、00 §4.2、§5 |
| I-34 | `PATCH /admin/me` を (a) `reauthIdToken` なしでメール変更、(b) 別ユーザーの ID トークン、(c) `auth_time` が 5 分より前の ID トークン、(d) 正しい `reauthIdToken` でメールアドレス変更、(e) 正しい `reauthIdToken` でパスワード変更、(f) `name` だけ変更 | (a) 422 `VALIDATION_ERROR`、(b)(c) 422 `CURRENT_PASSWORD_MISMATCH`（04 D04-47 改）。(d)(e) は 200 で `reloginRequired: true`、応答でセッション Cookie が削除され、旧 Cookie での `GET /admin/me` が 401（`revokeRefreshTokens`）。(d) は Admin SDK 側のメールアドレスが更新され `adminUsers` 文書にメールアドレスが保存されない（00 §2.2）。(f) は `adminUsers.displayName` に反映され `reloginRequired: false`。`auditLogs` に `account.update`（`details.fields` に項目名のみ） | 04 §5.1、D04-23、D04-47 改、D04-57、00 §2.2 |
...
| I-38 | `POST /auth/session` を (a) `auth_time` が 5 分より前の ID トークン、(b) 無効な ID トークン、(c) 有効な ID トークンで呼び、(c) の Cookie で `GET /admin/me` → `DELETE /auth/session` → 同じ Cookie で再度 `GET /admin/me` | (a)(b) 401 `ID_TOKEN_INVALID`。(c) 200 `{ expiresAt }`（発行から 7 日）+ `Set-Cookie`（HttpOnly、SameSite=Lax、Path=/）で `GET /admin/me` が 200。`DELETE` は 204 で Cookie 削除の `Set-Cookie`、その後の旧 Cookie は 401 `UNAUTHENTICATED`（`revokeRefreshTokens`）。`auditLogs` に `admin.login` は `POST /auth/session` では書かれず、`POST /admin/me/login-events` で 1 件書かれる（複数回呼んでも 1 回ごとに 1 件） | 04 §6.1、§6.2、§5.1、D04-24、D04-53、D04-54 |
| I-39 | `GET /admin/me` の `links` と `POST /admin/organization/invite-token` | `links.applicant`・`links.executive` が `NEXT_PUBLIC_APP_BASE_URL` 基準の URL（`q` が組織の文書 ID、`p` が `user`／`executives`）、`links.adminInvite` が常に `null`、`adminInviteIssuedAt` が `organizations.inviteTokenIssuedAt`。owner の再発行は 200 で `adminInvite`（`/admin/signup?q=` + base64url 43 文字）と `rotatedAt` を返し、`organizations.inviteTokenHash` が新しい平文の SHA-256 に置き換わり、`auditLogs` に `organization.rotate_invite_token`。`admin` の再発行は 403 `ROLE_REQUIRED`。平文が Firestore のどこにも無い | 04 §5.1、§5.2、D04-56、02 §9.7、D02-35 |
| I-37 | `fetchPopulation()` のクエリ結果: 組織に (a) 通常 2 件、(b) 除外 1 件、(c) 論理削除 1 件、(d) `scoringVersion` 旧 1 件（I-27 と同じ例外書き込み）、(e) チーム A 1 件、(f) 幹部 1 件、別組織に 1 件を投入し、`fetchPopulation({ organizationId, scope: { kind: "organization" } })` と `{ kind: "team", teamCode: "A" }` を直接呼ぶ | 前者は (a)(e)(f) の 4 件、後者は (e) の 1 件。`deletedAt == null` の等価条件が `null` を明示した文書に一致する（09 §6.4 の 1。一致しない場合は 02 が `isDeleted` を追加し本項の期待は変えない）。`select("traits", "compatibility")` の射影で `traits` map 全体が返る（09 §6.4 の 2。返らない場合は全フィールド読みに切り替え、本項は件数だけを検証）。戻り値が `PopulationRow[]`（`resultId` + `traits` + `compatibility`。02 §8.5）で氏名など個人情報を含まない | 00 §1.11、§3.4、02 §8.5、03 §7.1 |
...
| I-44 | `generating` かつ `results.aiGenerationStartedAt` が 11 分前の文書に GET（`aiGenerationStartedAt` は時刻のモックで作る。直接書き込みは使わない） | `failed` に戻り（`aiGenerationError == "timeout"`）、`auditLogs` に `result.ai_generate`（`status: "failed"`）が残る（02 `markAiGenerationFailed()`）。同じ状態で POST すると再開始される（02 `markAiGenerationStarted()` の `staleAfterMs`） | 04 D04-34、§5.9、02 §8.6、07 §6.3 |
...
| I-47 | 印刷用ページ（`/admin/results/{resultId}/print`）を、(a) トークンなし、(b) 署名改ざん、(c) 期限切れトークン、(d) 他組織の `resultId` を含むトークン、(e) パスの `resultId` とトークンの `resultId` の不一致、(f) トークンの `mode`／`scope` とクエリの不一致、(g) `admin` の `role` を持つトークンで幹部の `resultId` を指す（トークンのペイロードの `role` で可視性を再検証し、`adminUsers` は再読しない。04 §7.2、07 §9.4）、(h) 有効な管理者 Cookie のみ（トークンなし）で取得 | (a)〜(h) いずれも 404。管理者 Cookie なし + 有効トークンでは 200 で、応答に `X-Robots-Tag: noindex`、`Cache-Control: no-store`、`Referrer-Policy: no-referrer` が付く。ミドルウェアがログイン画面へリダイレクトしない | 04 §7.2、§10 (11)、07 §9.4、07/PDF-04、00 §3.3 |
...
| ID | ケース | 期待 | 根拠 |
|---|---|---|---|
| I-50 | `collectIndexWarnings()` で結合テスト全体（I-0x〜I-4x）の実行中に Emulator が出す「複合インデックスが必要」相当の警告を捕捉する（実装時確認: Emulator が警告を出す仕様と、その取得方法（Emulator のログ出力か、Admin SDK の例外か）。取得できない場合は、`lib/db/repositories/` の各クエリの `where`／`orderBy` の組み合わせを列挙し、`firestore.indexes.json` の定義と機械的に照合するテストに置き換える） | 警告 0 件。`firestore.indexes.json` の定義が 02 §7.1 の Q1〜Q14（`results` 4 本、`usageLogs` 2 本、`aiAnalyses` 2 本、`adminUsers`、`organizations`、`auditLogs` 2 本）と一致し、実装で使う全クエリ分の定義があり、使わない定義が無い | 00 §1.11、§2.1、D-33、02 §7 |
...
09 §6.4 の 46 件（1〜19 は K-06 時点、20〜46 は 01・02・04・08 の 2.0〜2.1 版で追加）を、本書のテスト ID とマイルストーンに割り当てます。各項目は「成立」または「代替に切り替え」を PR 本文に記録し、代替に切り替えた場合は該当分冊の改版 PR を先に出します（§1.2）。
...
| 4 | `createSessionCookie` の `expiresIn` の許容範囲 | I-02、I-38（7 日で発行できること。04 D04-53。14 日の上限値は `scripts/verify-firebase.ts` で確認） | M2 |
...
| 7 | 自己登録の無効化と Admin SDK `createUser` への非影響 | H-01（検証用プロジェクトのコンソール設定）、I-33、`scripts/verify-firebase.ts` | M2 |
| 8 | 再設定メールのアクション URL のカスタマイズ | H-02 | M4 |
| 9 | パスワードポリシーとエラーコード名 | E-18、H-02 | M4 |
| 10 | 現在のパスワードの検証方法 | 04 D04-47 改（ブラウザの再認証 + `reauthIdToken`）で確定。I-34、E-18 | M4 |
| 11 | `getAll()` の上限 | I-20（100 件を超える受検者を投入して一覧を取得。PF-05 のデータを流用。`getRespondentsByIds()` の 100 件分割。02 §8.4） | M4 |
| 12 | バッチ／トランザクションの書き込み上限 500 | I-13（5 書き込み）、I-32（4 書き込み）で上限内であることの確認のみ | M2〜M3 |
...
| 16 | `firebase --config firebase/firebase.json` での CLI 実行と `.firebaserc` の解決（01 D01-37） | PR-0.2、`ci.yml`・`e2e.yml` | M0 |
| 17 | 集計クエリの制約（日次上限の数え方） | 04 §2.8 で `countAiAnalysesSince()`（`aiAnalyses` の `count()`。当日 0 時以降。02 Q14）に確定。I-42 の周辺に「200 件投入後の POST が 429 `AI_DAILY_LIMIT_EXCEEDED`」を追加（`writeDocForTest` の例外を許容） | M5 |
| 18 | Storage の署名付き URL | 対象外（本フェーズは Storage 未使用。07 D07-21） | — |
| 19 | 倍精度 number の保存精度 | I-00、I-13 | M1〜M2 |
| 20 | `withConverter` 付き参照への `update()` の型付け（02 §5.2） | PR-2.2 の `typecheck`。合わなければコンバータ無し参照で `update()` | M2 |
| 21 | `create()` が既存文書を拒否すること（02 §8.3 `createAdminUser`） | I-09（2 回目の実行）、I-33（回復経路） | M2 |
| 22 | Admin SDK `getUsers()` の 1 回あたり上限 100 件（02 D02-01、04 §5.11） | I-08 の周辺に管理者一覧（`GET /admin/admin-users`）の項目を追加し、101 名で確認 | M4 |
| 23 | `DecodedIdToken.auth_time` の単位（秒）（02 §9.11、04 §6.1） | I-38 (a) | M2 |
| 24 | セッション Cookie に `email` クレームが含まれること（04 §2.5.1） | I-38 (c) の `GET /admin/me` の `email` | M2 |
| 25 | `updateUser({ password })`／`updateUser({ email })` が refresh token を自動失効させるか（04 §5.1） | I-34 (d)(e)（自動失効の有無にかかわらず `revokeAdminSessions` を呼ぶ設計は不変） | M4 |
| 26 | 乱数パスワードで作成した直後のユーザーへの `sendPasswordResetEmail` の成否と、メール列挙保護の影響（02 D02-39、04 D04-58） | H-02、`scripts/verify-firebase.ts` | M4 |
| 27 | `count()` 集計の不等号条件（`createdAt >=`）の可否と課金単位（02 §7.1 Q11・Q14） | I-11 のレート制限（21 件目が 429）、17 | M3 |
| 28 | `offset()` の読み飛ばし分の読み取り課金（02 §8.6 `listUsageLogs`） | PF-05 の周辺で Firebase コンソールの読み取り件数を記録（H-10） | M4 |
| 29 | トランザクションの「読み取りを書き込みの前に行う」制約と、コールバックが投げた例外で再試行しないこと（02 §8.1） | I-13、I-14、I-15 | M3 |
| 30 | `serverTimestamp()` が文書ごとに評価され得ることと、同一バッチで同じ時刻が必要な箇所の `Timestamp.now()`（02 §8.1） | I-13（`submittedAt`）、I-32（`deletedAt`）で 3 文書の値が一致 | M3 |
| 31 | `demo-` 接頭辞のプロジェクト ID の扱いとダミーの `apiKey`（01 D01-38） | PR-0.2、I-00 | M0 |
| 32 | `--config firebase/firebase.json` での `.firebaserc` の解決（01 D01-37） | PR-0.2、H-01（`--project staging`） | M0〜M2 |
| 33 | Firebase CLI が `GOOGLE_APPLICATION_CREDENTIALS` を認識すること（01 §6.4） | H-01（`firebase-deploy.yml` の成功） | M2 |
| 34 | CI 用サービスアカウントに必要な最小の役割（01 §5.3） | H-01 | M2 |
| 35 | `next.config.ts` の `serverExternalPackages`（`puppeteer-core`・`@sparticuz/chromium`）の要否（01 §8.7） | H-13 | M5 |
| 36 | Web API キーの制限後にログイン・パスワード再設定が動くこと（必要な API の一覧。01 D01-46） | H-06 | M6 |
| 37 | Firestore のスケジュールバックアップに必要なプランと設定箇所（01 §9.3 D01-43） | H-09 | M6 |
| 38 | Firebase Auth の組み込みメール送信の 1 日あたりの上限（01 §7.7） | H-02（上限値を運用文書 PR-6.3 に記録） | M4 |
| 39 | `firebase auth:export` にパスワードハッシュとパラメータが含まれること、`auth:import` の手順（01 §9.3） | H-09 | M6 |
| 40 | `@firebase/rules-unit-testing` の版と API（本書 I-01） | I-01 | M2 |
| 41 | Auth Emulator に対する Admin SDK の `createSessionCookie`／`verifySessionCookie` の動作（本書 §3.3.1） | I-02、I-38（動作しない場合は §3.3.1 の代替経路） | M0〜M2 |
| 42 | Emulator が生成する `oobCode`（再設定リンク）の REST での取得方法（01 §6.3、本書 E-18） | E-18 | M4 |
| 43 | Firebase Auth・Firestore のエラーコード名（04 §2.4 の対応表） | U-07、I-02、I-38 | M2 |
| 44 | Firestore 設定 `ignoreUndefinedProperties` の要否（01 §5.5、02 §5.3） | I-00（`undefined` を含む書き込みが拒否されること） | M2 |
| 45 | インデックス対象文字列の上限（約 1,500 バイト）と `fieldOverrides` による map 全体の除外（02 §7.2） | I-40（`rawText`・`output` の長い値で書き込みが失敗しない） | M5 |
| 46 | `verifySessionCookie(cookie, true)` のレイテンシ（01 D01-48、04 §2.5.1） | PF-03（管理者 API の応答時間に含めて計測） | M4 |
...
#### 3.4.2 `e2e.yml` の構成（01 §6.3 が正。本書は E2E 側の前提を示す）

2.1 版で CI の分担を 01 に合わせました: 結合テストは **`ci.yml`** が `pnpm test:integration:emu`（`firebase emulators:exec` の中で `vitest --project integration`）で毎回実行し、**`e2e.yml`** は `pnpm build` の後に `firebase emulators:exec` の中で `pnpm seed:local && pnpm test:e2e` を実行します（01 §6.2、§6.3）。Supabase 用の「変数名の読み替え」ステップ（1.x 版）は廃止し、Emulator 用の環境変数はジョブの `env` に直接置きます。PDF の結合・E2E テスト（I-45〜I-47、E-21）が必要とする Chromium のパス設定は **`pnpm exec playwright install --with-deps chromium` の直後** に置きます（07 §9.12）。

```yaml
# e2e.yml（01 §6.3 の抜粋。全体は 01 が正）
    env:
    ...
    steps:
      - uses: actions/setup-java@v4          # Emulator の実行に Java が必要（版は実装時確認）
      ...
      - name: Playwright の Chromium を PDF 生成に使う（07 §9.12）
        run: echo "PDF_CHROMIUM_EXECUTABLE_PATH=$(node -e "console.log(require('@playwright/test').chromium.executablePath())")" >> "$GITHUB_ENV"
      - run: pnpm build
      - name: Emulator を起動し、シードを投入して E2E を実行
        run: pnpm firebase emulators:exec --only auth,firestore --project demo-tekisei "pnpm seed:local && pnpm test:e2e"
```

- `FIREBASE_SERVICE_ACCOUNT_KEY` は CI に置きません（Emulator 使用時は省略可。00 §3.2、01 §4.3）。`e2e.yml`・`ci.yml` に Firebase の Secrets は一切不要で、`firebase-deploy.yml`（ルール・インデックスのデプロイ。01 §6.4）だけが `FIREBASE_CI_SERVICE_ACCOUNT_KEY` を GitHub Environment Secrets（`staging`／`production`）から読みます。
- `--project demo-tekisei` の値は `NEXT_PUBLIC_FIREBASE_PROJECT_ID` と一致させます（Admin SDK・クライアント SDK・Emulator が同じプロジェクト ID を見る必要があるため。01 D01-38。`demo-` 接頭辞の扱いは実装時確認）。
- Firebase CLI はリポジトリ直下から `pnpm firebase`（`firebase --config firebase/firebase.json`）で呼びます（01 D01-37）。`working-directory` の切り替えは行いません。
- Emulator のデータは `emulators:exec` の終了で破棄されるため、`--export-on-exit` は使いません。結合テストのテストファイルごとのクリアは §3.3.1 のとおりです。E2E はシード（`pnpm seed:local`。02 §13.2）の状態を前提にし、各シナリオが作る受検者は名前で区別します（E2E-{連番}）。
...
- 実行パスの取得に `require('@playwright/test').chromium.executablePath()` を使うのは、pnpm の厳密な `node_modules` では `playwright` パッケージが直接依存に無いと `require('playwright')`（07 §9.12 の例）が解決できないためです。
- `fonts-noto-cjk` のインストール（01 §6.3 では任意のステップとして残す）は 07 D07-20 により必須ではなくなりました。
- ローカルのコマンドは 01 §3.4 で確定済みです: `pnpm emulators`（Emulator の起動）、`pnpm test:integration`（起動済みの Emulator に対して実行）、`pnpm test:integration:emu`（`emulators:exec` で起動から停止まで）、`pnpm seed:local`、`pnpm test:e2e`。
...
| E-17 | 組織内分類・利用履歴・アカウント | 2×2 マトリクスの人数の合計が一覧の件数と一致（`admin` と `owner` で異なり得る）、分類クリックで説明、キャラクタークリックで該当者一覧（ひらがな名）、利用履歴ポップアップ、受検リンク 2 種のコピー、owner の「管理者追加用リンクを発行」で応答のリンクが 1 回表示されコピーできる（`admin` にはボタンが無い。04 D04-56、06 M-07） | 要件定義書 §6.2 A-06、A-11、A-12、D-04 |
...
| H-08 | 本番の初期オーナー作成 → ログイン → 受検リンク 2 種の表示と管理者追加用リンクの再発行 → 求職者リンクからの受検 → 一覧に表示（スモークテスト。受検者名は識別用文字列） | 本番 | M6 リリース直後 | 通し操作に失敗なし。テストデータは削除する |
| H-09 | Firestore のスケジュールバックアップ（日次、保持 7 日。01 §9.3 D01-43）を検証用プロジェクトで有効にし、バックアップから新しいデータベースへの復元を 1 回行う訓練（手順は 01 §9.3。実装時確認: 必要なプラン・権限、所要時間、`(default)` への反映方法、Auth ユーザーは Firestore に含まれないため `firebase auth:export`／`auth:import` を併用すること。09 §6.4 の 37・39） | 検証用 Firebase プロジェクト | M6 | 復元後にログイン・一覧表示ができる。手順書（PR-6.3）に所要時間を記録 |
...
| ワークフロー | 実行するもの | 契機 |
|---|---|---|
| `ci.yml`（01 §6.2） | lint、format、typecheck、`vitest --project unit`（M-、T-、P-、X-、U-、PF-01、PF-02）、**Emulator 上の結合テスト**（`pnpm test:integration:emu`。I-、I-50）、build（`pnpm build` は `next build --webpack`。Next.js 16.3.6 の Turbopack ビルドでは `outputFileTracingIncludes` が適用されず、`@sparticuz/chromium` の `bin/*.br` が PDF の関数に同梱されないため。PR #15）、build 後に `.next/server/app/api/v1/admin/results/[resultId]/pdf/route.js.nft.json` に `chromium/bin/chromium.br` が含まれることの grep による確認（PR #15）、バンドル検査（サーバ専用の値がブラウザ用バンドルに無いこと。`firebase-admin` の import 制限は ESLint。00 §3.3）、`node --no-experimental-require-module` で `firebase-admin/app`・`firebase-admin/auth`・`firebase-admin/firestore` を require できることの確認（Vercel の関数実行環境では CommonJS から ES Module を require できず、`firebase-admin/auth` → `jwks-rsa@4.1.0` → `jose@6` の読み込みで `ERR_REQUIRE_ESM` になり `/admin/login` が 500 になったため。`package.json` の `pnpm.overrides` に `"jwks-rsa>jose": "^5.10.0"` を置いて jose を 5 系に固定している。PR #11）、**§7.6 の再生成検査** | 全 push、PR |
| `e2e.yml`（01 §6.3 + §3.4.2） | `pnpm build` の後、`firebase emulators:exec` の中で `pnpm seed:local && pnpm test:e2e`（E-） | `run-e2e` ラベル、手動、`main` へのマージ後（設計判断 D08-06: `main` でも実行して回帰を検知する。01 に `on.push.branches: [main]` と、push イベントでジョブがスキップされないための `if: github.event_name != 'pull_request' \|\| github.event.label.name == 'run-e2e'` を置く） |
| `firebase-deploy.yml`（01。1.x 版の `db-migrate.yml` に相当） | `firebase --config firebase/firebase.json deploy --only firestore`（対象プロジェクトは `staging`／`production` の入力で選ぶ） | 手動（`production` は承認付き）。マイグレーションは存在しないため、デプロイ対象はルールとインデックスだけ |
...
| アカウント | 受検リンク 2 種のコピー、管理者追加用リンクの再発行と表示（owner のみ。再発行の応答でだけ表示。04 D04-56）、メール変更・パスワード変更（現在のパスワードによる再認証必須。04 D04-47 改）、変更後の再ログイン、ログアウト | A-12 |
...
| # | 準備物 | 用途 | 期限 | 手順の参照 | 備考 |
|---|---|---|---|---|---|
| 1 | GitHub リポジトリ（private）と実装者の Write 権限、Branch protection、Environments（`staging`、`production`。`firebase-deploy.yml` の対象） | 開発全般、CI | M0 着手時 | 01 | 本リポジトリをそのまま使う場合は権限設定のみ。10 K-17（2026-09-24 改定）により、リポジトリは非公開（Private）・GitHub 無料プランで運用する。無料プランの非公開リポジトリでは Branch protection と Environment を使えないため、`firebase-ops`・`firebase-deploy` の Secret・Variable はリポジトリ全体に置き、実行前の承認は無い |
| 2 | Vercel Pro チームと実装者の招待、GitHub 連携 | Preview／本番デプロイ | M0 着手時（Preview が M0 の完了条件） | 01、D01-13 | 環境変数の登録は実装者が行う。依頼主は Preview を見るためチームメンバーである必要がある。現在の運用は Vercel の Hobby プラン |
| 3 | Firebase プロジェクト（本番用・検証用の 2 つ。01 §4.2・D01-08）の作成、Firebase Authentication（メール＋パスワード）の有効化、Cloud Firestore（Native モード。ロケーションは作成時に確定し変更不可）の作成、実装者の招待（プロジェクトの編集者相当。権限の粒度は 01） | Firestore・Auth | M0 着手時 | 01、00 §2.1 | **依頼主が完了済み**（少なくとも 1 プロジェクト）。2 プロジェクト構成にする場合は検証用を M2 着手までに追加する。Google Cloud の請求先アカウントの紐付け（従量制）と予算アラートの設定も依頼主 |
...
| 6 | 本番ドメインの決定と DNS の設定権限 | 本番 URL、受検リンク、Firebase Auth の承認済みドメインとメール内リンク | M6 着手の 2 週間前 | 01 | 証明書発行と DNS 伝播の時間を見込む。**完了済み**: 本番 URL は依頼主のドメインのサブドメイン（Vercel のカスタムドメイン、DNS は CNAME）とし、Firebase Authentication の承認済みドメインに追加済み |
| 7 | Anthropic Console のアカウント、支払い方法、月額上限、API キー 2 本（本番・検証） | AI 解説 | M5 着手時 | 01、D01-15 | キーは作成時にしか表示されないため、Vercel への登録を同じ場で行う。API キーはワークスペースに属するもの（workspace-scoped）を発行する。属さないキーでは API が 400 `invalid_request_error`（`anthropic-workspace-id` ヘッダーを求めるメッセージ）を返す（2026-09-24 に発行し直して解決） |
...
| R-15 | 検証用データ（`existing_results.json`）は依頼主の既存システム由来であり、公開範囲の変更で扱いに注意が必要（tests/fixtures/README.md） | 情報管理 | 低 | リポジトリを private に保つ（§5 の 1）。公開する場合は依頼主の承認と当該ファイルの除外（2026-09-24 までは公開していたが、10 K-17 の改定で非公開にした。公開中に閲覧・複製された内容は取り消せない） | リポジトリ設定の確認 |
...
| R-20 | 管理者セッション Cookie の制約。`createSessionCookie` の `expiresIn` の上限（14 日と推定）を超える設定はできない。カスタムクレームを含む Cookie のサイズがブラウザの Cookie 上限（1 件 4 KB 程度と推定）に近づく。Cookie 発行時点のクレームが固定されるため役割変更が即時に反映されない | ログイン維持期間の制限、Cookie 拒否、役割変更の遅延 | 低 | 有効期限は 7 日・延長なし（04 D04-53）。I-02・I-38 で 7 日での発行を確認し、上限値は `scripts/verify-firebase.ts` で確認する。クレームは `organizationId`・`role` の 2 つだけにし（00 §5）、他の情報を入れない。役割変更・利用停止は `revokeRefreshTokens` で失効させ、`verifySessionCookie(cookie, true)` の失効チェックで次のリクエストから拒否する（I-02 (d)(e)。拒否されない場合は `adminUsers.isSuspended` を毎回確認。00 §5） | I-02、H-02 |
...
| 2 | `firebase-deploy.yml` を `production` で実行し、`firebase --config firebase/firebase.json deploy --only firestore`（`firebase/` 配下のルール・インデックス）を本番 Firebase プロジェクトに適用（`production` Environment の必須レビュアーが承認）。複合インデックスの構築完了（コンソールで「有効」）を待つ | 実装者 → 運用責任者が承認 | Actions の成功、コンソールでルールが全拒否・インデックスが有効 |
...
| 7 | 初期組織と初期オーナーを `pnpm owner:create`（`scripts/create-owner.ts`。本番プロジェクトのサービスアカウント鍵を実行時の環境変数で渡し、実行後に端末から消す。02 §9.6、01 §7.6）で作成する。受入用組織も同時に作る。Firebase コンソールからのユーザー追加は使わない（クレームと `adminUsers` 文書ができないため。§2.3）（実施済み: 本番の初期オーナーは 2026-09-23 に GitHub Actions の firebase-ops ワークフローの create-owner（10 K-17）で作成し、パスワード再設定メールで初期パスワードを設定してログインできた） | 依頼主または運用責任者（実装者は手順の案内のみ。01） | オーナーがログイン画面の「パスワードをお忘れの方」から再設定メール（Firebase Auth の標準メール）を受け取り、設定後にログインできる。標準出力の管理者追加用リンクは運用者が安全な経路でオーナーに渡す |
...
2. `firestore.rules`・`firestore.indexes.json` の変更を含む場合は、先に `firebase-deploy.yml`（`staging`）で検証用プロジェクトに適用し、Preview で確認（01）。
...
| 宛先 | 事項 |
|---|---|
| 01（構成） | **2.1 版: 01 2.0〜2.1 版に反映済み**（(1) `package.json` のコマンド名は 01 §3.4 で確定: `emulators`、`test:integration`、`test:integration:emu`、`test:e2e`、`seed:local`、`owner:create`、`admin:set-role`、`firebase:verify`。(2) `e2e.yml` は 01 §6.3、結合テストは `ci.yml`（01 §6.2）に配置。(3) `firebase-deploy.yml`（`--only firestore`、`staging`／`production`）。(4) 環境分離（本番・検証、Preview は検証）・`hnd1`・サービスアカウント（01 §4.6、§5.3）・スケジュールバックアップ（01 §9.3）。(5) `ci.yml` のバンドル検査（01 §6.2）。(6) メールは Firebase Auth のテンプレート（01 §5.3、§7.7））。以下は 2.0 版で依頼した内容の記録。1.x 版で反映済みの事項（`masters:generate`・`texts:generate`・`masters:check`、`ci.yml` の `pnpm masters:check`、`e2e.yml` の `on.push.branches: [main]` + `if` 条件（D08-06）、`PDF_TOKEN_SECRET`・`PDF_CHROMIUM_EXECUTABLE_PATH`）は 2.0 版でも維持を依頼する。**2.0 版での依頼**: (1) `package.json` に `emulators`（`firebase emulators:start --only auth,firestore`）、`test:integration`・`test:e2e`（`firebase emulators:exec` 経由）、`seed:local`（`scripts/seed-local.ts`）、`owner:create`、`set-admin-role` のコマンドを置く（名称は 01 が確定。本書は仮名で書いた）。(2) `e2e.yml` を §3.4.2 の構成（Java のセットアップ、Emulator 用の環境変数、`emulators:exec` 内での結合・E2E 実行、Firebase の Secrets を持たない）にする。`seed:questions` と Supabase 用の読み替えステップは廃止。(3) `db-migrate.yml` を `firebase-deploy.yml`（`--only firestore`。`staging`／`production` の選択、`production` は承認付き）に置き換える。(4) 環境分離（本番用・検証用の 2 プロジェクト、Preview は開発用に接続）と、Vercel の関数リージョン（東京。R-02）、サービスアカウントの最小権限とローテーション手順（R-19）、Firestore の定期エクスポートの設定（§8.1 の 11、H-09）を 01 で定める。(5) `ci.yml` に秘密情報の検出（R-19）を置く。(6) SMTP は契約しない（D08-29）ため、01 のメール関連は Firebase Auth のテンプレート設定（日本語化、送信元表示名、アクション URL）に置き換える。残る依頼はカバレッジ閾値（§3.1.2）の運用 |
| 02（データモデル） | **2.1 版: 02 2.1 版に反映済み**（(1) `fetchPopulation({ organizationId, scope })` は `PopulationRow[]`（02 §8.5）。(2) 書き込み単位は 02 §8.1・§8.2。(3) `deletedAt: null` の明示（02 §2）。(4) Q1〜Q14（02 §7）。(5) `main()` と処理関数の分離（02 §13.2）。(6) 時刻の差し替えは `lib/utils/clock.ts` ではなく、`tokenExpiresAt` を引数で受け取るリポジトリ関数の契約（02 §8.4）とテスト側の時刻モックで対応。(7) `lib/db/types.ts`・`schemas/` の配置（02 §5.1））。以下は 2.0 版で依頼した内容の記録。結合テスト I-00〜I-09、I-36、I-37、I-50 が前提とする事項: (1) `lib/db/repositories/` の `fetchPopulation(organizationId, scope)` が 00 §1.11 のクエリで `PopulationMember[]` を返す。(2) 受検登録・送信・チーム／除外更新・削除・AI 保存がそれぞれ 1 バッチまたは 1 トランザクションで、送信は `status` を再読み取りして二重送信を拒否する（I-15）。(3) すべての文書に `deletedAt: null` を明示的に書く（I-00、I-37）。(4) `firestore.indexes.json` が実装の全クエリを覆う（I-50）。(5) `scripts/create-owner.ts`・`scripts/seed-local.ts`・`scripts/set-admin-role.ts` の関数を結合テストから import できる形（副作用を持つ `main()` と、純粋な関数を分ける）にする（I-09）。(6) `assessmentSessions.tokenExpiresAt` を短くする、または時刻を差し替える経路（I-16。時刻取得を `lib/utils/clock.ts` のような 1 箇所にすることを提案）。(7) 文書型の名称（`Doc`）と `lib/db/types/`・`lib/db/schemas/` の配置は 02 が確定し、本書 PR-2.1 の仮置きを 02 の記載で読み替える。PF-05・I-27 のために Admin SDK で `results` に直接書く例外を許容 |
...
| D08-06 | 設計判断 | `e2e.yml` を `main` への push でも実行する | 回帰検知のため。01（1.1 版で反映済み。2.0 版でも維持されている。01 §6.3）に `on.push.branches: [main]` と、push でジョブがスキップされないための `if: github.event_name != 'pull_request' \|\| github.event.label.name == 'run-e2e'` を置く | 01 |
...
| D08-19 | 設計判断（2.0 版で更新） | 受入は本番環境の受入用組織で行い、完了後に受検者を論理削除する | 検証用 Firebase プロジェクトとの設定差異（Auth のメールテンプレート・承認済みドメイン・API キー制限、本番ドメイン、実キー、本番インデックス）を受入で検証するため | 01 |
...
| D08-28 | 設計判断（2.0 版。2.1 版で 01 §6.3 に合わせて改） | E2E は `pnpm build` 済みのアプリ（`next start`）に対して、`firebase emulators:exec` の中で `pnpm seed:local && pnpm test:e2e` を実行する | 2.0 版の `next dev` 案は取り下げ（01 が `pnpm build` を CI に含めるため）。ローカルでは `PLAYWRIGHT_BASE_URL` で開発サーバに向けてもよい（§3.1） | 01 |
...
| D08-33 | 設計判断（2.0 版。2.1 版で 46 件に拡張） | Firebase 固有の実装時確認事項（09 §6.4 の 46 件）の扱い | §3.3.4 でテスト ID とマイルストーンに割り当て、M0〜M6 で確認する。Emulator と本番の差異（R-22）に備え、本番の挙動に依存する項目は検証用プロジェクトに対する `scripts/verify-firebase.ts` の 1 回実行を M2・M4 の完了条件に含める | 01、02、04 |
| D08-34 | 設計判断（2.0 版。2.1 版で 01 D01-38 の `demo-tekisei` に統一） | テストの Firebase プロジェクト ID | Emulator 専用の ID `demo-tekisei`（`demo-` 接頭辞。慣例は実装時確認）を `NEXT_PUBLIC_FIREBASE_PROJECT_ID` と `--project` に与え、Emulator 用の環境変数が欠けても本物のプロジェクトに書き込まれないようにする（§3.1、§3.4.2） | 01 |
...
| 2.2 | 2026-09-24 | 本番デプロイで判明した事項の反映（§3.10 の `ci.yml` に `next build --webpack` と PDF 関数の `chromium.br` 同梱確認（PR #15）、`firebase-admin` の require 確認と jose 5 系固定（PR #11）。§3.2.2 に `recruitment-v2` の注記（10 K-19、PR #13・#14）。§5 の 1・2・6・7（リポジトリ公開 10 K-17、Vercel Hobby プラン、本番ドメイン完了、workspace-scoped の API キー）、§6 R-02・R-15、§8.1 の初期オーナー作成の実施記録（10 K-17）） |
| 2.3 | 2026-09-24 | リポジトリを非公開（GitHub 無料プラン）にした（10 K-17 の改定）。§5 の 1 と §6 R-15 を更新。`firebase-ops`・`firebase-deploy` は Environment を使わず、リポジトリ全体の Secret・Variable で動く（実行前の承認なし） |
