...
本書が定めないもの（参照のみ）: システム構成・CI の定義そのもの（01 分冊）、DDL・RLS（02 分冊）、採点式（03 分冊）、API の入出力（04 分冊）、画面（05・06 分冊）、AI 解説と PDF の生成方式（07 分冊）。07 分冊は本書と同時に執筆中のため、本書は 07 が定める事項を「07 分冊参照」とし、07 の確定を待つ箇所を §9 に列挙します。
...
| 01 分冊 §3、§6、§7、§9 | ライブラリ（vitest、Playwright）、`ci.yml`・`e2e.yml`・`db-migrate.yml`、依頼主の環境準備手順、監視・バックアップ |
| 02 分冊 §8、§10.3、§14 | 母集団取得関数、監査ログ、シードの検証（08 へ）、設計判断 D02-xx |
| 03 分冊 §4.5、§6、§7.7、§10、§11 | マスタ生成方針、不具合修正の反映点、検証用データの期待値、テスト設計 T／M／P、08 への引き渡し |
| 04 分冊 §2.4、§7、§10 | エラーコード、長時間処理、結合テストの観点 |
| 05 分冊 §12、§13 | 受検者画面のテスト観点 T-01〜T-19、設計判断 D05-xx |
| 06 分冊 §9、§10.6、§11、§12 | PDF との関係、マスタの不変条件 X-01〜X-10、テスト観点、設計判断 D06-xx |
...
- 設計判断 D08-01: M3 と M4 は M2 完了後に並行して進めます。両者は `lib/services/` と `app/api/v1/` の別ディレクトリ（`respondent/` と `admin/`）を触るため衝突が少なく、共通部分（`lib/auth/`、`lib/services/errors.ts`、`lib/services/audit.ts`、Route Handler の共通ラッパー `handle()`（04 D04-14））は M2 の最後の PR で先に入れます。
...
| PR-1.5 | 表示用の純関数 | `lib/presentation/{rounding,colors,trait-highlights,chart-series,comparison-scope-params,exam-pages}.ts`（03 §8〜§9、06 §5、§6.8、§10.2、05 §10.1） | P-01〜P-04、X-08〜X-10、05/T-01 |
...
| PR | 内容 | 主な成果物 | テスト ID |
|---|---|---|---|
| PR-2.1 | スキーマ | `supabase/migrations/`（02 §11 の 01〜17）、`supabase/config.toml`、`scripts/generate-question-seed.ts`（01）、`supabase/seed/questions.sql`（生成物）、ローカル専用 `supabase/seed.sql`（01 §11）、`lib/db/database.types.ts`（生成物） | I-01、I-02 |
...
| PR-2.3 | 認可 | `lib/auth/`（04 §2.5: 管理者コンテキスト、受検者トークン検証、`admin-role.ts`、`pdf-token.ts`）、`middleware.ts`（04 §8.5） | U-02、I-03〜I-06 |
| PR-2.4 | 共通基盤 | `lib/services/errors.ts`（04 §2.4）、`audit.ts`（04 §2.6）、`schemas/`（zod）、Route Handler 共通ラッパー `handle()`（04 §2.10）、結合テストのヘルパー（§3.3.2） | I-07、I-08 |
| PR-2.5 | 検証環境 | `db-migrate.yml` を `staging` で実行し、検証用 Supabase にスキーマとシードを適用。Vercel Preview の環境変数を検証用に設定 | H-01 |

完了条件:

- ローカル `supabase db reset` でマイグレーション 17 本とシードが成功し、`questions` が 204 行、`is_active = true` が 144 行（02 §10.3）。
- 結合テスト I-01〜I-08 が合格（RLS の組織分離、役割差、論理削除の不可視、監査ログの自己挿入制約）。
- Supabase ダッシュボードの Advisors で RLS の警告がゼロ（01 §9.1）。
- 検証用プロジェクトで初期オーナーを作成し（02 §7.2、01 §7.6）、Auth ログインが成立する（画面はまだ無いため Supabase の SDK で確認）。

依頼主の確認ポイント:

| ID | 内容 |
|---|---|
| D02-04、01 §7.6 | 組織と初期オーナーの作成を運用者（実装者）が SQL とダッシュボードで行う手順の承認 |
| D02-10、D02-17 | 論理削除の物理削除 180 日後、監査ログ保持 1 年の仮置き |
...
| PR | 内容 | 主な成果物 | テスト ID |
|---|---|---|---|
| PR-3.1 | 受検者 API | `app/api/v1/respondent/**`（04 §4）、`lib/services/{respondent-registration,answers,submission}.ts`、RPC `register_respondent()`・`finalize_assessment_session()` の呼び出し、レート制限（04 §2.8） | I-10〜I-16、PF-03 |
| PR-3.2 | 画面 | `app/(respondent)/exam/**`、`components/respondent/*`（05 §3、§5）、`lib/presentation/{exam-choices,registration-rules,exam-texts}.ts`（05 §10） | U-03、05/T-03〜T-07 の単体部分 |
...
| D05-16、D04-06 | 電話番号の検証規則（国内形式のみ） |
...
| PR | 内容 | 主な成果物 | テスト ID |
|---|---|---|---|
| PR-4.1 | 管理者 API | `app/api/v1/admin/**`（04 §5）、`app/auth/{callback,invite}/route.ts`（04 §6）、`lib/services/{admin-account,results,comparison,respondents,classification,usage-logs}.ts` | I-20〜I-32 |
...
| PR | 内容 | 主な成果物 | テスト ID |
|---|---|---|---|
| PR-5.1 | AI 連携 | `lib/ai/{types,provider,prompts/,validate}.ts`、`lib/ai/providers/{anthropic,stub}.ts`（07 分冊、00 §3.6、01 D01-07）、付録D §2 の zod スキーマ | U-10〜U-13 |
| PR-5.2 | AI の API と画面 | `POST/GET …/ai-analysis`（04 §5.9、§7.1）、`lib/services/ai-analysis.ts`、結果詳細の AI 解説セクション（06 §3.5） | I-40〜I-44、E-20 |
| PR-5.3 | PDF 生成 | `lib/pdf/`、印刷用ページ `app/(admin)/admin/results/[resultId]/print/page.tsx`、`components/pdf/*`、日本語フォントの同梱（§6 R-03、07 分冊） | U-14、U-02（pdf-token） |
| PR-5.4 | PDF の API と画面 | `GET …/pdf`（04 §5.10、§7.2）、ダウンロードポップアップ S-10（06 §3.5.11） | I-45〜I-47、E-21 |
| PR-5.5 | E2E と手動確認 | `tests/e2e/admin/ai-and-pdf.spec.ts`、Preview での実 API・実 PDF の確認記録 | E-20、E-21、H-03、H-04 |

完了条件:

- `AI_PROVIDER=stub` で生成 → 保存 → 再表示 → 非表示が動き、`ai_analyses` に 1 行、`results.ai_generation_status = completed`。
- 実 API（Preview、検証用キー）で 3 件以上生成し、すべて付録D §2 のスキーマ検証に合格する。生成時間の中央値を記録し、60 秒を超える場合は非同期化の判断を 07 と行う（04 §7.1）。
- PDF が A4 縦で 2 モード出力され、日本語が豆腐（□）にならない。`restricted` で評価・合致度・リスクが出ない（06 §3.5.11 の表）。
- PDF 生成が Vercel Preview で 120 秒以内、関数のデプロイサイズが上限内（01 §5.4）。

依頼主の確認ポイント:

| ID | 内容 |
|---|---|
| D-17 | 使用モデルと生成パラメータ（07 分冊が提案、依頼主が承認）。月額上限（D01-15） |
| D-20 | 付録D プロンプトの「コンタクター」を「コンダクター」に直すか |
| D01-17 | AI 生成の組織あたり日次上限 200 回 |
| D-18 | PDF のレイアウト（結果詳細と同一構成を A4 縦に組んだもの）。第二候補を続けて印字するか（06 §9） |
...
| PR-6.3 | 運用文書 | `docs/運用/`（初回リリース手順 §8.1、通常リリース §8.2、ロールバック §8.3、月次点検 01 §9.1、初期オーナー作成、招待トークン再発行、物理削除の実行） | — |
...
| `U-` | 単体（本書で追加。`lib/db/mappers`、`lib/auth`、`lib/ai`、`lib/services/schemas`、コンポーネント） | 本書 §3.2 |
| `I-` | 結合 | 本書 §3.3 |
| `E-` | E2E | 本書 §3.4 |
| `H-` | 手動 | 本書 §3.5 |
| `PF-` | 性能 | 本書 §3.9 |
...
| U-03 | `lib/presentation/registration-rules.ts`、`exam-choices.ts` | 氏名 1〜100 文字・改行不可、電話番号の NFKC 正規化と `^0[0-9]{9,10}$`、選択肢 5 件の表示順と `choice_code` の対応 | 05 §10.2、§10.3、D05-16、D05-17 |
...
| U-07 | `lib/services/errors.ts`、`handle()` | `ApiError` → JSON（`{ error: { code, message, details } }`）と HTTP ステータスの対応が 04 §2.4 の表と一致。予期しない例外は 500 `INTERNAL_ERROR` で `details` が空 | 04 §2.4、D04-14 |
| U-08 | `lib/utils/env.ts` | 必須変数の欠落で失敗、`AI_PROVIDER = stub` は `NODE_ENV = production` かつ `VERCEL_ENV = production` で拒否 | 01 §4.3、D01-07 |
| U-10 | `lib/ai/validate.ts` | 付録D §2 のスキーマ: 正常 JSON を受理。`verdict.sougou` の未定義値、`levers` が 4 件でない・順序違い・`label` 不正、`strengths` が 1 件、`questions` が 6 件、余分なキー、コードフェンス付きテキスト、先頭が `{` でないテキストを拒否 | 付録D §2、§3「出力形式」 |
| U-11 | `lib/ai/prompts/` | 付録D §3 の入力テンプレートの山括弧プレースホルダがすべて置換され、未置換の山括弧が残らない。信頼係数は `%` 付き、リスク 7 項目のラベルが 00 §1.5 の AI 入力ラベル、資質は表示名（直感型…）、職業は `occupations.ts` の表示名 | 付録D §3、00 §1.4、§1.5 |
| U-12 | `lib/ai/providers/stub.ts` | 決定的な `AiAnalysisOutput` を返し、`rawText` が JSON 文字列、`model` が `AI_MODEL` の値 | 01 D01-07 |
| U-13 | `lib/ai/provider.ts` | `getAiProvider()` が `AI_PROVIDER` に応じて `anthropic` / `stub` を返し、未知の値で失敗する。`signal` が中断済みなら provider が即座に失敗する | 04 §7.1、D04-38 |
| U-14 | `lib/pdf/` | `PdfSectionVisibility` が `restricted` のとき評価・合致度 ×2・リスク ×2 を非表示にする（06 §3.5.11 の表）。印刷用ページのデータ整形が 06 と同じ `lib/presentation/` を使う | 06 §9、03 §11 |
...
| I-43 | stub を「不正 JSON を返す」モードにして POST | 502 `AI_GENERATION_FAILED`、`ai_generation_status = failed`、`ai_generation_error = "invalid_json"`、`ai_analyses` に行が増えない。再度 POST で `generating` に戻り成功する | 02 D02-14、04 §7.1 |
| I-44 | `generating` かつ `ai_generation_started_at` が 11 分前の行に GET | `failed` に戻る | 04 D04-34 |
| I-45 | `GET …/pdf?mode=full`、`?mode=restricted`、`?mode=x` | 前 2 者は `application/pdf` で 1 KB 以上、`%PDF-` で始まる。後者は 422 | 04 §5.10 |
| I-46 | `GET …/pdf?scope=team&teamCode=C`（母集団 0） | 409 `POPULATION_EMPTY` | 04 D04-35 |
| I-47 | 印刷用ページを、期限切れトークン・他組織の `resultId` を含むトークン・`admin` の ID で幹部の `resultId` を指すトークンで取得 | いずれも 404 | 04 §7.2 |
...
01 §6.3 のワークフローに次の 2 ステップを追加します（01 §6.3 末尾の「読み替えステップ」の具体化）。

```yaml
      - name: supabase status の変数名をアプリの変数名に読み替える
        run: |
      ...
      - name: Chromium と日本語フォント（PDF の E2E 用）
        run: sudo apt-get update && sudo apt-get install -y fonts-noto-cjk
```

- `PDF_TOKEN_SECRET` は 04 D04-40 で追加された環境変数です（01 §4.1 への追記待ち。§9）。
- `fonts-noto-cjk` は CI ランナー上の Chromium 用です。本番（Vercel）では §6 R-03 のとおりフォントを同梱します。
...
| E-21 | PDF ダウンロード | ポップアップで 2 モード選択 → ダウンロード → ファイルが `application/pdf` で 1 KB 以上。`restricted` の PDF テキストに「評価」の値と「合致度」「リスク」の数値が含まれない（`pdf-parse` 相当でテキスト抽出） | 要件定義書 §6.2 A-10、06 §3.5.11 |
...
|---|---|---|
| 1. 値の範囲と刻み | T-10b | 73 件の 16 尺度・リスク・信頼係数、`現行ロジック整合 = true` の 31 件の相性・資質・スタイル。判定関数は `lib/scoring` の値域定義（03 §5.10）を import して使う（テストに範囲を直書きしない） |
...
| `e2e.yml`（01 §6.3 + §3.4.2） | `vitest --project integration`（I-）、`playwright test`（E-） | `run-e2e` ラベル、手動、`main` へのマージ後（設計判断 D08-06: `main` でも実行して回帰を検知する。01 §6.3 の `on` に `push: branches: [main]` を追加） |
...
- AS-3 で `full` と `restricted` をダウンロードし、H-04 の基準で確認する。`restricted` に評価・合致度・リスクが無い。比較組織を選んだ状態でダウンロードすると比較系が印字される（D06-16）。
...
| 8 | 使用モデルと日次上限の承認 | AI 解説 | M5 着手時 | 07 分冊、D-17、D01-17 | 07 分冊の提案に対する回答 |
...
| R-03 | PDF の日本語フォント。`@sparticuz/chromium` が日本語フォントを同梱しない可能性（06 §9） | PDF の日本語が豆腐になる | 高 | 設計判断 D08-09: 日本語フォント（Noto Sans JP 相当のオープンライセンスのもの）を `lib/pdf/fonts/` に同梱し、印刷用ページで `@font-face` として読み込む（`public/` に置かず、Vercel のデプロイサイズ上限に注意してサブセット化を検討）。07 分冊が方式を確定。M5 の最初の PR（PR-5.3）で Preview 上の PDF を目視する | H-04、E-21 の PDF テキスト抽出で日本語が取れること |
| R-04 | Vercel の関数デプロイサイズ上限（Chromium + フォント） | PDF 関数がデプロイできない | 中 | フォントのサブセット化、Chromium の版固定、PDF の Route Handler を独立させて他の関数に影響させない。超える場合の代替（`@react-pdf/renderer` 等）は 07 分冊 | Vercel のビルドログ |
| R-05 | AI の出力ばらつき。付録D §2 のスキーマに合わない JSON、コードフェンス付き、判定語彙の逸脱、氏名の多用 | 生成失敗（502）、表示崩れ | 中 | zod で厳密検証（U-10）し、失敗は `failed` にして再試行可能にする（D-10、D02-14）。07 分冊で、モデルの構造化出力機能（応答を JSON スキーマに制約する仕組み）の利用を検討し、利用する場合は U-10 のスキーマと同一定義から生成する（設計判断 D08-10: スキーマの二重管理を避ける）。H-03 で 3 件以上を実測し、失敗率が 10% を超えるならプロンプトの「出力形式」節を強化する | `ai_generation_error` の集計、H-03 |
...
| R-12 | 07 分冊の確定待ち事項（モデル、PDF 方式、Storage 利用の有無）が M5 着手までに決まらない | M5 の遅れ | 中 | M5 着手条件として §9 の一覧を 07 と確認する。PDF は 01 D01-04 の推奨方式で先行実装し、07 が変更したら差し替える | M5 着手時 |
| R-13 | 電話番号の検証規則（D05-16）が実運用と合わない（国際番号、内線） | 登録できない受検者が出る | 低 | M3 の確認ポイントで承認を得る。規則は `registration-rules.ts` 1 箇所（05 §10.3） | 受入 A-03、運用後の問い合わせ |
...
| 5 スタイル | 00 §1.7（付録B §7、付録E §1、§6） | 式を持たないため、生成スクリプト内の定数として `SocialStyleDefinition` 4 件を書き出す（`sortOrder` と `chartOrder` を別に） | 4 件、`chartOrder` が `1,2,4,3`（driving, expressive, amiable, analytical の順） |
...
| §1 各タイプ | `### {タイプ名}` の下に `**特徴**`、`**適性職種**`、`**アドバイス**` の見出しとコードブロック | `type-texts.json` の `feature`、`suitableJobs`、`advice`（フィールド名は 06 §4.2 の `AptitudeTypeTexts` に合わせる） | 16 タイプ × 3 本文、空でない |
...
| 6 | 初期組織とオーナーを作成（01 §7.6、02 §7.2）。受入用組織も同時に作る | 実装者（依頼主立ち会い） | オーナーがログインできる |
...
| 宛先 | 事項 |
|---|---|
| 01（構成） | `package.json` に `masters:generate`、`texts:generate`、`masters:check` を追加（§7.1）。`ci.yml` に `pnpm masters:check` ステップを追加（§7.6）。`e2e.yml` に §3.4.2 の 2 ステップと `on: push: branches: [main]` を追加（D08-06）。環境変数 `PDF_TOKEN_SECRET`（04 D04-40）を 01 §4.1 と 00 §3.2 に追記。カバレッジ閾値（§3.1.2）の運用 |
| 02（DB） | 結合テスト I-01〜I-08、I-50 が前提とする `GRANT`・ポリシー名（`audit_logs_insert_self`）・関数名（`soft_delete_respondent()`、`fetch_population()`、`purge_deleted_respondents()`）は 02 の記載を正とし、改名時は本書の ID 表を更新する。PF-05 のためにサービスロールで `results` に直接 INSERT する例外を許容 |
| 03（採点） | `tests/unit/scoring/helpers.ts` の `uniformAnswers`／`cyclicAnswers` を結合・受入からも再利用する（`tests/unit/` 外からの import を許すため `tests/support/answers.ts` に移動することを提案）。M-07 の対象範囲拡大（D08-04） |
| 04（API） | 結合テストの `callRoute` が Route Handler の `GET`／`POST` を直接呼べる形（`(req, ctx) => Promise<Response>`）であること。`handle()` が `X-Request-Id` を応答に含めること（U-07） |
| 05（受検者画面） | E2E E-05 のために「次へ」でのネットワーク要求が `PUT …/answers` 1 本であること。テスト用に `data-testid` を各設問カード・ラジオ・ボタンに付与 |
| 06（管理者画面） | 文言マスタを `generate-texts.ts` の生成 JSON から読む構成（D08-11。型・エクスポート名は不変）。`PositionDefinition.description` は `descriptionLines` の改行結合（D08-12）。E-14 のためにゲージの `circle` 要素に `data-testid` と `stroke` 属性で色を持たせる。E-12 のイラスト `img` の `src` 規約 |
| 07（AI・PDF） | 07 が確定すべき事項で本書が待つもの: (1) `AI_MODEL` の初期値と生成パラメータ（D-17）、(2) 構造化出力機能の利用有無と U-10 スキーマの共有（D08-10）、(3) PDF の生成方式・フォント同梱方法（D08-09、R-03、R-04）、(4) Storage 利用の有無（02 §9）、(5) `stub` provider の「不正 JSON を返すモード」の切替方法（I-43 用。提案: 環境変数ではなく `AiAnalysisInput.respondentName` が `"__INVALID_JSON__"` のとき）、(6) 印刷用ページで第二候補を続けて印字するか（06 §9） |
...
| D08-06 | 設計判断 | `e2e.yml` を `main` への push でも実行する | 回帰検知のため。01 §6.3 の `on` に追加 | 01 |
...
| D08-09 | 設計判断（07 が確定） | PDF の日本語フォントをリポジトリに同梱し `@font-face` で読み込む | サブセット化でデプロイサイズを抑える | 07、01 |
| D08-10 | 設計判断（07 が確定） | AI の構造化出力機能を使う場合、U-10 の zod スキーマと同一定義から生成する | スキーマの二重管理を避ける | 07 |
...
| D08-22 | 設計判断（07 への提案） | `stub` provider の不正 JSON モードは `respondentName = "__INVALID_JSON__"` で切り替える | 環境変数を増やさない | 07 |
