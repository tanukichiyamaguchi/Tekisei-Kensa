...
| I-22 | `GET /admin/classification`・`GET /admin/usage-logs` を `admin` と `owner` で | 幹部が `admin` の人数・履歴に含まれない（`results.respondentKind` の複製フィールドで絞り込む。利用履歴側の区分の持ち方は 02） | 04 D04-31、00 §5、§2.2 |
