...
| # | 準備物 | 用途 | 期限 | 手順の参照 | 備考 |
|---|---|---|---|---|---|
| 1 | GitHub リポジトリ（private）と実装者の Write 権限、Branch protection、Environments（`staging`、`production`。`firebase-deploy.yml` の対象） | 開発全般、CI | M0 着手時 | 01 | 本リポジトリをそのまま使う場合は権限設定のみ。10 K-17 により、リポジトリは公開（Public）のまま運用する。Environment `production`（Secret と必須レビュアー）は継続する |
...
| R-15 | 検証用データ（`existing_results.json`）は依頼主の既存システム由来であり、公開範囲の変更で扱いに注意が必要（tests/fixtures/README.md） | 情報管理 | 低 | リポジトリを private に保つ（§5 の 1）。公開する場合は依頼主の承認と当該ファイルの除外（10 K-17 により、リポジトリは公開（Public）のまま運用している） | リポジトリ設定の確認 |
